...
DEF NEW SHARED VAR i-emp-s LIKE emsuni.empresa.cod_empresa.
...
FOR EACH tt-resumido:
    DELETE tt-resumido.
END.
IF tt-param.rt-tipo-relat = 2 THEN DO:
    FOR EACH tt-detalhe:
        DELETE tt-detalhe.
    END.
END.
...
IF AVAIL plano_cta_ctbl THEN DO:
   ...
   FIND FIRST empresa NO-LOCK USE-INDEX empresa_id
        WHERE empresa.cod_empresa = tt-param.c-cod-empresa NO-ERROR.
   IF AVAIL empresa THEN DO:
      
      DO dt-for = tt-param.dt-periodo-ini TO tt-param.dt-periodo-fin:
         ...
         FOR EACH movto-estoq NO-LOCK USE-INDEX data-conta 
            WHERE movto-estoq.dt-trans  = dt-for 
              AND movto-estoq.ct-codigo = tt-param.ct-trans-ord-prod-mat /* Conta Transit¢ria das Ordens de Produ‡Æo de Materiais*/ 
              :
            
             
            IF LOOKUP(movto-estoq.cod-estabel,tt-param.c-lista-cod-estab) = 0 THEN NEXT.
            ...
            IF {ininc/i03in218.i 04 movto-estoq.esp-docto} <> 'DIV' AND 
               ...
               {ininc/i03in218.i 04 movto-estoq.esp-docto} <> 'SOB' AND
            ...
            IF {ininc/i03in218.i 04 movto-estoq.esp-docto} = 'DIV' THEN  /* S‚ries de Documento para GGF */
               IF INDEX(tt-param.c-lista-serie-ggf,movto-estoq.serie-docto) = 0 THEN NEXT.

            IF {ininc/i03in218.i 04 movto-estoq.esp-docto} <> 'REQ' AND 
               movto-estoq.nat-operacao                    <> '' THEN NEXT.

            IF INDEX(tt-param.c-lista-itens,item.it-codigo) <> 0 THEN NEXT.
            ...
            IF NOT AVAIL ord-prod THEN DO:
               ...
               RUN MESSAGE.p ('NÆo achou ordem de produ‡Æo',
                ...
                RETIRADA consistˆncia para poder rodar em rpw  */            
               NEXT.
            END.

            if ord-prod.tipo = 6 then next. /* Nao considera ordem de manuten‡Æo - Chamado 28387 */
            if ord-prod.nr-ord-produ >= 700000000 then next. /* Desconsidera Ordem de Ferramentaria */

            IF ord-prod.nr-linha = 40 /* nao considera linha 40 - transforma‡Æo de produtos CD */ THEN NEXT.

            IF AVAIL ord-prod THEN FIND FIRST lin-prod OF ord-prod NO-LOCK NO-ERROR.
            ...
            IF cc-codigo-db = '' THEN DO:
               ...
               IF NOT AVAIL dc-lin-prod THEN DO:
                  ...
                  PUT ord-prod.nr-linha AT 01
                      'Centro de Custo da linha igual a branco ou zero!'.
                  IF ord-prod.nr-linha <> 0 THEN DO:
                     CREATE dc-lin-prod.
            ...
            IF ITEM.ge-codigo = tt-param.i-gr-estoq-db-dir THEN DO: /* item de d‚bito direto */
               ...
               IF NOT AVAIL b-item THEN
                  NEXT.
               IF LOOKUP(string(b-item.ge-codigo),tt-param.c-gr-estoq-mat-prima)  <> 0 
                  THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-prima.
                  ELSE IF LOOKUP(STRING(b-item.ge-codigo),tt-param.c-gr-estoq-mat-secund) <> 0 
                          THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-secund.
                          ELSE IF LOOKUP(STRING(b-item.ge-codigo),tt-param.c-gr-estoq-mat-embal)  <> 0 
                                  THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-embal.
                                  ELSE DO:
                                      IF b-item.ge-codigo = 6 THEN NEXT. /* Solicita‡Æo da Marilza em 06/04/2004 */
                                      RUN MESSAGE.p ('NÆo achou conta cont bil 01',
                                                     'Grupo Estoque: ' + string(b-item.ge-codigo) + ' - OP: ' + STRING(movto-estoq.nr-ord-produ) + ' ser  usada conta ' + tt-param.ct-mat-prima).
                                      ASSIGN c-ct-codigo-db = tt-param.ct-mat-prima.
                                  END.
            END.
            ELSE DO: /* itens de estoque que nao sao debito direto */
               IF LOOKUP(STRING(ITEM.ge-codigo),tt-param.c-gr-estoq-mat-prima) <> 0 
                  THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-prima.
                  ELSE IF LOOKUP(STRING(ITEM.ge-codigo),tt-param.c-gr-estoq-mat-secund) <> 0
                          THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-secund.
                          ELSE IF LOOKUP(STRING(item.ge-codigo),tt-param.c-gr-estoq-mat-embal)  <> 0
                                  THEN ASSIGN c-ct-codigo-db = tt-param.ct-mat-embal.
                                  ELSE DO:
                                      IF item.ge-codigo = 6 THEN NEXT. /* Solicita‡Æo da Marilza em 06/04/2004 */
                                      RUN MESSAGE.p ('NÆo achou conta cont bil 02',
               ...
               FIND FIRST tt-detalhe NO-LOCK
                    WHERE tt-detalhe.cod-empresa   = emsuni.empresa.cod_empresa
                      ...
                      AND tt-detalhe.it-codigo     = IF AVAIL b-item 
                                                        ...
                                                        ELSE ITEM.it-codigo /* movto-estoq.it-codigo */ NO-ERROR.
         ...
         FOR EACH  movto-estoq NO-LOCK USE-INDEX data-conta 
            ...
            /* NÆo considera para MEKAL */
            IF movto-estoq.cod-estabel = "101"  THEN NEXT.
            ...
            ASSIGN c-ct-codigo-db = tt-param.ct-db-requis-ordem.

            FIND FIRST ord-prod NO-LOCK 
                 WHERE ord-prod.nr-ord-produ = movto-estoq.nr-ord-produ NO-ERROR.

            if avail ord-prod Then do:
                if ord-prod.tipo = 6 then next. /* Desconsidera Ordem de Manuten‡Æo */
                if ord-prod.nr-ord-produ >= 700000000 then next. /* Desconsidera Ordem de Ferramentaria */
            end.
            ...
            IF tt-param.rt-tipo-relat = 2 THEN DO:
               ...
               IF NOT AVAIL tt-detalhe THEN DO:
                  ...
                  ASSIGN tt-detalhe.cod-empresa   = emsuni.empresa.cod_empresa    
                         ...
                         tt-detalhe.esp-docto     = movto-estoq.esp-docto         
                         tt-detalhe.serie-docto   = movto-estoq.serie-docto       
                         tt-detalhe.it-codigo     = movto-estoq.it-codigo
                         tt-detalhe.ge-codigo     = ITEM.GE-CODIGO
                                                    
                         tt-detalhe.compr-fabric  = ITEM.compr-fabri.
               END.
               ASSIGN tt-detalhe.quantidade       = tt-detalhe.quantidade + IF movto-estoq.tipo-trans = 1 
                                                                               THEN movto-estoq.quantidade 
                                                                               ELSE movto-estoq.quantidade * - 1
...
IF tt-param.rt-tipo-relat = 1 THEN DO: /* Resumido */
   FOR EACH tt-resumido
...
IF tt-param.rt-tipo-relat = 2 THEN DO: /* Detalhado */
   FOR EACH tt-detalhe
   ...
   FOR EACH tt-resumido NO-LOCK
       ...
       IF tt-resumido.valor-total < 0 THEN DO:
          RUN pi-gera-contabilizacao(INPUT 'itens', 
                                     INPUT 'CR',
                                     INPUT tt-param.ct-contra-partida,
                                     INPUT tt-param.ccusto-contra-partida,
                                     INPUT (tt-resumido.valor-total * -1),
                                     INPUT STRING(' Conta ' + tt-resumido.ct-codigo + '.' + tt-resumido.cc-codigo),
                                     INPUT tt-resumido.cod-estab).
          ASSIGN i-sequencia = i-sequencia + 10.
          RUN pi-gera-contabilizacao(INPUT 'itens', 
                                     INPUT 'DB',
    ...
    CASE p-table:
        WHEN 'lote' THEN DO:
            ...
            ASSIGN tt_integr_lote_ctbl.tta_cod_modul_dtsul                     = 'FGL' 
                   ...
                   tt_integr_lote_ctbl.tta_des_lote_ctbl                       = 'Custos Materiais ' + STRING(month(tt-param.dt-periodo-fin),'99') + '/' + STRING(year(tt-param.dt-periodo-fin))
        ...
        WHEN 'itens' THEN DO:        
            ...
            ASSIGN tt_integr_item_lancto_ctbl.ttv_rec_integr_lancto_ctbl       = RECID(tt_integr_lancto_ctbl)
                   ...
                   tt_integr_item_lancto_ctbl.tta_cod_estab                    = IF p-estab = "" THEN entry(1, tt-param.c-lista-cod-estab) /*para-ped.estab-padrao*/ ELSE p-estab
                   tt_integr_item_lancto_ctbl.tta_cod_unid_negoc               = emsuni.empresa.cod_empresa /*'DOC'*/
                   tt_integr_item_lancto_ctbl.tta_des_histor_lancto_ctbl       = 'Custos Materiais ' + STRING(month(tt-param.dt-periodo-fin),'99') + '/' + STRING(year(tt-param.dt-periodo-fin)) + p-hist
