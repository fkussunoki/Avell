...
*******************************************************************************/
{include/i-prgvrs.i DOC025 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i DOC025 DOC}
&ENDIF
...
DEFINE VARIABLE h_b-1doc025 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-2doc025 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadpai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q-1doc025 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-1doc025 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 90 BY 15.17 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dovwr/v-1doc025.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-1doc025 ).
       RUN set-position IN h_v-1doc025 ( 2.54 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 2.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Contas|Usu rios' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'doqry/q-1doc025.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dozoom/doc025z.w,
                     ProgVaPara = dovp/doc025vp.w,
                     ProgIncMod = dop/doc025a.w,
                     Implantar = no     Keys-Accepted =':U ,
             OUTPUT h_q-1doc025 ).
       RUN set-position IN h_q-1doc025 ( 1.00 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.50 , 7.00 ) */

       /* Links to SmartViewer h_v-1doc025. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_v-1doc025 ).
       RUN add-link IN adm-broker-hdl ( h_q-1doc025 , 'Record':U , h_v-1doc025 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q-1doc025. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_q-1doc025 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q-1doc025 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q-1doc025 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q-1doc025 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-1doc025 ,
             h_p-exihel , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Layout = ,
                     Create-On-Add = ?,
                     ProgAtributo = dop/doc025b.w,
                     ProgIncMod = ':U ,
             OUTPUT h_b-1doc025 ).
       RUN set-position IN h_b-1doc025 ( 7.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 85.00 ) */

       /* Links to BrowserFormaFilho h_b-1doc025. */
       RUN add-link IN adm-broker-hdl ( h_q-1doc025 , 'Record':U , h_b-1doc025 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-1doc025 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dobrw/b-2doc025.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     Layout = ,
                     Create-On-Add = ?,
                     ProgAtributo = dop/doc025c.w,
                     ProgIncMod = ':U ,
             OUTPUT h_b-2doc025 ).
       RUN set-position IN h_b-2doc025 ( 7.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 85.00 ) */

       /* Links to BrowserFormaFilho h_b-2doc025. */
       RUN add-link IN adm-broker-hdl ( h_q-1doc025 , 'Record':U , h_b-2doc025 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-2doc025 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "DOC025" "1.00.00.000"}
