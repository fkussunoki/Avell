...
*******************************************************************************/
{include/i-prgvrs.i DOC025B 1.00.00.000}
...
DEF TEMP-TABLE tt-origem NO-UNDO
    FIELD cod-grupo       LIKE dc-orc-grupo-conta.cod-grupo
    FIELD cod_cta_ctbl    LIKE dc-orc-grupo-conta.cod_cta_ctbl
    FIELD cod_plano_ctbl  LIKE dc-orc-grupo-conta.cod_plano_ctbl
...
&Scoped-define FIELDS-IN-QUERY-br-source-browse tt-origem.cod_cta_ctbl tt-origem.des_tit_ctbl   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-source-browse   
&Scoped-define SELF-NAME br-source-browse
&Scoped-define QUERY-STRING-br-source-browse FOR EACH tt-origem        WHERE tt-origem.cod_cta_ctbl MATCHES "*" + c-cod-cta-orig + "*"           OR tt-origem.des_tit_ctbl MATCHES "*" + c-cod-cta-orig + "*"
&Scoped-define OPEN-QUERY-br-source-browse OPEN QUERY {&SELF-NAME}     FOR EACH tt-origem        WHERE tt-origem.cod_cta_ctbl MATCHES "*" + c-cod-cta-orig + "*"           OR tt-origem.des_tit_ctbl MATCHES "*" + c-cod-cta-orig + "*".
...
&Scoped-define FIELDS-IN-QUERY-br-target-browse tt-destino.cod_cta_ctbl tt-destino.cod_plano_ctbl tt-destino.des_tit_ctbl   
...
&Scoped-Define ENABLED-OBJECTS RECT-1 rt-key-parent c-plano-conta ~
c-cod-cta-orig bt-ver-origem br-source-browse br-target-browse bt-add ~
...
&Scoped-Define DISPLAYED-OBJECTS c-plano-conta c-cod-cta-orig 
...
DEFINE VARIABLE c-cod-cta-orig AS CHARACTER FORMAT "x(12)" 
     LABEL "Conta" 
     VIEW-AS FILL-IN 
     SIZE 17 BY .88 NO-UNDO.

DEFINE VARIABLE c-plano-conta AS CHARACTER FORMAT "x(8)" INITIAL "PCDOCOL" 
     LABEL "Plano Conta" 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-source-browse w-window _FREEFORM
  QUERY br-source-browse NO-LOCK DISPLAY
      tt-origem.cod_cta_ctbl  
      tt-origem.des_tit_ctbl
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 54.43 BY 13
         FONT 4.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-target-browse w-window _FREEFORM
  QUERY br-target-browse DISPLAY
      tt-destino.cod_cta_ctbl  
      tt-destino.cod_plano_ctbl
      tt-destino.des_tit_ctbl
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME f-formation
     ...
     dc-orc-grupo.descricao AT ROW 1.38 COL 35 COLON-ALIGNED NO-LABEL WIDGET-ID 4
          VIEW-AS FILL-IN 
          SIZE 62 BY .88
     c-plano-conta AT ROW 3 COL 10 COLON-ALIGNED WIDGET-ID 8
     c-cod-cta-orig AT ROW 3 COL 32 COLON-ALIGNED
     bt-ver-origem AT ROW 3 COL 51
     br-source-browse AT ROW 4.25 COL 2
     br-target-browse AT ROW 4.25 COL 62
     bt-add AT ROW 7.75 COL 56.57
     bt-del AT ROW 11.5 COL 56.72
...
OPEN QUERY {&SELF-NAME}
    FOR EACH tt-origem
       WHERE tt-origem.cod_cta_ctbl MATCHES "*" + c-cod-cta-orig + "*"
          OR tt-origem.des_tit_ctbl MATCHES "*" + c-cod-cta-orig + "*".
     _END_FREEFORM
...
/*              tt-item.it-codigo = sim-item.it-codigo NO-ERROR.                 */
/*         IF NOT AVAIL tt-item THEN DO:                                         */
/*             CREATE tt-item.                                                   */
/*             ASSIGN tt-item.it-codigo = sim-item.it-codigo                     */
...
DO:
    ASSIGN INPUT FRAME {&FRAME-NAME} c-plano-conta
                                     c-cod-cta-orig.
...
&Scoped-define SELF-NAME c-cod-cta-orig
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-cod-cta-orig w-window
ON ANY-PRINTABLE OF c-cod-cta-orig IN FRAME f-formation /* Conta */
DO:
    ASSIGN INPUT FRAME {&FRAME-NAME} c-cod-cta-orig.
    {&OPEN-QUERY-br-source-browse}
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c-plano-conta c-cod-cta-orig 
  ...
  IF AVAILABLE dc-orc-grupo THEN 
    DISPLAY dc-orc-grupo.cod-grupo dc-orc-grupo.descricao 
      WITH FRAME f-formation IN WINDOW w-window.
  ENABLE RECT-1 rt-key-parent c-plano-conta c-cod-cta-orig bt-ver-origem 
...
/*       RUN pi-inicializar IN h-acomp ("Carregando"). */
      
      FOR EACH cta_ctbl NO-LOCK
          WHERE cta_ctbl.cod_plano_cta_ctbl = c-plano-conta
          AND   cta_ctbl.dat_inic_valid <= TODAY
          AND   cta_ctbl.dat_fim_valid  >= TODAY:

/*           RUN pi-acompanhar IN h-acomp ("Conta: " + cta_ctbl.cod_cta_ctbl).  */

          IF NOT CAN-FIND(dc-orc-grupo-conta WHERE
                          dc-orc-grupo-conta.cod-grupo      = dc-orc-grupo.cod-grupo AND    
                          dc-orc-grupo-conta.cod_cta_ctbl   = cta_ctbl.cod_cta_ctbl  AND    
                          dc-orc-grupo-conta.cod_plano_ctbl = cta_ctbl.cod_plano_cta_ctbl) THEN DO:
              CREATE tt-origem.
              ASSIGN tt-origem.cod-grupo      = dc-orc-grupo.cod-grupo
                     tt-origem.cod_cta_ctbl   = cta_ctbl.cod_cta_ctbl
                     tt-origem.cod_plano_ctbl = cta_ctbl.cod_plano_cta_ctbl
                     tt-origem.des_tit_ctbl   = cta_ctbl.des_tit_ctbl.  
          END.
      END.
...
/*       RUN pi-finalizar IN h-acomp.  */
      ...
      FOR EACH dc-orc-grupo-conta NO-LOCK
         WHERE dc-orc-grupo-conta.cod-grupo = dc-orc-grupo.cod-grupo:

          FIND FIRST cta_ctbl NO-LOCK
               WHERE cta_ctbl.cod_cta_ctbl       = dc-orc-grupo-conta.cod_cta_ctbl  
                 AND cta_ctbl.cod_plano_cta_ctbl = dc-orc-grupo-conta.cod_plano_ctbl NO-ERROR.

          CREATE tt-destino.
          ASSIGN tt-destino.cod-grupo      = dc-orc-grupo-conta.cod-grupo
                 tt-destino.cod_cta_ctbl   = dc-orc-grupo-conta.cod_cta_ctbl
                 tt-destino.cod_plano_ctbl = dc-orc-grupo-conta.cod_plano_ctbl
                 tt-destino.des_tit_ctbl   = cta_ctbl.des_tit_ctbl WHEN AVAIL cta_ctbl.                
      END.
  END.
...
------------------------------------------------------------------------------*/
   ...
   */
   FIND tt-origem WHERE ROWID(tt-origem) = v-row-select-in-source NO-ERROR.
   
   IF AVAILABLE tt-origem THEN DO:
       
       CREATE tt-destino.
       ASSIGN tt-destino.cod-grupo      = INPUT FRAME {&FRAME-NAME} dc-orc-grupo.cod-grupo     
              tt-destino.cod_cta_ctbl   = tt-origem.cod_cta_ctbl  
              tt-destino.cod_plano_ctbl = tt-origem.cod_plano_ctbl
              tt-destino.des_tit_ctbl   = tt-origem.des_tit_ctbl.
       DELETE tt-origem.
...
DO ON ERROR UNDO, RETRY:
    ...
    FOR EACH dc-orc-grupo-conta EXCLUSIVE-LOCK
       WHERE dc-orc-grupo-conta.cod-grupo = INPUT FRAME {&FRAME-NAME} dc-orc-grupo.cod-grupo:

        FIND FIRST tt-destino WHERE tt-destino.cod-grupo      = dc-orc-grupo-conta.cod-grupo
                                AND tt-destino.cod_plano_ctbl = dc-orc-grupo-conta.cod_plano_ctbl
                                AND tt-destino.cod_cta_ctbl   = dc-orc-grupo-conta.cod_cta_ctbl NO-ERROR.
        IF NOT AVAIL tt-destino THEN
            DELETE dc-orc-grupo-conta.

    END.

    FOR EACH tt-destino:
        FIND FIRST dc-orc-grupo-conta EXCLUSIVE-LOCK
             WHERE dc-orc-grupo-conta.cod-grupo      = tt-destino.cod-grupo     
               AND dc-orc-grupo-conta.cod_cta_ctbl   = tt-destino.cod_cta_ctbl  
               AND dc-orc-grupo-conta.cod_plano_ctbl = tt-destino.cod_plano_ctbl NO-ERROR.
        IF NOT AVAIL dc-orc-grupo-conta THEN DO:
            CREATE dc-orc-grupo-conta.
            ASSIGN dc-orc-grupo-conta.cod-grupo      = tt-destino.cod-grupo     
                   dc-orc-grupo-conta.cod_cta_ctbl   = tt-destino.cod_cta_ctbl  
                   dc-orc-grupo-conta.cod_plano_ctbl = tt-destino.cod_plano_ctbl.
        END.
    END.
END.    
...
------------------------------------------------------------------------------*/
  ...
  {include/i-frm180.i}
   ...
   FIND tt-destino WHERE ROWID( tt-destino ) = v-row-target-browse NO-ERROR.
   if AVAILABLE tt-destino THEN DO:
       CREATE tt-origem.
       ASSIGN tt-origem.cod-grupo      = tt-destino.cod-grupo     
              tt-origem.cod_cta_ctbl   = tt-destino.cod_cta_ctbl  
              tt-origem.cod_plano_ctbl = tt-destino.cod_plano_ctbl
              tt-origem.des_tit_ctbl   = tt-destino.des_tit_ctbl.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH cta_ctbl NO-LOCK
        ...
        FIND FIRST tt-destino WHERE 
                   ...
                   tt-destino.cod_cta_ctbl   = cta_ctbl.cod_cta_ctbl                            AND
                   tt-destino.cod_plano_ctbl = cta_ctbl.cod_plano_cta_ctbl NO-ERROR.
