...
DEF INPUT PARAM TABLE FOR tt-ccusto-cta.
...
DEF VAR de-variac               AS DEC  NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Imprime - DOC024B */
DO:
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST cta_ctbl NO-LOCK
         WHERE cta_ctbl.cod_plano_cta_ctbl = "PCDOCOL"
           AND cta_ctbl.cod_cta_ctbl       = p-cod-cta NO-ERROR.
    IF AVAIL cta_ctbl THEN
       PUT UNFORMATTED "Conta.......: " AT 01
           cta_ctbl.cod_cta_ctbl " - " cta_ctbl.des_tit_ctbl  FORMAT "x(50)" SKIP(1).

    DO WITH FRAME f-ccusto-cta:
       FOR EACH tt-ccusto-cta NO-LOCK
          WHERE tt-ccusto-cta.cod_cta_ctbl = p-cod-cta
             BY tt-ccusto-cta.val-movto DESC:
    
          ASSIGN de-variac = (tt-ccusto-cta.val-movto / tt-ccusto-cta.val-orcado * 100).
    
          DISP tt-ccusto-cta.cod_ccusto  
               tt-ccusto-cta.des_tit_ctbl FORMAT "x(50)"           COLUMN-LABEL "T°tulo"   
               tt-ccusto-cta.val-orcado   FORMAT "->>>,>>>,>>9.99"        LABEL "Oráado"   
               tt-ccusto-cta.val-previsto FORMAT "->>>,>>>,>>9.99"        LABEL "Previsto"
               tt-ccusto-cta.val-movto    FORMAT "->>>,>>>,>>9.99"        LABEL "Realizado"
               de-variac                                                  LABEL "% Var"
               WITH STREAM-IO WIDTH 132 NO-BOX DOWN FRAME f-ccusto-cta.
          DOWN WITH FRAME f-ccusto-cta.
    
          ASSIGN de-tot-orcado = de-tot-orcado + tt-ccusto-cta.val-orcado
                 de-tot-prev   = de-tot-prev + tt-ccusto-cta.val-previsto
                 de-tot-realiz = de-tot-realiz + tt-ccusto-cta.val-movto.
    
       END.
    
       UNDERLINE tt-ccusto-cta.val-orcado
                 tt-ccusto-cta.val-previsto
                 tt-ccusto-cta.val-movto
                 de-variac WITH FRAME f-ccusto-cta.
       DISP "TOTAL"                              @ tt-ccusto-cta.cod_ccusto
            de-tot-orcado                        @ tt-ccusto-cta.val-orcado
            de-tot-prev                          @ tt-ccusto-cta.val-prev
            de-tot-realiz                        @ tt-ccusto-cta.val-movto
           (de-tot-realiz / de-tot-orcado) * 100 @ de-variac WITH FRAME f-ccusto-cta.
    END.
