...
{include/i-prgvrs.i DPD611 1.00.00.000}
...
DEFINE VARIABLE h_b01dpd611 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02dpd611 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Bonifica‡äes de Verba Cooperada"
         HEIGHT             = 23.08
         WIDTH              = 89.29
         MAX-HEIGHT         = 24.17
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manuten‡Æo Bonifica‡äes de Verba Cooperada */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manuten‡Æo Bonifica‡äes de Verba Cooperada */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.17 , 72.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v01dpd611 ).
       RUN set-position IN h_v01dpd611 ( 2.50 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Pedidos|T¡tulos' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 13.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 88.72 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'doqry/q01dpd611.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dozoom/dpd611z.w,
                     ProgVaPara = dovp/dpd611vp.w,
                     ProgIncMod = dop/dpd611a.w,
                     Implantar = no':U ,
             OUTPUT h_q01dpd611 ).
       RUN set-position IN h_q01dpd611 ( 1.00 , 59.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_v01dpd611 ).
       RUN add-link IN adm-broker-hdl ( h_q01dpd611 , 'Record':U , h_v01dpd611 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01dpd611. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_q01dpd611 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01dpd611 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01dpd611 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01dpd611 ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = dop/dpd611b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b01dpd611 ).
       RUN set-position IN h_b01dpd611 ( 14.96 , 2.43 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 86.00 ) */

       /* Links to BrowserCadastro2 h_b01dpd611. */
       RUN add-link IN adm-broker-hdl ( h_q01dpd611 , 'Record':U , h_b01dpd611 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01dpd611 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'dobrw/b02dpd611.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = dop/dpd611c.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b02dpd611 ).
       RUN set-position IN h_b02dpd611 ( 14.96 , 2.43 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 86.00 ) */

       /* Links to BrowserCadastro2 h_b02dpd611. */
       RUN add-link IN adm-broker-hdl ( h_q01dpd611 , 'Record':U , h_b02dpd611 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02dpd611 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
{utp/ut9000.i "DPD611" "1.00.00.000"}
