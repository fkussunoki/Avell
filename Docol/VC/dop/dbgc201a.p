...
**                           16/10/2019 - 12.00.04.000 - Comentada reescrita de tt_realiz.ind_tip_movto
**                                                       estava distorcendo os dados da tela 
**                           16/10/2019 - 12.00.04.000 - Acertada soma da v_movto_total. Nao estava 
**                                                       mudando na alteracao do Radio-set     
**                           16/10/2019 - 12.00.04.00  - Alteracao de data de movimento dos empenhos                     
**                                                       (Flavio Kussunoki - FKIS Consultoria)
**************************************************************************************************/
...
DEF VAR v_val_tot_real           as dec format "->>>,>>>,>>9.99" no-undo.
DEF VAR v_val_tot_orcto          as dec format "->>>,>>>,>>9.99" no-undo.
...
on value-changed of rs_tip_movto in frame f_det do:
   ...
   if v_cod_ccusto <> "" then
    ...
    for each tt_realiz no-lock:
        case tt_realiz.ind_tip_movto:
        
        when "Realizado" then
        assign v_val_tot_real = v_val_tot_real + tt_realiz.val_movto.
        when "Empenhado" then
        assign v_val_tot_orcto = v_val_tot_orcto + tt_realiz.val_movto.
       ...
       when 1 then do:
           OPEN QUERY qr_realiz
              FOR EACH tt_realiz
                  ...
                  ASSIGN v_val_tot_movto = v_val_tot_orcto.
       end.
       when 2 then do:
           OPEN QUERY qr_realiz
              FOR EACH tt_realiz
                  ...
                  ASSIGN v_val_tot_movto = v_val_tot_real.
       end.
       when 3 then do:
           OPEN QUERY qr_realiz
              FOR EACH tt_realiz
                  ...
                  ASSIGN v_val_tot_movto = v_val_tot_orcto + v_val_tot_real.
       end.                      
   end.
        ...
        FOR EACH orig_movto_empenh NO-LOCK
            ...
            IF NOT AVAIL tt_realiz THEN DO:
                ...
                ASSIGN tt_realiz.rec_id                   = RECID (orig_movto_empenh)
                       ...
                       tt_realiz.dat_movto                = date(entry(2, orig_movto_empenh.cod_livre_1, chr(10))) //orig_movto_empenh.dat_sdo_ctbl
            ...
            IF AVAIL tt_orig_movto_empenh_aux THEN DO:
                ...
                case tt_orig_movto_empenh_aux.tta_num_orig_movto_empenh:
                    ...
                    when 12 /* C. Compra por Medi‡Æo */ then do: /* CNP */
                         ASSIGN tt_realiz.cod_modul_dtsul = "CNP".
                        ASSIGN tt_realiz.ind_tip_movto   = "Empenhado".
                    ...
                    when 35 /* Req./Solic. Material MI*/ then do:
                        ASSIGN tt_realiz.cod_modul_dtsul = "MIP".
                        ASSIGN tt_realiz.ind_tip_movto   = "Realizado".
                    end.
...
PROCEDURE pi_lista_origem:
    ...
    DEF VAR i-length AS INTEGER NO-UNDO. //criar intervalo de (90 - 37) entradas para atender documentos customizados.
    ...
    //documentos customizados:
        ASSIGN i-length = NUM-ENTRIES(v_des_lista_orig, ",").
        DO i =  i-length TO 89:
            ASSIGN v_des_lista_orig = v_des_lista_orig + ",".
                
        END.
        
        ASSIGN v_des_lista_orig = v_des_lista_orig + "Verba Cooperada" + "," + 
                                                     "Verba Cooperada".
