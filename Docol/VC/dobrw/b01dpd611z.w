...
{include/i-prgvrs.i B01DPD611Z 1.00.00.000}
...
&Scoped-define FIELDS-IN-QUERY-br-table amkt-solic-vl-bonific.sequencia ~
fn-tipo-documento(amkt-solic-vl-bonific.tipo-documento) @ c-tipo-documento ~
amkt-solic-vl-bonific.documento amkt-solic-vl-bonific.data-hora-lib ~
amkt-solic-vl-bonific.vl-liberado amkt-solic-vl-bonific.vl-realizado ~
amkt-solic-vl-bonific.cod-usuario amkt-solic-vl-bonific.data-hora-inclusao 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table 
&Scoped-define QUERY-STRING-br-table FOR EACH amkt-solic-vl-bonific WHERE ~{&KEY-PHRASE} ~
      AND amkt-solic-vl-bonific.sequencia >= fi-sequencia-ini and ~
emsdocol.amkt-solic-vl-bonific.sequencia <= fi-sequencia-fim NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS IMAGE-1 IMAGE-2 fi-sequencia-ini ~
fi-sequencia-fim bt-confirma br-table 
&Scoped-Define DISPLAYED-OBJECTS fi-sequencia-ini fi-sequencia-fim 
...
DEFINE VARIABLE fi-sequencia-fim AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 999999999 
     VIEW-AS FILL-IN 
     SIZE 10.14 BY .88 NO-UNDO.

DEFINE VARIABLE fi-sequencia-ini AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 0 
     LABEL "Sequˆncia" 
     VIEW-AS FILL-IN 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 86 BY 10.83
         FONT 1.
...
DEFINE FRAME F-Main
     fi-sequencia-ini AT ROW 1 COL 23 COLON-ALIGNED
     fi-sequencia-fim AT ROW 1 COL 50 COLON-ALIGNED NO-LABEL
...
/* Query rebuild information for BROWSE br-table
     ...
     _Where[1]         = "emsdocol.amkt-solic-vl-bonific.sequencia >= fi-sequencia-ini and
emsdocol.amkt-solic-vl-bonific.sequencia <= fi-sequencia-fim"
     _FldNameList[1]   = emsdocol.amkt-solic-vl-bonific.sequencia
...
"fn-tipo-documento(amkt-solic-vl-bonific.tipo-documento) @ c-tipo-documento" "Tipo Documento" "x(20)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = emsdocol.amkt-solic-vl-bonific.documento
...
DO:
    assign input frame {&frame-name} fi-sequencia-ini fi-sequencia-fim.
