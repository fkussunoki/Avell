...
&Scoped-define QUERY-STRING-br-table FOR EACH amkt-aprov-pend NO-LOCK WHERE                                  amkt-aprov-pend.cd-solicitacao = amkt-solicitacao.numero AND                                  amkt-aprov-pend.tipo           = 2, ~
       /* Comprova‡Æo */                             LAST amkt-solic-comprov NO-LOCK WHERE                                  amkt-solic-comprov.numero      = amkt-aprov-pend.cd-solicitacao AND                                  amkt-solic-comprov.data-envio <= amkt-aprov-pend.dt-pendencia                                  BY amkt-aprov-pend.dt-situacao
...
DO:
    ...
    /*IF SEARCH(amkt-solic-comprov.url-arquivo) = ? THEN DO:
        RUN dop/MESSAGE.p("Arquivo nÆo encontrado.", "Favor entrar em contato com a TIC.").
        RETURN NO-APPLY.
    END.*/
