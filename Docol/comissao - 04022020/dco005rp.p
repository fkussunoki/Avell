...
DEF VAR c-tp-comiss                      AS CHAR                                 NO-UNDO.
DEF VAR de-perc-pis                      AS DECIMAL                              NO-UNDO.
DEF VAR de-perc-cofins                   AS DECIMAL                              NO-UNDO.
DEF VAR de-vlr-pis-cofins                AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
DEF VAR de-vlr-liquido                   AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
DEF VAR de-cli-pis-cofins                AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
DEF VAR de-cli-vlr-liquido               AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
DEF VAR de-rep-pis-cofins                AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
DEF VAR de-rep-vlr-liquido               AS DECIMAL FORMAT "->>>,>>>,>>9.99"     NO-UNDO.
...
/* ======================================================= */
    ...
    DEF VAR c-pis               AS DEC      NO-UNDO.
    DEF VAR c-cofins            AS DEC      NO-UNDO.
    ...
    {include/i-openpar504.i CHARACTER c-lista-cdn-repres 
                                      ...
                                      c-fim-referencia 
                                      rs-data}
    {include/i-openpar504.i DECIMAL   c-pis
                                      c-cofins}
...
FORM 
    ...
    de-vlr-pis-cofins               COLUMN-LABEL 'PIS/COFINS'
    de-vlr-liquido                  COLUMN-LABEL 'Vlr.Liquido'
...
IF  tg-imp-comissoes THEN DO:
    IF c-destino = "Planilha" THEN DO:
        {doinc/dco005rp.i6 tt-movto.cdn_cliente}
...
IF  tg-imp-resumo THEN DO:
    ...
    FOR EACH tt-repres NO-LOCK
        ...
        FIND FIRST representante NO-LOCK WHERE representante.cod_empresa = tt-repres.cod_empresa
                                         AND   representante.cdn_repres  = tt-repres.cdn_repres NO-ERROR.
...
/*                        IF AVAIL classif_impto THEN                                                                                       */
                       ...
                       ASSIGN de-perc-cofins = ( c-cofins).
/*                                                   */
/*                        ELSE                       */
/*                                                   */
...
/*            tt-repres.de-tot-vincu[2]    = 0 THEN NEXT. */
        ...
        DISPLAY tt-repres.cdn_repres 
               ...
               (tt-repres.de-tot-pg-baixa[1] + tt-repres-202.de-tot-pg-baixa + tt-repres.de-tot-pg-lp-lm[1])  COLUMN-LABEL "Pagto Baixa"
                ...
                (tt-repres.de-tot-vinc-mes[1] * ((de-perc-pis + de-perc-cofins) / 100))                       COLUMN-LABEL "PIS/COFINS"
        ...
        PUT     c-periodo-prox                                                                      AT 31                                        
               ...
               (tt-repres.de-tot-pg-baixa[2] + tt-repres.de-tot-pg-lp-lm[2])   FORMAT '>>>>>,>>9.99' SPACE
                ...
                tt-repres.de-tot-vincu[2]                                      FORMAT '->>>>,>>9.99' SPACE
                (tt-repres.de-tot-vinc-mes[2] * ((de-perc-pis + de-perc-cofins) / 100)) .
        ...
        IF LAST(tt-repres.cdn_repres) THEN DO:
            ...
            PUT c-periodo-atu                                                                                   AT 31 
                ...
                (ACCUM TOTAL tt-repres.de-tot-vinc-mes[1] + tt-repres-202.de-tot-vinc-mes) *  ((de-perc-pis + de-perc-cofins) / 100) FORMAT '>>>>,>>9.99'.   
            ...
            PUT c-periodo-prox                                                                                  AT 31 
                ...
                (ACCUM TOTAL tt-repres.de-tot-vinc-mes[2]) *  ((de-perc-pis + de-perc-cofins) / 100) FORMAT '>>>>,>>9.99'.       
        ...
        IF tg-gera-movto-comis THEN DO: /* Gera Movimento de Comiss∆o */
           ...
           */
           ...
           /* PIS e COFINS na liquidacao (Tranche 2 da planilha) */
           ASSIGN i-transacao = 302.
           FIND FIRST representante NO-LOCK WHERE representante.cod_empresa = tt-repres.cod_empresa
                                            AND   representante.cdn_repres  = tt-repres.cdn_repres NO-ERROR.
             FIND FIRST funcionario NO-LOCK WHERE funcionario.cod_id_feder = representante.cod_id_feder NO-ERROR.
             IF NOT AVAIL funcionario THEN DO:
...
/*                             ASSIGN de-perc-cofins = 0.                                                                                        */
                       ...
                       ASSIGN de-lancamento = (tt-repres.de-tot-pg-baixa[1] + tt-repres.de-tot-pg-emiss[1] - tt-repres.de-tot-pg-estor[1] +
                                              tt-repres.de-tot-pg-baixa[2] + tt-repres.de-tot-pg-emiss[2] - tt-repres.de-tot-pg-estor[2]) * ((de-perc-pis + de-perc-cofins) / 100).
                       {doinc/dco005rp.i2}
                       ...
                       FOR EACH tt-movto NO-LOCK
                          ...
                          FIND FIRST movto_comis_repres OF tt-movto NO-LOCK
                               WHERE movto_comis_repres.ind_tip_movto         = "REALIZADO"
                                 AND movto_comis_repres.ind_natur_lancto_ctbl = "CR" NO-ERROR.
                          ASSIGN de-orig-lancamento = tt-movto.val_movto_comis  * ((de-perc-pis + de-perc-cofins) / 100).
                          {doinc/dco005rp.i4 de-orig-lancamento}
                          {doinc/dco005rp.i3}
                       END.
                    END. /* if avail dc-comis-trans */
                 END. /* Fim PIS e COFINS liquidacao Tranche 2 */
             END.
           ...
           /* PIS e COFINS sobre Comiss∆o Vinculada do Màs */
           IF tt-repres.de-tot-vinc-mes[1] <> 0 OR tt-repres.de-tot-vinc-mes[2] <> 0 THEN DO: 
              FIND  LAST dc-comis-trans NO-LOCK 
                   WHERE dc-comis-trans.cod_transacao = 390 NO-ERROR.
...
/*                             ASSIGN de-perc-cofins = 0.                                                                                        */
                       ...
                       ASSIGN  de-lancamento = (tt-repres.de-tot-vinc-mes[1] + tt-repres.de-tot-vinc-mes[2]) * ((de-perc-pis + de-perc-cofins) / 100).
                       {doinc/dco005rp.i2}
                       ...
                       FOR EACH tt-movto NO-LOCK
                          ...
                          ASSIGN de-orig-lancamento = (tt-movto.val_movto_vincul_mes) * ((de-perc-pis + de-perc-cofins) / 100).
                             
                          {doinc/dco005rp.i4 de-orig-lancamento}
                          {doinc/dco005rp.i3}
                       END.
                    END. /* if avail dc-comis-trans */
                 END. /* Fim PIS e COFINS Comiss∆o Vinculada do Màs */
              END.

           /* PIS COFINS Estorno da Comiss∆o Vinculada */
           IF tt-repres.de-tot-pg-estor[1] <> 0 OR tt-repres.de-tot-pg-estor[2] <> 0 THEN DO: 
              FIND  LAST dc-comis-trans NO-LOCK 
                   WHERE dc-comis-trans.cod_transacao = 391 NO-ERROR.
...
/*                              ASSIGN de-perc-cofins = 0.                                                                                        */
                        ...
                        ASSIGN de-lancamento = (tt-repres.de-tot-pg-estor[1] + tt-repres.de-tot-pg-estor[2]) * ((de-perc-pis + de-perc-cofins) / 100).
                        {doinc/dco005rp.i2}
                        ...
                        FOR EACH tt-movto NO-LOCK
                           ...
                           ASSIGN de-orig-lancamento = (tt-movto.val_movto_estorno) * ((de-perc-pis + de-perc-cofins) / 100).
                           {doinc/dco005rp.i4 de-orig-lancamento}
                           {doinc/dco005rp.i3}
                        END.
                     END. /* if avail dc-comis-trans */
                  END. /* PIS COFINS Fim estorno Comiss∆o Vinculada */
              END.
