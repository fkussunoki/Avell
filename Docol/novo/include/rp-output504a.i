...
***************************************************************************/
DEF VAR c-arq-impressora AS CHAR NO-UNDO.
...
DO:  /* seta a saida da impressao */
    CASE c-destino:
        ...
        WHEN "Impressora" /*l_printer*/ THEN DO:
            ...
            IF NUM-ENTRIES(c-arquivo,":") > 2 THEN DO: /* Imprime em Arquivo */
               ASSIGN c-arq-impressora = TRIM(SUBSTR(c-arquivo,LENGTH(STRING(c-impressora + ":" + c-layout + ":")) + 1,50)).

               FIND FIRST tip_imprsor NO-LOCK
                        WHERE tip_imprsor.cod_tip_imprsor = layout_impres.cod_tip_imprsor NO-ERROR.

               IF v_num_ped_exec_corren = 0 THEN DO: /* On-Line */
                  OUTPUT {&stream} TO VALUE(c-arq-impressora)
                         PAGED PAGE-SIZE VALUE(i-qtd-linhas) CONVERT TARGET tip_imprsor.cod_pag_carac_conver {&APPEND}.
               END.
               ELSE DO: /* RPW */
                   ...
                   OUTPUT {&stream} TO VALUE(TRIM(servid_exec.nom_dir_spool) + "/" + c-arq-impressora)
                          PAGED PAGE-SIZE VALUE(i-qtd-linhas) CONVERT TARGET tip_imprsor.cod_pag_carac_conver {&APPEND}.
               END.
            END. /* FIM-Imprime em Arquivo */
            ELSE DO:
                FIND FIRST tip_imprsor NO-LOCK
                     WHERE tip_imprsor.cod_tip_imprsor = layout_impres.cod_tip_imprsor NO-ERROR.
            ...
            /* Caracteres de Configura‡Æo da Impressora */
            ...
            /* FIM-Caracteres de Configura‡Æo da Impressora */
        ...
        WHEN "Arquivo" /*l_file*/  THEN DO:
            ...
            ELSE DO:
                ...
                FIND FIRST servid_exec NO-LOCK
                     WHERE servid_exec.cod_servid_exec = ped_exec.cod_servid_exec NO-ERROR.
                OUTPUT {&stream} TO VALUE(TRIM(servid_exec.nom_dir_spool) + "/" + c-arquivo)
                      PAGED PAGE-SIZE VALUE(i-qtd-linhas) CONVERT TARGET 'iso8859-1' {&APPEND}.
            END.
