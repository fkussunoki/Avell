...
FOR EACH {1}.movto-estoq USE-INDEX data-conta NO-LOCK
   WHERE {1}.movto-estoq.dt-trans  = da-aux:
    ...
    RUN pi-acompanhar IN h-acomp ("Data Movimento: " + STRING(da-aux,"99/99/9999")).


    FIND FIRST tt-estab      NO-LOCK WHERE
        tt-estab.cod-estabel = {1}.movto-estoq.cod-estabel NO-ERROR.      


    FIND FIRST tt-ccusto WHERE
               tt-ccusto.cod_empresa      = tt-estab.cod-emp-ems5  AND
               tt-ccusto.cod_plano_ccusto = tt-estab.cc-plano AND
