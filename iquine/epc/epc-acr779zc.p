...
IF p-ind-event = "cria_registro_duplic" THEN DO:
os-delete value(SESSION:TEMP-DIRECTORY + "FINAL5.txt").
    ...
    FOR EACH tt_epc WHERE  tt_epc.cod_event     = p-ind-event:

        CASE tt_epc.cod_parameter:
            WHEN "cod-emitente" THEN DO:
                CREATE tt-tit-acr-dev.
                ASSIGN tt-tit-acr-dev.cod-emitente   = int(tt_epc.val_parameter).
            END.
            WHEN "cod-esp-ems5" THEN
                ASSIGN tt-tit-acr-dev.cod-esp        = (tt_epc.val_parameter).
            WHEN "serie-ems5" THEN
                ASSIGN tt-tit-acr-dev.serie        = (tt_epc.val_parameter).

            WHEN "cod-estab-ems5" THEN
                ASSIGN tt-tit-acr-dev.cod-estab-ems5 = (tt_epc.val_parameter).
            WHEN "nr-docto-ems5" THEN
                ASSIGN tt-tit-acr-dev.nr-docto       = (tt_epc.val_parameter).
            WHEN "parcela" THEN
                ASSIGN tt-tit-acr-dev.parcela        = (tt_epc.val_parameter).
            WHEN "serie-nota" THEN
                ASSIGN tt-tit-acr-dev.serie-nota     = (tt_epc.val_parameter).
            WHEN "nr-docto-nota" THEN
                ASSIGN tt-tit-acr-dev.nota-fiscal    = (tt_epc.val_parameter).
            WHEN "nat-operacao" THEN
                ASSIGN tt-tit-acr-dev.nat-operacao   = (tt_epc.val_parameter).
            WHEN "cod-estab" THEN
                ASSIGN tt-tit-acr-dev.cod-estab-ems2 = (tt_epc.val_parameter).

        END CASE.
    END.


    FOR EACH tt-tit-acr-dev:

        FIND FIRST natur-oper NO-LOCK WHERE natur-oper.nat-operacao = tt-tit-acr-dev.nat-operacao NO-ERROR.

        FIND FIRST ext-tit-acr-dev WHERE ext-tit-acr-dev.cod-estab-ems5  = tt-tit-acr-dev.cod-estab-ems5  
                                   and   ext-tit-acr-dev.cod-esp         = tt-tit-acr-dev.cod-esp         
                                   and   ext-tit-acr-dev.nr-docto        = tt-tit-acr-dev.nr-docto        
                                   and   ext-tit-acr-dev.serie           = tt-tit-acr-dev.serie           
                                   and   ext-tit-acr-dev.parcela         = tt-tit-acr-dev.parcela         
                                   and   ext-tit-acr-dev.cod-emitente    = tt-tit-acr-dev.cod-emitente   NO-ERROR.

        IF NOT AVAIL ext-tit-acr-dev THEN DO:

            FIND FIRST docum-est NO-LOCK WHERE docum-est.serie-docto  = tt-tit-acr-dev.serie-nota
                                         AND   docum-est.nro-docto    = tt-tit-acr-dev.nota-fiscal
                                         AND   docum-est.cod-emitente = tt-tit-acr-dev.cod-emitente
                                         NO-ERROR.

            CREATE ext-tit-acr-dev.
            ASSIGN ext-tit-acr-dev.cod-estab-ems5          = tt-tit-acr-dev.cod-estab-ems5 
                   ext-tit-acr-dev.cod-esp                 = tt-tit-acr-dev.cod-esp        
                   ext-tit-acr-dev.serie                   = tt-tit-acr-dev.serie          
                   ext-tit-acr-dev.nr-docto                = tt-tit-acr-dev.nr-docto       
                   ext-tit-acr-dev.parcela                 = tt-tit-acr-dev.parcela        
                   ext-tit-acr-dev.cod-emitente            = tt-tit-acr-dev.cod-emitente   
                   ext-tit-acr-dev.cod-esp-cr              = natur-oper.cod-esp     
                   ext-tit-acr-dev.sdo-tit-acr             = tt-tit-acr-dev.sdo-tit-acr    
                   ext-tit-acr-dev.sdo-aplicar             = tt-tit-acr-dev.sdo-aplicar    
                   ext-tit-acr-dev.nota-fiscal             = tt-tit-acr-dev.nota-fiscal    
                   ext-tit-acr-dev.serie-nota              = tt-tit-acr-dev.serie-nota     
                   ext-tit-acr-dev.nat-operacao            = tt-tit-acr-dev.nat-operacao   
                   ext-tit-acr-dev.cod-estab-ems2          = tt-tit-acr-dev.cod-estab-ems2
                   ext-tit-acr-dev.dt-transacao            = docum-est.dt-trans
                   ext-tit-acr-dev.ctr-parcela             = '1'.
        END.
    ...
    FOR EACH ext-tit-acr-dev:
        ...
        FIND FIRST tit_acr NO-LOCK WHERE tit_acr.cod_estab       = ext-tit-acr-dev.cod-estab-ems5
                                   ...
                                   AND   tit_acr.cdn_cliente     = ext-tit-acr-dev.cod-emitente       NO-ERROR.
        



        IF avail tit_acr THEN DO:
            ASSIGN ext-tit-acr-dev.sdo-tit-acr = tit_acr.val_sdo_tit_acr
                   ext-tit-acr-dev.seq         = ext-tit-acr-dev.seq + 1.
        END.
    END.    
        FOR EACH ext-tit-acr-dev:

        FIND FIRST docum-est NO-LOCK WHERE docum-est.serie-docto  = ext-tit-acr-dev.serie-nota
                                     AND   docum-est.nro-docto    = ext-tit-acr-dev.nota-fiscal
                                     AND   docum-est.cod-emitente = ext-tit-acr-dev.cod-emitente
                                     AND   docum-est.nat-operacao = ext-tit-acr-dev.nat-operacao NO-ERROR.


        IF avail docum-est THEN DO:

            FIND FIRST ext-docum-est-dev WHERE    ext-docum-est-dev.nota-fiscal     =  docum-est.nro-docto  
                                  and      ext-docum-est-dev.serie-nota      =  docum-est.serie-docto  
                                  and      ext-docum-est-dev.nat-operacao    =  docum-est.nat-operacao  
                                  and      ext-docum-est-dev.cod-estab-ems2  =  docum-est.cod-estabel  
                                  and      ext-docum-est-dev.cod-emitente    =  docum-est.cod-emitente  NO-ERROR.

            IF NOT avail ext-docum-est-dev THEN DO:


                ASSIGN i-sequencia = i-sequencia + 1.
                CREATE ext-docum-est-dev.
                ASSIGN ext-docum-est-dev.nota-fiscal     =  docum-est.nro-docto    
                       ext-docum-est-dev.serie-nota      =  docum-est.serie-docto  
                       ext-docum-est-dev.nat-operacao    =  docum-est.nat-operacao 
                       ext-docum-est-dev.cod-estab-ems2  =  docum-est.cod-estabel  
                       ext-docum-est-dev.cod-emitente    =  docum-est.cod-emitente
                       ext-docum-est-dev.v-vlr-nota      =  docum-est.tot-valor.
...
/*     ASSIGN i-controle-interno = 0.                                                           */
/*                                                                                              */
/*     FOR EACH tit_acr NO-LOCK WHERE tit_acr.cod_estab       = ext-tit-acr-dev.cod-estab-ems5  */
/*                              AND   tit_acr.cod_espec_docto = ext-tit-acr-dev.cod-esp         */
/*                              AND   tit_acr.cod_ser_docto   = ext-tit-acr-dev.serie           */
/*                              AND   tit_acr.cod_tit_acr     = ext-tit-acr-dev.nr-docto        */
/*                              AND   tit_acr.cdn_cliente     = ext-tit-acr-dev.cod-emitente    */
/*                              AND   tit_acr.ind_orig_tit_acr = "FATEMS20":                    */
/*         ASSIGN i-controle-interno = i-controle-interno + 1.                                  */
/*                                                                                              */
/*     END.                                                                                     */
/*                                                                                              */
...
/*     END.                                                                                                        */
/*                                                                                                                 */
/*     IF i-controle-interno = 0 THEN DO:                                                                          */
/*                                                                                                                 */
/*                                                                                                                 */
/*                                                                                                                 */
/*         FOR EACH ext-tit-acr-dev:                                                                               */
/*             ASSIGN contra-prova = contra-prova  + (ext-tit-acr-dev.sdo-tit-acr - ext-tit-acr-dev.sdo-aplicar).  */
/*         END.                                                                                                    */
/*                                                                                                                 */
/*         FIND FIRST ext-docum-est-dev NO-ERROR.                                                                  */
/*             ASSIGN contra-prova1 = ext-docum-est-dev.v-vlr-nota.                                                */
/*                                                                                                                 */
/*         IF contra-prova = 0                                                                                     */
/*         AND contra-prova1 > 0 THEN DO:                                                                          */
/*                                                                                                                 */
/*                                                                                                                 */
/*                                                                                                                 */
/*         RUN pi-encerra.                                                                                         */
...
/* END. */
    ...
    titulo_acr:
    FOR EACH ext-tit-acr-dev WHERE (ext-tit-acr-dev.sdo-tit-acr - ext-tit-acr-dev.sdo-aplicar) > 0:


        FIND FIRST ext-docum-est-dev WHERE ext-docum-est-dev.nota-fiscal = ext-tit-acr-dev.nota-fiscal
                              AND   ext-docum-est-dev.serie-nota  = ext-tit-acr-dev.serie-nota
                              AND   ext-docum-est-dev.cod-emitente = ext-tit-acr-dev.cod-emitente NO-ERROR.


        IF   ext-docum-est-dev.v-vlr-nota > 0
        AND  ext-docum-est-dev.v-vlr-nota <= (ext-tit-acr-dev.sdo-tit-acr - ext-tit-acr-dev.sdo-aplicar) THEN DO:

        ASSIGN ext-tit-acr-dev.sdo-aplicar = ext-docum-est-dev.v-vlr-nota
               ext-docum-est-dev.v-vlr-nota = 0.
        NEXT titulo_acr.
        ...
        IF  ext-docum-est-dev.v-vlr-nota > 0
        AND ext-docum-est-dev.v-vlr-nota > (ext-tit-acr-dev.sdo-tit-acr - ext-tit-acr-dev.sdo-aplicar) THEN DO:
            ASSIGN ext-tit-acr-dev.sdo-aplicar = (ext-tit-acr-dev.sdo-tit-acr)
                   ext-docum-est-dev.v-vlr-nota    = (ext-docum-est-dev.v-vlr-nota - ext-tit-acr-dev.sdo-tit-acr).
        NEXT titulo_acr.
...
/*                                                                                                                                                                          */
/*         FIND FIRST EXT-DOCUM-EST-DEV WHERE EXT-DOCUM-EST-DEV.v-vlr-nota > 0                                                                                              */
/*                                      AND   ext-docum-est-dev.vlr-nota   = 0 NO-ERROR.                                                                                    */
/*                                                                                                                                                                          */
/*         IF AVAIL EXT-DOCUM-EST-DEV THEN DO:                                                                                                                              */
/*                                                                                                                                                                          */
/*             FIND FIRST ext-tit-acr-dev WHERE ext-tit-acr-dev.nota-fiscal  = EXT-DOCUM-EST-DEV.nota-fiscal                                                                */
/*                                      AND   ext-tit-acr-dev.serie-nota     = EXT-DOCUM-EST-DEV.serie-nota                                                                 */
/*                                      AND   ext-tit-acr-dev.cod-emitente   = EXT-DOCUM-EST-DEV.cod-emitente NO-ERROR.                                                     */
/*                                                                                                                                                                          */
/*                 EMPTY TEMP-TABLE tt_integr_acr_lote_impl.                                                                                                                */
/*                                                                                                                                                                          */
/*                 EMPTY TEMP-TABLE tt_integr_acr_item_lote_impl.                                                                                                           */
...
/*                         ASSIGN l-parcela-unica = TRUE                                                                                                                    */
/*                                v_parcela = string(i).                                                                                                                    */
/*                                                                                                                                                                          */
/*                     END.                                                                                                                                                 */
/*                                                                                                                                                                          */
/*                     ELSE DO:                                                                                                                                             */
/*                         ASSIGN l-parcela-unica = FALSE                                                                                                                   */
/*                                i = i + 1                                                                                                                                 */
/*                                v_parcela = string(i).                                                                                                                    */
/*                     END.                                                                                                                                                 */
/*                                                                                                                                                                          */
...
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*                 DO WHILE l_unica <> TRUE:                                                                                                                                */
/*                     run pi_referencia (Input "T",                                                                                                                        */
/*                                      Input today,                                                                                                                        */
/*                                      output v_cod_refer) .                                                                                                               */
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*                           FIND FIRST movto_tit_acr NO-LOCK WHERE movto_tit_acr.cod_estab = tit_acr.cod_estab                                                             */
/*                                                            AND   movto_tit_acr.cod_refer = v_cod_refer NO-ERROR.                                                         */
/*                                                                                                                                                                          */
/*                           IF AVAILABLE movto_tit_acr THEN DO:                                                                                                            */
/*                               ASSIGN l_unica = FALSE.                                                                                                                    */
/*                           END.                                                                                                                                           */
/*                                                                                                                                                                          */
/*                           ELSE DO:                                                                                                                                       */
/*                               ASSIGN l_unica = TRUE                                                                                                                      */
/*                                      v_refer_unica = v_cod_refer.                                                                                                        */
/*                           END.                                                                                                                                           */
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*                 END.                                                                                                                                                     */
...
/*         assign tt_integr_acr_lote_impl.tta_cod_empresa                = STRING(tit_Acr.cod_empresa)                                                                      */
/*                tt_integr_acr_lote_impl.tta_cod_estab                  = EXT-TIT-ACR-DEV.cod-estab-ems5                                                                   */
/*                tt_integr_acr_lote_impl.tta_cod_estab_ext              = EXT-TIT-ACR-DEV.cod-estab-ems2                                                                   */
/*                tt_integr_acr_lote_impl.tta_cod_refer                  = v_refer_unica                                                                                    */
...
/*                tt_integr_acr_lote_impl.tta_val_tot_lote_impl_tit_acr  = ext-docum-est-dev.v-vlr-nota                                                                     */
/*                tt_integr_acr_lote_impl.tta_val_tot_lote_infor_tit_acr = ext-docum-est-dev.v-vlr-nota                                                                     */
...
/*                tt_integr_acr_item_lote_impl.tta_num_seq_refer              = 10                                                                                          */
...
/*                tt_integr_acr_item_lote_impl.tta_cod_parcela                = v_parcela                                                                                   */
/*                tt_integr_acr_item_lote_impl.tta_cod_portador               = tit_acr.cod_portador                                                                        */
/*                tt_integr_acr_item_lote_impl.tta_cod_portad_ext             = "" /*l_*/                                                                                   */
/*                tt_integr_acr_item_lote_impl.tta_cod_cart_bcia              = tit_acr.cod_cart_bcia                                                                       */
...
/*                tt_integr_acr_item_lote_impl.tta_val_liq_tit_acr            = ext-docum-est-dev.v-vlr-nota                                                                */
/*                tt_integr_acr_item_lote_impl.tta_val_tit_acr                = ext-docum-est-dev.v-vlr-nota                                                                */
...
/*                tt_integr_acr_relacto_pend.tta_cod_parcela                  = ext-tit-acr-dev.parcela                                                                     */
/*                tt_integr_acr_relacto_pend.tta_val_relacto_tit_acr          = ext-docum-est-dev.v-vlr-nota.                                                               */
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
...
/*                                                                                                                                                                          */
/*                                                                                                                                                                          */
/*         OUTPUT TO value(SESSION:TEMP-DIRECTORY + "FINAL12.txt") NO-CONVERT append.                                                                                       */
/*         FIND FIRST tt_log_erros_atualiz  NO-ERROR.                                                                                                                       */
/*                                                                                                                                                                          */
/*             IF AVAIL tt_log_erros_atualiz  THEN DO:                                                                                                                      */
/*                                                                                                                                                                          */
/*                 MESSAGE "erro gerado em: " SESSION:TEMP-DIRECTORY + "final12.txt." VIEW-AS ALERT-BOX.                                                                    */
/*                                                                                                                                                                          */
...
/*                        nota_devol_tit_acr.cod_parcela            = v_parcela                                                                                             */
...
/*                                                                                                                                                                          */
/*                ASSIGN EXT-DOCUM-EST-DEV.v-vlr-nota = 0.                                                                                                                  */
/*             END.                                                                                                                                                         */
/*                                                                                                                                                                          */
/*         END.                                                                                                                                                             */
/* END PROCEDURE.                                                                                                                                                           */
