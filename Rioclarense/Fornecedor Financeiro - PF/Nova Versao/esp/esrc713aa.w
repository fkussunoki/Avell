...
c-dig-cta c-descricao l-prefer l-poupanca l-pf 
...
DEFINE FRAME D-Dialog
     ...
     l-prefer AT ROW 7 COL 2.43 WIDGET-ID 18
     l-poupanca AT ROW 7.88 COL 2.43 WIDGET-ID 20
     l-pf AT ROW 8.79 COL 2.43 WIDGET-ID 24
     c-cpf AT ROW 10.17 COL 5.29 COLON-ALIGNED WIDGET-ID 22
...
DO:
    ...
    if  v_rec_banco <> ?
    then do:
        find emsfin.banco where recid(banco) = v_rec_banco no-lock no-error.
...
DO:
    IF INPUT FRAME {&frame-name} l-pf = YES THEN 
        ASSIGN c-cpf:SENSITIVE = YES.

    ELSE 
        ASSIGN c-cpf:SENSITIVE = NO.
...
FIND FIRST fornec_financ  WHERE recid(fornec_financ)= v_rec_fornec_financ 
    ...
    IF NOT avail cta_corren_fornec THEN DO:
        ...
        END.
            ELSE DO:
            ASSIGN substring(cta_corren_fornec.cod_livre_2, 89, 11) = INPUT FRAME {&FRAME-NAME} c-cpf.
