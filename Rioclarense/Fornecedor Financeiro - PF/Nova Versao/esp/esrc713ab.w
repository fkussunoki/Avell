...
c-dig-agencia c-cta c-dig-cta c-descricao l-prefer l-poupanca l-pf bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c-banco c-agencia c-dig-agencia c-cta ~
c-dig-cta c-descricao l-prefer l-poupanca l-pf c-cpf 
...
DEFINE VARIABLE c-cpf AS CHARACTER FORMAT "9(11)":U INITIAL "00000000000" 
     LABEL "CPF" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE l-pf AS LOGICAL INITIAL no 
     LABEL "Pessoa Fisica" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME D-Dialog
     ...
     l-poupanca AT ROW 8 COL 2.72 WIDGET-ID 20
     l-pf AT ROW 9.08 COL 2.72 WIDGET-ID 22
     c-cpf AT ROW 10.25 COL 5.14 COLON-ALIGNED WIDGET-ID 26
...
/* SETTINGS FOR FILL-IN c-cpf IN FRAME D-Dialog
   NO-ENABLE                                                            */
...
DO:
    FIND FIRST fornec_financ NO-LOCK WHERE recid(fornec_financ)= v_rec_fornec_financ NO-ERROR.

    CREATE tt-conta.
    ASSIGN tt-conta.cdn_fornecedor = fornec_financ.cdn_fornecedor 
           tt-conta.cod_banco      = p-banco
           tt-conta.cod_agenc_bcia = p-agencia
           tt-conta.cod_cta_corren = p-conta.
   ...
   IF LOGICAL(l-prefer:SCREEN-VALUE IN FRAME {&FRAME-NAME})= YES THEN DO:
    ...
    FIND FIRST fornec_financ NO-LOCK WHERE recid(fornec_financ)= v_rec_fornec_financ NO-ERROR.
        ...
        FIND FIRST cta_corren_fornec  WHERE cta_corren_fornec.cdn_fornecedor = fornec_financ.cdn_fornecedor
                                             AND   cta_corren_fornec.log_cta_corren_prefer = YES 
...
&Scoped-define SELF-NAME l-pf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL l-pf D-Dialog
ON VALUE-CHANGED OF l-pf IN FRAME D-Dialog /* Pessoa Fisica */
DO:
    IF INPUT FRAME {&frame-name} l-pf = YES THEN 
        ASSIGN c-cpf:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
     ELSE
        ASSIGN c-cpf:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
------------------------------------------------------------------------------*/
  DISPLAY c-banco c-agencia c-dig-agencia c-cta c-dig-cta c-descricao l-prefer 
          l-poupanca l-pf c-cpf 
      WITH FRAME D-Dialog.
  ENABLE rt-buttom RECT-2 c-banco c-agencia c-dig-agencia c-cta c-dig-cta 
         c-descricao l-prefer l-poupanca l-pf bt-ok bt-cancela bt-ajuda 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST fornec_financ NO-LOCK WHERE recid(fornec_financ) = v_rec_fornec_financ NO-ERROR.
    ...
    ASSIGN c-banco:SCREEN-VALUE IN FRAME {&FRAME-NAME} = cta_corren_fornec.cod_banco
           ...
           l-prefer:CHECKED IN FRAME {&FRAME-NAME} = logical(string(cta_corren_fornec.log_cta_corren_prefer))
           l-poupanca:CHECKED IN FRAME {&FRAME-NAME} = IF (SUBSTRING(cta_corren_fornec.cod_livre_2, 100,3) = '' OR SUBSTRING(cta_corren_fornec.cod_livre_2, 100,3) = ? OR SUBSTRING(cta_corren_fornec.cod_livre_2, 100,2) = 'no') THEN NO ELSE logical(substring(cta_corren_fornec.cod_livre_2, 100, 3))
           l-pf:CHECKED IN FRAME {&FRAME-NAME} =  IF (SUBSTRING(cta_corren_fornec.cod_livre_2, 103,3) = '' OR SUBSTRING(cta_corren_fornec.cod_livre_2, 103,3) = ? OR SUBSTRING(cta_corren_fornec.cod_livre_2, 103,2) = 'no')   THEN NO ELSE logical(SUBSTRING(cta_corren_fornec.cod_livre_2, 103,3))
           c-cpf:SCREEN-VALUE IN FRAME {&FRAME-NAME} = IF SUBSTRING(cta_corren_fornec.cod_livre_2, 89, 11) = '' THEN '00000000000' ELSE SUBSTRING(cta_corren_fornec.cod_livre_2, 89, 11).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE BUFFER b-conta FOR cta_corren_fornec.
FOR EACH tt-conta:
    FOR EACH cta_corren_fornec           WHERE   cta_corren_fornec.cdn_fornecedor = tt-conta.cdn_fornecedor
                                         AND     cta_corren_fornec.cod_banco      = tt-conta.cod_banco
                                         AND     cta_corren_fornec.cod_agenc_bcia = tt-conta.cod_agenc_bcia
                                         AND     cta_corren_fornec.cod_cta_corren_bco = tt-conta.cod_cta:




        ASSIGN cta_corren_fornec.cod_banco      = c-banco:SCREEN-VALUE IN FRAME {&FRAME-NAME}
        ...
        IF  INPUT FRAME {&FRAME-NAME} l-pf = NO 
        THEN DO:
        
            ASSIGN substring(cta_corren_fornec.cod_livre_2, 89, 11) = "00000000000".

            ASSIGN SUBSTRING(cta_corren_fornec.cod_livre_2, 103, 3) = "no ".
        END.
            ELSE DO:
            ASSIGN substring(cta_corren_fornec.cod_livre_2, 89, 11) = c-cpf:SCREEN-VALUE IN FRAME {&FRAME-NAME}.

            ASSIGN SUBSTRING(cta_corren_fornec.cod_livre_2, 103, 3) = "yes".
            END.
         IF INPUT FRAME {&FRAME-NAME} l-poupanca = NO
         THEN
            substring(cta_corren_fornec.cod_livre_2, 100, 3) = "no ".
         ELSE

             substring(cta_corren_fornec.cod_livre_2, 100, 3) = "yes".

        IF LOGICAL(l-prefer:SCREEN-VALUE IN FRAME {&FRAME-NAME}) = YES THEN DO:

            FIND FIRST fornec_financ  WHERE fornec_financ.cdn_fornecedor = tt-conta.cdn_fornecedor NO-ERROR.

            ASSIGN fornec_financ.cod_banco = c-banco:SCREEN-VALUE IN FRAME {&FRAME-NAME}
                   fornec_financ.cod_agenc_bcia = c-agencia:SCREEN-VALUE IN FRAME {&FRAME-NAME}
                   fornec_financ.cod_cta_corren_bco = c-cta:SCREEN-VALUE IN FRAME {&FRAME-NAME}
                   fornec_financ.cod_digito_agenc_bcia = c-dig-agencia:SCREEN-VALUE IN FRAME {&FRAME-NAME}
                   fornec_financ.cod_digito_cta_corren =  c-dig-cta:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
