...
Def New Global Shared Var h-preco-fornec      As HANDLE       No-undo.
...
IF p-ind-event  = 'AFTER-INITIALIZE' AND
   p-ind-object = 'CONTAINER' THEN DO:
    ...
    IF AVAIL mla-usuar-aprov THEN DO:
       ...
       IF (V_LOTACAO <> "10401"
       AND  V_LOTACAO <> "10402") THEN

           RUN PI-DESPESAS.


       IF (V_LOTACAO =  "10402") THEN
...
/*             DO:                                                             */
/* /*                 MESSAGE 'TRECHO NOVO' VIEW-AS ALERT-BOX.  */             */
/*                                                                             */
...
/*                                 ASSIGN  h-un:SCREEN-VALUE = "".             */
/*                              END.                                           */
/*                                                                             */
/*                              IF h-objeto3:NAME = "preco-fornec" THEN DO:    */
/*                                                                             */
/*                                 ASSIGN  h-preco-fornec = H-OBJETO3.         */
/*                                 ASSIGN  h-preco-fornec:SCREEN-VALUE = "0".  */
/*                              END.                                           */
/*                                                                             */
...
/*             DO:                                                               */
/*                                                                               */
/* /*                 MESSAGE 'ACESSEI INICIO' VIEW-AS ALERT-BOX.  */            */
/*                 IF h_frame:TYPE = "frame" THEN DO:                            */
/*                                                                               */
...
/*             MESSAGE 'TRECHO 1' VIEW-AS ALERT-BOX.  */
...
/* FIM DO NOVO TRECHO */
            IF h_frame:NAME = "fpage1" THEN
            DO:
/*                 MESSAGE 'ACESSEI MEIO' VIEW-AS ALERT-BOX.  */
                    ...
                    do  while valid-handle(h-objeto3):
                         ...
                         IF  h-objeto3:TYPE <> "field-group" and
                            ...
                            IF h-objeto3:NAME = "requisitante" THEN DO:
                                ...
                                ON 'tab':U OF h_REQUISITANTE PERSISTENT RUN upc/upcc0300b4.p.
...
/*                                                                                               */
/*                                                                                               */
/*                 ON 'f5':U OF h_dt-vencim PERSISTENT RUN upc/upcc0300b3.p.                     */
...
/*             MESSAGE 'PERTO FIM' VIEW-AS ALERT-BOX.  */
...
/*         MESSAGE 'FIMO' VIEW-AS ALERT-BOX.  */

END PROCEDURE.


PROCEDURE PI-SUPRIMENTOS:
    /* Encontra a fPage1 */
    assign h_frame = p-wgh-frame:first-child.
    assign h_frame = h_frame:first-child.
                    ...
                    do  while valid-handle(h-objeto3):
                         ...
                         IF  h-objeto3:TYPE <> "field-group" and
                             ...
                             IF h-objeto3:NAME = "requisitante" THEN DO:
                                ...
                                ASSIGN h_requisitante = h-objeto3.
                                ON 'tab':U OF h_REQUISITANTE PERSISTENT RUN upc/upcc0300b5.p.

                             END. /* requisitante */
/*                 ON 'MOUSE-SELECT-DBLCLICK':U OF h_dt-vencim PERSISTENT RUN upc/upcc0300b6.p.  */
/*                                                                                               */
/*                                                                                               */
/*                 ON 'f5':U OF h_dt-vencim PERSISTENT RUN upc/upcc0300b6.p.                     */
        ...
        if  h_frame:name = "it-codigo" then do:
                ...
                ON 'f3':U OF h_dt-vencim PERSISTENT RUN upc/upcc0300b6.p.
