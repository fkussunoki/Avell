...
*******************************************************************************/
{include/i-prgvrs.i G01ESRC610a 1.00.00.000}
...
DEFINE FRAME d-vapara
     c_nat-operacao AT ROW 1.96 COL 29.86 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Natureza */
DO:
FIND ext-conta-ft NO-LOCK
    WHERE ext-conta-ft.nat-operacao EQ INPUT FRAME {&FRAME-NAME} c_nat-operacao NO-ERROR.

IF NOT AVAILABLE ext-conta-ft THEN DO:
    {utp/ut-table.i mgesp ext-conta-ft 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(ext-conta-ft).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Natureza */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ESRC610" "1.00.00.001"}
