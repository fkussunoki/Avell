...
*******************************************************************************/
{include/i-prgvrs.i ESBRW0300 1.00.00.001}
...
def var rw-item        as rowid           no-undo.
...
DEFINE VARIABLE h_esbrw0300 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Itens de Debito Direto"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Itens de Debito Direto */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Itens de Debito Direto */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-ok w-pesquisa
ON CHOOSE OF bt-ok IN FRAME f-zoom /* OK */
DO:
  RUN pi-rowid IN h_esbrw0300 (OUTPUT rw-item).

  MESSAGE STRING(rw-item) VIEW-AS ALERT-BOX.
    ASSIGN  gr-item = rw-item.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/esbrw0300.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esbrw0300 ).
       RUN set-position IN h_esbrw0300 ( 3.00 , 3.57 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 80.14 ) */

       /* Links to SmartBrowser h_esbrw0300. */
       RUN add-link IN adm-broker-hdl ( h_esbrw0300 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_esbrw0300 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
