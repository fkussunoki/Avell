...
def var v_cdn_bco             as CHAR  no-undo.
...
PROCEDURE pi_segto_tipo_A:
        ...
        if  v_cdn_tip_forma_pagto = 6 then /* Ordem de Pagto */
            assign v_dsl_segto = v_dsl_segto + string(int(tt_param_program_formul.ttv_des_contdo),'999'). /* Banco do Brasil */
        else
            assign v_dsl_segto = v_dsl_segto + string(int(tt_param_program_formul.ttv_des_contdo),'999').

        assign v_cdn_bco = tt_param_program_formul.ttv_des_contdo.
        ...
        FIND FIRST cta_corren_fornec NO-LOCK WHERE cta_corren_fornec.cod_cta_corren_bco = string(tt_param_program_formul.ttv_des_contdo) 
                                             AND substring(cta_corren_fornec.cod_livre_2, 89, 11) <> "" NO-ERROR.

        IF AVAIL cta_corren_fornec THEN DO:
        
                ASSIGN l-pessoa-fisica = YES
                       c-cpf          = fill('0', 14 - length(substring(cta_corren_fornec.cod_livre_2, 89, 11))) +  string(substring(cta_corren_fornec.cod_livre_2, 89, 11)).
.
        END.
...
/*         assign v_dsl_segto = v_dsl_segto + string(int(tt_param_program_formul.ttv_des_contdo),'999'). /* C¢digo do banco - 001 a 003 */ */
        ...
        FIND FIRST cta_corren_fornec NO-LOCK WHERE cta_corren_fornec.cod_cta_corren_bco = string(tt_param_program_formul.ttv_des_contdo)
                                             and  cta_corren_fornec.cod_agenc_bcia = v_cod_agenc_fav
                                             and  cta_corren_fornec.cod_banco = string(v_cdn_bco)
                                             AND  length(substring(cta_corren_fornec.cod_livre_2, 89, 11)) = 11 
                                             and  substring(cta_corren_fornec.cod_livre_2, 89, 11) <> "00000000000" no-error.
        IF AVAIL cta_corren_fornec THEN DO:
        ...
        ASSIGN v_dsl_segto = v_dsl_segto + string(c-cpf).
