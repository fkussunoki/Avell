...
FIND FIRST tit_Acr WHERE tit_acr.cod_estab              = tt-titulos.ttv-estab
                   ...
                   AND   tit_Acr.cod_parcela            = tt-titulos.ttv-parcela NO-ERROR.

IF AVAIL tit_Acr THEN
    ASSIGN v_serie = int(tit_Acr.cod_ser_docto) + 10.
ELSE ASSIGN v_serie = INT(tit_Acr.cod_ser_docto).
...
RUN utp/ut-perc.p PERSISTENT SET h-prog.
        ...
        IF AVAIL portad_edi THEN DO:
            ...
            ASSIGN v_cod_nosso_numero = entry(1, portad_edi.des_tip_var_portad_edi, CHR(10)) + SUBSTRING(tit_Acr.cod_estab, 3, 1) + tit_Acr.cod_tit_acr  + SUBSTRING(tt-titulos.ttv-parcela, 2, 1).
...
ASSIGN tt_integr_acr_item_lote_impl_9.ttv_rec_lote_impl_tit_acr        = RECID(tt_integr_acr_lote_impl)
       ...
       tt_integr_acr_item_lote_impl_9.tta_cod_tit_acr_bco              = v_cod_nosso_numero + v_cod_digito_nosso_num.
...
FOR EACH tt_integr_acr_item_lote_impl_9 FIELDS (tta_cod_espec_docto tta_cod_ser_docto tta_cod_tit_acr ttv_rec_item_lote_impl_tit_acr tta_val_tit_acr)  USE-INDEX tt_id :
    ...
    FIND FIRST tt-titulos NO-lock where tt-titulos.ttv-estab = tt_integr_acr_lote_impl.tta_cod_estab
                                   and   tt-titulos.ttv-especie = tt_integr_acr_item_lote_impl_9.tta_cod_espec_docto  
                                   and   tt-titulos.ttv-titulo  = tt_integr_acr_item_lote_impl_9.tta_cod_tit_acr no-error.
