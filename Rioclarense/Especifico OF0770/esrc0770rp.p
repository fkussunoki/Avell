...
for each nota-fiscal where nota-fiscal.cod-estabel = tt-param.cod-estabel
                     and   nota-fiscal.dt-emis-nota >= tt-param.dt-periodo-ini
                     and   nota-fiscal.dt-emis-nota <= tt-param.dt-periodo-fim
                     and   nota-fiscal.idi-sit-nf-eletro = 3
                     and   nota-fiscal.ind-sit-nota <> 4
                     and   nota-fiscal.ind-tip-nota <> 8,
         ...
         each it-nota-fisc NO-LOCK WHERE it-nota-fisc.cod-estabel   = nota-fiscal.cod-estabel
                                   ...
                                   AND  it-nota-fisc.nome-ab-cli = nota-fiscal.nome-ab-cli
                                   : /* uso autorizado */
...
/*                                                                                                           */
        ...
        ASSIGN 
        tt-faturamento.ttv-cst-pis                        = "Tributado".

        IF SUBSTRING(it-nota-fisc.char-2, 96, 1) = "2"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-pis                        = "Isento".

        IF SUBSTRING(it-nota-fisc.char-2, 96, 1) = "3"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-pis                        = "Outros".

        IF SUBSTRING(it-nota-fisc.char-2, 96, 1) = "1"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-pis                        = "Reduzido".


        IF SUBSTRING(it-nota-fisc.char-2, 97, 1) = "1"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-cofins                        = "Tributado".

        IF SUBSTRING(it-nota-fisc.char-2, 97, 1) = "2"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-cofins                        = "Isento".

        IF SUBSTRING(it-nota-fisc.char-2, 97, 1) = "3"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-cofins                        = "Outros".

        IF SUBSTRING(it-nota-fisc.char-2, 97, 1) = "1"  THEN

        ASSIGN 
        tt-faturamento.ttv-cst-cofins                       = "Reduzido".
        ...
        tt-faturamento.ttv-vlr-PIS                        = (it-nota-fisc.vl-tot-item - it-nota-fisc.vl-icms-it)  * DECIMAL(substring(it-nota-fisc.char-2,76,5)) / 100 /* movto-estoq.val-cofins */
        tt-faturamento.ttv-vlr-COFINS                     = (it-nota-fisc.vl-tot-item - it-nota-fisc.vl-icms-it) * DECIMAL(substring(it-nota-fisc.char-2,81,5)) / 100  /*movto-estoq.valor-pis*/
...
FOR EACH DOcum-est WHERE docum-est.cod-estabel = tt-param.cod-estabel
                   AND   DOcum-est.dt-trans    >= tt-param.dt-periodo-ini
                   AND   DOcum-est.dt-trans    <= tt-param.dt-periodo-fim,


    EACH item-doc-est WHERE item-doc-est.cod-emitente = DOcum-est.cod-emitente
                          AND   item-doc-est.nro-docto    = DOcum-est.nro-docto
                          AND   item-doc-est.serie-docto  = DOcum-est.serie-docto
                          :

/* FOR EACH it-doc-fisc NO-LOCK WHERE it-doc-fisc.dt-docto = doc-fiscal.dt-docto          */
...
/*                              AND   it-doc-fisc.cod-emitente = doc-fiscal.cod-emitente: */
        ...
        FIND FIRST it-doc-fisc NO-LOCK WHERE it-doc-fisc.cod-estabel = DOcum-est.cod-estabel
                                       ...
                                       AND   it-doc-fisc.cod-emitente = item-doc-est.cod-emitente NO-ERROR.
...
/*                                     AND   item-doc-est.it-codigo    = it-doc-fisc.it-codigo NO-ERROR. */
...
/*                                        AND   movto-estoq.cod-emitente = doc-fiscal.cod-emitente NO-ERROR. */
        ...
        FIND FIRST emitente NO-LOCK WHERE emitente.cod-emitente = it-doc-fisc.cod-emitente NO-ERROR.
            ...
            tt-faturamento.ttv-descricao                      = ITEM.descricao-1
            tt-faturamento.ttv-ncm                            = it-doc-fisc.class-fiscal.
            ...
            RUN utp/ut-liter.p (INPUT {diinc/i01di084.i 04 it-doc-fisc.cd-trib-icm},
                                                                                INPUT "",
                                                                                INPUT "").
...
OUTPUT TO c:\desenv\esrc0770.txt.
