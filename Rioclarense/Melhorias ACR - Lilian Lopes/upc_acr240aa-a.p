...
IF VALID-HANDLE (whQryBrTable) THEN DO TRANS:
    whQryBrTable:GET-CURRENT().
    
    ASSIGN h_buffer_table = whQryBrTable:GET-BUFFER-HANDLE(1).

    IF VALID-HANDLE(whColUF_BrT) THEN DO:
        ...
        /*END.*/
        ASSIGN whColUF_BrT:SCREEN-VALUE       = v_uf
               whColTipoPgto_BrT:SCREEN-VALUE = v_tipoPgto.
    END.

    IF VALID-HANDLE(whColCodRep_BrT) THEN DO:
        /*IF fncNumerico(h_buffer_table:BUFFER-FIELD("tta_cod_tit_acr"):BUFFER-VALUE) THEN DO:*/
            ...
            END.
       /*END.*/
        ASSIGN whColCodRep_BrT:SCREEN-VALUE = STRING(v_cod_rep).
    END.

    IF VALID-HANDLE(whColPort_BrT) THEN DO:
        /*IF fncNumerico(h_buffer_table:BUFFER-FIELD("tta_cod_tit_acr"):BUFFER-VALUE) THEN DO:*/
            FOR FIRST tit_acr NO-LOCK
                WHERE tit_acr.cod_estab = STRING(h_buffer_table:BUFFER-FIELD("tta_cod_estab"):BUFFER-VALUE)
                  AND tit_acr.cod_espec_docto = STRING(h_buffer_table:BUFFER-FIELD("tta_cod_espec_docto"):BUFFER-VALUE)
            ...
            END.
        /*END.*/
        ASSIGN whColPort_BrT:SCREEN-VALUE = STRING(v_port).
    END.

END. /* IF VALID-HANDLE (whQryBrTable) THEN DO: */

    IF VALID-HANDLE(whColTipoEntrega_BrT) THEN DO:
        ...
        ASSIGN whColTipoEntrega_BrT:SCREEN-VALUE = string(v_situacao)
               whColEmpenho_BrT:SCREEN-VALUE     = string(v_empenho)     
               whColPedTotvs_BrT:SCREEN-VALUE    = string(v_pedido)    
               whColProcLicita_BrT:SCREEN-VALUE  = string(v_proc_licitacao)  
               whColLicitacao_BrT:SCREEN-VALUE   = STRING(v_licitacao)   
               whColProcesso_BrT:SCREEN-VALUE    = string(v_processo)
               whColVlTotPedido_BrT:SCREEN-VALUE = string(v_total)    
               whColVlTotPendente_BrT:SCREEN-VALUE = STRING(v_total_pendente).
