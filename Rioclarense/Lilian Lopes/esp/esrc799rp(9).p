...
assign i = 1.
          ...
          FOR EACH tit_acr NO-LOCK WHERE tit_acr.cod_empresa = v_cod_empres_usuar
              ...
              DO n1 = 1 TO num-1:
              FIND FIRST unid-neg-fat NO-LOCK WHERE unid-neg-fat.cod-estabel = tit_acr.cod_estab
                                        AND   unid-neg-fat.serie             = tit_acr.cod_ser_docto
                                        AND   unid-neg-fat.nr-nota-fis       = tit_acr.cod_tit_acr  
                                        AND   unid-neg-fat.cod_unid_negoc    = TRIM(entry(n1, tt-param.cod-unid-negoc)) NO-ERROR.

                         IF NOT AVAIL unid-neg-fat THEN NEXT blk_tit_acr.

                         ASSIGN v-cod-unid = unid-neg-fat.cod_unid_negoc.
                 ...
                 IF FIRST-OF(tit_acr.cdn_cliente) THEN DO:
                           ...
                           DO n3 = 1 TO num-3:
                               ...
                               FIND FIRST representante NO-LOCK WHERE representante.cdn_repres = clien_financ.cdn_repres 
                                                                ...
                                                                AND   representante.cdn_repres  = int(entry(n3, tt-param.cod-representante))
                                                                NO-ERROR.
...
/*     FOR EACH tt_titulos_aberto_rioclarense USE-INDEX tt_padrao:  */
    ...
    ASSIGN arquivo-num                     = 1                          
           ...
           celula-linha                    = m-linha                          
           celula-cor-interior             = 58                          
           celula-valor                    = string(tit_acr.val_origin_tit_Acr * (unid-neg-fat.val-unid-neg / tit_acr.val_origin_tit_Acr))                 
           celula-fonte-cor                = 1.      
    ...
    ASSIGN arquivo-num                     = 1                          
           ...
           celula-linha                    = m-linha                          
           celula-cor-interior             = 58                          
           celula-valor                    = string(tit_acr.val_sdo_tit_acr * (unid-neg-fat.val-unid-neg / tit_acr.val_origin_tit_Acr))               
           celula-fonte-cor                = 1.      
