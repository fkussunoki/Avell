...
def var p-ult-dia as integer no-undo.

def var v-dat-vencto-resolvida as date no-undo.
...
run qtde_dias(input month(today),
              output p-ult-dia).


assign v-dat-vencto-resolvida = date(month(today), 01, year(today)).


ASSIGN v_dat_liquidac_inicial      = v-dat-vencto-resolvida
       v_dat_liquidac_final        = TODAY
       v_dat_transacao_ini         = v-dat-vencto-resolvida
       v_dat_transacao_fim         = TODAY 
       v_dat_liquidac_inic         = v-dat-vencto-resolvida 
       v_dat_liquidac_fim          = TODAY 
       v_dat_inic_period_aux       = v-dat-vencto-resolvida 
...
DEF INPUT  param p_dias AS INTEGER.
DEF OUTPUT PARAM p_num_dia AS INTEGER.     

         if  (p_dias = 4) or (p_dias = 6) or
             (p_dias = 9) or (p_dias = 11)
          then do:
              ...
              IF (p_dias = 1) OR
              (p_dias = 3) or  (p_dias = 5) or
              (p_dias = 7) or  (p_dias = 8) OR
              (p_dias = 10) OR (p_dias = 12)
       then do:
