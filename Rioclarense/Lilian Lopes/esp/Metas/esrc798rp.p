...
DEFINE VAR c-arquiv                   AS CHAR                                  NO-UNDO.
    ...
    IF i-num-ped-exec-rpw > 0 THEN DO:
       ...
       IF AVAIL ped_exec THEN DO:
           ...
           IF AVAIL servid_exec THEN 
              ASSIGN c_arquivo = TRIM(servid_exec.nom_dir_spool) + '\' + 'ESRC798' + STRING(TIME) + '.xlsx'.
       END.
    END.

    ELSE DO:
        ASSIGN c_arquivo = SESSION:TEMP-DIRECTORY + 'ESRC798' + STRING(TIME) + '.xlsx'.
...
RUN pi-inicializar IN h-acomp (INPUT "Gerando relatorio - Wait").
...
PROCEDURE pi-prepara-auxiliar:
    ...
    RUN pi-acompanhar IN h-acomp (INPUT "PI-Auxiliar " + tt_temporaria.ttv-ramo).
...
PROCEDURE pi-prepara-liquidac:
    ...
    FOR EACH tt_tit_acr_liquidac WHERE tt_tit_acr_liquidac.tta_ind_espec_docto = "Normal" BREAK BY (tt_tit_acr_liquidac.tta_cdn_cliente):
        ...
        RUN pi-acompanhar IN h-acomp ("PI-Prepara Cliente " + STRING(tt_tit_acr_liquidac.tta_cdn_cliente) + " titulo " + tt_tit_acr_liquidac.tta_cod_tit_Acr).
...
PROCEDURE pi-prepara-abertos:
    ...
    FOR EACH tt_titulos_aberto_rioclarense:
        ...
        IF tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr  >= TODAY THEN
            ASSIGN v-cod-situacao = "AV".
        ELSE
            ASSIGN v-cod-situacao = "V".

        IF ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) >= 0
        AND ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) <= 30  THEN
            ASSIGN v-faixa = "Faixa1".

        IF ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) > 30
        AND ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) <= 60  THEN
            ASSIGN v-faixa = "Faixa2".

        IF ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) > 60
        AND ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) <= 90  THEN
            ASSIGN v-faixa = "Faixa3".

        IF ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) > 90
        AND ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) <= 120  THEN
            ASSIGN v-faixa = "Faixa4".

        IF ABS(TODAY - tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr) > 120 THEN
            ASSIGN v-faixa = "Faixa5".
        
    
    

        RUN pi-acompanhar IN h-acomp ("Prepara Abertos-Estab " + tt_titulos_aberto_rioclarense.tta_cod_estab + " Titulo " + tt_titulos_aberto_rioclarense.tta_cod_tit_acr).
...
PROCEDURE pi-prepara:
    ...
    IF v-ramo = "v1"
    OR v-ramo = "v2" THEN
    ASSIGN tt_temporaria.ttv-ramo                 = "VIP".
    ELSE
    ASSIGN tt_temporaria.ttv-ramo                 = "NORMAL".
...
/* prepara cabecalho */
  ...
  ASSIGN tt-dados.arquivo-num                     = 1                          
         ...
         tt-dados.celula-valor                    = string(TODAY)       
  ...
  ASSIGN tt-dados.arquivo-num                     = 1                          
    ...
    ASSIGN tt-dados.arquivo-num                     = 1                          
           ...
           tt-dados.celula-cor-interior             = 6                          
           tt-dados.celula-fonte-negrito            = YES
           tt-dados.celula-valor                    = "Liquidado Geral (V + AV)"                    
           tt-dados.celula-fonte-cor                = 1.               
    ...
    ASSIGN tt-dados.arquivo-num                     = 1                          
           ...
           tt-dados.celula-cor-interior             = 6                          
           tt-dados.celula-fonte-negrito            = YES
           tt-dados.celula-valor                    = "Meta (AV) (R$)"                    
           tt-dados.celula-fonte-cor                = 1.              
    ...
    ASSIGN tt-dados.arquivo-num                     = 1                          
           ...
           tt-dados.celula-cor-interior             = 6                          
           tt-dados.celula-fonte-negrito            = YES
           tt-dados.celula-valor                    = "Qto falta p/ meta (AV)"                    
           tt-dados.celula-fonte-cor                = 1.
    ...
    ASSIGN tt-dados.arquivo-num                     = 1                          
           ...
           tt-dados.celula-cor-interior             = 6                          
           tt-dados.celula-fonte-negrito            = YES
           tt-dados.celula-valor                    = "Meta (V) (R$)"                    
           tt-dados.celula-fonte-cor                = 1.
...
.                  tt-dados.celula-fonte-cor                = 1.           
        ...
        AND tt-fato2.ttv-num-situacao = "V" THEN DO:
            ...
            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  19 NO-ERROR.

            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[1] + tt-fato2.ttv-valor[2] + tt-fato2.ttv-valor[3] + tt-fato2.ttv-valor[4] + tt-fato2.ttv-valor[5]).

            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  33 NO-ERROR.

            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[1]).
            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  35 NO-ERROR.
            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[2]).

            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  37 NO-ERROR.
            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[3]).

            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  39 NO-ERROR.
            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[4]).

            FIND FIRST tt-dados WHERE tt-dados.celula-linha  = m-linha
                                AND   tt-dados.celula-coluna =  41 NO-ERROR.
            ASSIGN tt-dados.celula-valor  =  string(DECIMAL(tt-dados.celula-valor) + tt-fato2.ttv-valor[5]).
    ...
    FOR EACH tt_temporaria WHERE  tt_temporaria.ttv-dt-liquida = ? BREAK BY tt_temporaria.ttv-nom-cobrador:
        ACCUMULATE tt_temporaria.ttv_vlr_saldo_movimento (SUB-TOTAL BY tt_temporaria.ttv-nom-cobrador).

        IF LAST-OF(tt_temporaria.ttv-nom-cobrador) THEN DO:

            FIND FIRST tt-dados WHERE tt-dados.celula-coluna = 1
                                AND   tt-dados.planilha-num  = 1
                                AND   tt-dados.celula-valor  = tt_temporaria.ttv-nom-cobrador 
                                NO-ERROR.

            FIND FIRST b-tt-dados WHERE b-tt-dados.celula-coluna = 4
                                  AND   b-tt-dados.planilha-num  = 1
                                  AND   b-tt-dados.celula-linha  = tt-dados.celula-linha NO-ERROR.

           ASSIGN b-tt-dados.celula-valor                    = string(ACCUM SUB-TOTAL BY tt_temporaria.ttv-nom-cobrador tt_temporaria.ttv_vlr_saldo_movimento).                   
    ...
    FOR EACH tt_temporaria WHERE  tt_temporaria.ttv-dt-liquida <> ? BREAK BY tt_temporaria.ttv-nom-cobrador:
        ...
        IF LAST-OF(tt_temporaria.ttv-nom-cobrador) THEN DO:
            ...
            FIND FIRST b-tt-dados WHERE b-tt-dados.celula-coluna = 5
                              AND   b-tt-dados.planilha-num  = 1
                              AND   b-tt-dados.celula-linha  = tt-dados.celula-linha NO-ERROR.
    ...
    FOR EACH tt_temporaria BREAK BY tt_temporaria.ttv-nom-cobrador:
        ACCUMULATE tt_temporaria.ttv_vlr_implantacao (SUB-TOTAL BY tt_temporaria.ttv-nom-cobrador).
        ...
        IF LAST-OF(tt_temporaria.ttv-nom-cobrador) THEN DO:
            ...
            FIND FIRST b-tt-dados WHERE b-tt-dados.celula-coluna = 6
                              AND   b-tt-dados.planilha-num  = 1
                              AND   b-tt-dados.celula-linha  = tt-dados.celula-linha NO-ERROR.

           ASSIGN tt-dados.celula-valor                    = string(ACCUM SUB-TOTAL BY tt_temporaria.ttv-nom-cobrador tt_temporaria.ttv_vlr_implantacao).                 
