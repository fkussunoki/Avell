...
IF i-num-ped-exec-rpw > 0 THEN DO:
   ...
   IF AVAIL ped_exec THEN DO:
       ...
       IF AVAIL servid_exec THEN 
          ASSIGN c-arquiv = TRIM(servid_exec.nom_dir_spool) + '\' + 'ESRC799' + STRING(TIME) + '.xlsx'.
   END.
END.

ELSE DO:
    ASSIGN c-arquiv = SESSION:TEMP-DIRECTORY + 'ESRC799' + STRING(TIME) + '.xlsx'.
...
assign i = 1.
       ...
       block_tit_Acr:         
       FOR EACH tt-especie,
           EACH tit_acr NO-LOCK WHERE tit_acr.cod_empresa = tt-param.cod-empresa
