...
assign tt-param.arquivo = replace(tt-param.arquivo, ".tmp", STRING(TIME) + ".xlsx").
assign tt-param.arquivo = replace(tt-param.arquivo, ".lst", STRING(TIME) + ".xlsx").

IF tt-param.tp-execucao = 2 THEN
    ASSIGN c-arquiv = SESSION:TEMP-DIRECTORY + tt-param.arquivo.
...
assign i = 1.
       ...
       block_tit_Acr:         
       FOR EACH tt-especie,
           EACH tit_acr NO-LOCK WHERE tit_acr.cod_empresa = v_cod_empres_usuar
              ...
              IF FIRST-OF(tit_acr.cdn_cliente) THEN DO:
                            ...
                            FIND FIRST tt-representante WHERE tt-representante.ttv-cdn-representante = representante.cdn_repres NO-ERROR.


                            IF NOT AVAIL tt-representante THEN NEXT block_tit_Acr.
...
PROCEDURE pi-cria-planilha:
    ...
    ASSIGN arquivo-num                     = 1                          
        ...
        ASSIGN arquivo-num                     = 1                          
            ...
            IF (TODAY - tit_acr.dat_vencto_tit_acr) > 0
            AND (TODAY - tit_acr.dat_vencto_tit_acr) <= 30 THEN 
            ...
            IF (TODAY - tit_acr.dat_vencto_tit_acr) > 30
            AND (TODAY - tit_acr.dat_vencto_tit_acr) <= 60 THEN 
            ...
            IF (TODAY - tit_acr.dat_vencto_tit_acr) > 60
            AND (TODAY - tit_acr.dat_vencto_tit_acr) <= 90 THEN 
            ...
            IF (TODAY - tit_acr.dat_vencto_tit_acr) > 90
            AND (TODAY - tit_acr.dat_vencto_tit_acr) <= 120 THEN 
            ...
            IF (TODAY - tit_acr.dat_vencto_tit_acr) > 120
            THEN 
...
PROCEDURE pi-gera-dados:
    ...
    FOR EACH tt_titulos_aberto_rioclarense  WHERE tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr <= TODAY
        ...
        IF FIRST-OF(tt_titulos_aberto_rioclarense.ttv-nom-cobrador) THEN DO:
            ...
            ASSIGN vendedores = vendedores + "," + tt_titulos_aberto_rioclarense.ttv-nom-cobrador.
   ...
   DO i = 1 TO i-countador:
       ...
       FOR EACH tt_titulos_aberto_rioclarense  WHERE tt_titulos_aberto_rioclarense.tta_dat_vencto_tit_acr <= TODAY 
                                               AND  tt_titulos_aberto_rioclarense.ttv-nom-cobrador = ENTRY(i, vendedores)
                                               BREAK BY tt_titulos_aberto_rioclarense.ttv-nom-cobrador:
