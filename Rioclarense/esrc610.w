...
*******************************************************************************/
{include/i-prgvrs.i ESCR610 1.00.00.001}
...
DEFINE VARIABLE h_q01esrc610 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01esrc610 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 85.29 BY 15.63 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Regras Natureza"
         HEIGHT             = 15.63
         WIDTH              = 85.29
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Regras Natureza */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Regras Natureza */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 65.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01esrc610.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01esrc610 ).
       RUN set-position IN h_v01esrc610 ( 3.13 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 82.86 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01esrc610.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom\z01esrc610.w,
                     ProgVaPara = esvwr\v01esrc610a.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01esrc610 ).
       RUN set-position IN h_q01esrc610 ( 1.75 , 57.29 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01esrc610. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01esrc610 ).
       RUN add-link IN adm-broker-hdl ( h_q01esrc610 , 'Record':U , h_v01esrc610 ).

       /* Links to SmartQuery h_q01esrc610. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01esrc610 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01esrc610 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01esrc610 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01esrc610 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESRC610" "1.00.00.001"}
