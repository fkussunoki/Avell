...
procedure pi-memoria:
  ...
  /* Cria os titulos para as colunas do relat÷rio */
        ...
        RUN pi-acompanhar in h-prog (input "Nota " + tt-faturamento.ttv-nr-nota-fis).
            ...
            chworksheet:range("M" + STRING(m-linha)):SetValue(tt-faturamento.ttv-cfop).
...
ASSIGN c-cfop      =   "1201,1551,1949,2201,2202,2203,2208,2209,2557,6151,6152,6201,6202,5201,5202,5551,5915".
ASSIGN c-cfop-icms =   "1201,1551,1949,2201,2202,2203,2208,2209,2557,6151,6152,6201,6202,5201,5202,5551,5915".
ASSIGN c-cfop-ipi  =   "1201,1551,1949,2201,2202,2203,2208,2209,2557,6151,6152,6201,6202,5201,5202,5551,5915".
ASSIGN c-cfop-pis  =   "1201,1551,1949,2201,2202,2203,2208,2209,2557,6151,6152,6201,6202,5201,5202,5551,5915".
ASSIGN c-cfop-cofins = "1201,1551,1949,2201,2202,2203,2208,2209,2557,6151,6152,6201,6202,5201,5202,5551,5915".
...
//ASSIGN c-cfop-ipi  = "5101,5102,5911,6101,6102,6107,6122,6123,6910,6911".
...
FOR EACH it-nota-fisc NO-LOCK WHERE it-nota-fisc.dt-emis-nota >= tt-param.ttv-dt-ini
                              AND   it-nota-fisc.dt-emis-nota <= tt-param.ttv-dt-fim
                              AND   it-nota-fisc.cod-estabel  >= tt-param.ttv-estab-ini
                              AND   it-nota-fisc.cod-estabel  <= tt-param.ttv-estab-fim
...
/*                                                                                                           */
    ...
    FIND FIRST ped-venda NO-LOCK WHERE ped-venda.nome-abrev  = it-nota-fisc.nome-ab-cli
        ...
        IF AVAIL b-natur-oper THEN
            ASSIGN tt-faturamento.ttv-vlr-icms                  = if (it-nota-fisc.cd-trib-icm = 1  OR it-nota-fisc.cd-trib-icm = 4) then it-nota-fisc.vl-icms-it else 0.
        ELSE 
