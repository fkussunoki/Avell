...
FOR EACH it-nota-fisc NO-LOCK WHERE it-nota-fisc.dt-emis-nota >= p-dt-ini
                              AND   it-nota-fisc.dt-emis-nota <= p-dt-fim
                              AND   it-nota-fisc.nat-operacao = natur-oper.nat-operacao
                              AND   it-nota-fisc.cod-estabel  >= p-estab-ini
                              AND   it-nota-fisc.cod-estabel  <= p-estab-fim
...
/*     EXPORT DELIMITER "|" TT-FATURAMENTO.  */
    ...
    IF LAST-OF(tt-faturamento.ttv-concatena) THEN DO:
        ...
        chworksheet:range("d" + STRING(m-linha)):SetValue("=sum(d" + STRING("3") + ":" + "b" + STRING(m-linha - 1) + ")" ).
