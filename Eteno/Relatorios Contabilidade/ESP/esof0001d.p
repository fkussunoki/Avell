...
DEF INPUT PARAM p-dt-ini AS DATE.
DEF INPUT PARAM p-dt-fim AS DATE.
...
{office/office.i Excel chExcel}
    ...
    DEF TEMP-TABLE tt-compras
        ...
        FIELD ttv-natur-oper-it     AS CHAR
        FIELD ttv-natur-oper-doc    AS CHAR
...
procedure pi-memoria:
    ...
    chworksheet:range("n" + STRING(m-linha)):SetValue("Nat.Oper Item").
    chworksheet:range("o" + STRING(m-linha)):SetValue("Nat.Oper Doc").

    assign m-linha = m-linha + 1.

    for each tt-compras:
    RUN pi-acompanhar IN h-prog(INPUT "Data " + string(tt-compras.ttv-cod-estabel)).
        chworksheet:range("A" + STRING(m-linha)):SetValue(tt-compras.ttv-cod-estabel)   .
        chworksheet:range("B" + STRING(m-linha)):SetValue(tt-compras.ttv-serie)         .
        chworksheet:range("C" + STRING(m-linha)):SetValue(tt-compras.ttv-nr-nota-fis)   .
        chworksheet:range("d" + STRING(m-linha)):SetValue(tt-compras.ttv-nome-fornec)   .
        chworksheet:range("e" + STRING(m-linha)):SetValue(tt-compras.ttv-it-codigo)     .
        chworksheet:range("f" + STRING(m-linha)):SetValue(tt-compras.ttv-ge-codigo)    .
        chworksheet:range("g" + STRING(m-linha)):SetValue(tt-compras.ttv-fm-codigo)     .
        chworksheet:range("h" + STRING(m-linha)):SetValue(tt-compras.ttv-quantidade)      .
        chworksheet:range("i" + STRING(m-linha)):SetValue(tt-compras.ttv-vlr-mercadoria)  .
        chworksheet:range("j" + STRING(m-linha)):SetValue(tt-compras.ttv-vlr-PIS)         .
        chworksheet:range("k" + STRING(m-linha)):SetValue(tt-compras.ttv-vlr-COFINS)      .
        chworksheet:range("l" + STRING(m-linha)):SetValue(tt-compras.ttv-vlr-ipi)         .
        chworksheet:range("m" + STRING(m-linha)):SetValue(tt-compras.ttv-vlr-icms)        .
        chworksheet:range("n" + STRING(m-linha)):SetValue(tt-compras.ttv-natur-oper-it)        .
        chworksheet:range("o" + STRING(m-linha)):SetValue(tt-compras.ttv-natur-oper-doc)        .
...
FOR EACH docum-est NO-LOCK WHERE docum-est.dt-trans >= p-dt-ini
                           AND   docum-est.dt-trans <= p-dt-fim
                           AND   docum-est.cod-estabel >= p-estab-ini
                           AND   docum-est.cod-estabel <= p-estab-fim
                           :
    ...
    FOR EACH item-doc-est NO-LOCK WHERE item-doc-est.cod-emitente = docum-est.cod-emitente
        ...
        ASSIGN tt-compras.ttv-cod-estabel           = docum-est.cod-estabel
               ...
               tt-compras.ttv-natur-oper-it         = item-doc-est.nat-operacao
               tt-compras.ttv-natur-oper-doc        = docum-est.nat-operacao.
...
END.
                ...
                chworksheet:range("b" + STRING(m-linha)):SetValue("=sum(b" + STRING("3") + ":" + "b" + STRING(m-linha - 1) + ")" ).
                chworksheet:range("c" + STRING(m-linha)):SetValue("=sum(c" + STRING("3") + ":" + "c" + STRING(m-linha - 1) + ")" ).
                chworksheet:range("d" + STRING(m-linha)):SetValue("=sum(d" + STRING("3") + ":" + "d" + STRING(m-linha - 1) + ")" ).
...
END.
                ...
                chworksheet:range("g" + STRING(m-linha)):SetValue("Total").
                chworksheet:range("h" + STRING(m-linha)):SetValue("=sum(h" + STRING("3") + ":" + "h" + STRING(m-linha - 1) + ")" ).

                chworksheet:range("i" + STRING(m-linha)):SetValue("Total").
                chworksheet:range("j" + STRING(m-linha)):SetValue("=sum(j" + STRING("3") + ":" + "j" + STRING(m-linha - 1) + ")" ).
