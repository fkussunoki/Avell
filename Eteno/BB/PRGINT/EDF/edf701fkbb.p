...
if p_cdn_segment_edi = 309 and
   p_cdn_element_edi = 2666 then do:
...
/* Detalhe Vari˜vel - Registro Detalhe */
if p_cdn_segment_edi = 309 and
   ...
   case v_cod_segto:
     when "A" /*l_A*/  then  /* Pagamento com Cheque, OP, DOC, TED e Crýdito em Conta Corrente */

         run pi_segto_tipo_A.

     when "J" /*l_J*/  then  /* Liquida¯?o de Tðtulos (bloquetos) em cobran¯a no Ita? e em outros Bancos */
     ...
     when "O" /*l_o*/  then  /* Pagamento Contas Concession˜rias */

         run pi_segto_tipo_O.

   end case.

   return v_dsl_segto. /* Vai retornar registro com dados de detalhe vari˜vel */
end.

/* Detalhe Vari˜vel - Registro Detalhe Trailler */
if p_cdn_segment_edi = 311 and
   ...
   for each reg_proces_entr_edi no-lock where 
       ...
       /* somatorios */
       assign de-aux  = de-aux  + dec(entry(33,reg_proces_entr_edi.dsl_dados_entr_edi,chr(24)))   /* Valor Nominal do Tðtulo */       
...
message (trim(tt_param_program_formul.ttv_des_contdo)) view-as alert-box.
...
PROCEDURE pi_segto_tipo_A:
        ...
        find first tt_param_program_formul where
             ...
             no-lock no-error.
        assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,'99999999999999') + /* posicao 204 a 217 (Nr. InscriîÒo) */
                             fill(' ',12) + /* posicao 218 a 229 (Brancos) */
                             '0' + /* posicao 230 a 230 (Aviso ao Favorecido) */
                             fill(' ',10) + CHR(10). /* posicao 231 a 240 (Ocorrªncias) */
...
PROCEDURE pi_segto_tipo_J:
                ...
                if  avail tt_param_program_formul then do:
                 ...
                 assign v_dsl_segto = v_dsl_segto + string(v_dec_aux,'999999999999999'). /* posicao 130 a 144 (Acrìscimos) */
                 ...
                 assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,"x(20)" /*l_x20*/ ) + /* posicao 183 a 202 (Seu Numero) */
                                      ...
                                      fill(' ',10) + chr(10). /* posicao 231 a 240 (Brancos) */
...
procedure pi_segto_tipo_j52:
    ...
    assign v_dsl_segto = v_dsl_segto + '341' .
...
/*****************************************************************************************************************************/    
    ...
    find tt_param_program_formul
        ...
        assign v_dsl_segto = v_dsl_segto + substring(tt_param_program_formul.ttv_des_contdo, 1, 40).
	...
	/*Tipo de Incri?’o do Favorecido  9(001)*/
/*    find tt_param_program_formul
         where tt_param_program_formul.tta_cdn_segment_edi = 289
    ...
    /************************** Variable Definition End *************************/
                 ...
                 case v_forma_pagto:
                    when '16' then do: /* DARF */
                         ...
                         assign v_dsl_segto = '02'. /* IdentificaîÒo Tributo */
                         ...
                         find first emsuni.fornecedor where
                         ...
                         assign v_dsl_segto = v_dsl_segto + tt_param_program_formul.ttv_des_contdo + /* Perðodo */
                         ...
                         assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,"x(20)" /*l_x20*/ ) +  /* Seu N?mero */
                                              ...
                                              string(' ',"x(10)" /*l_x(10)*/ ) + chr(10). /* Ocorr¬ncias */
                    end.
                    when '17' then do: /* Pagamento GPS */
                        ...
                        assign v_dsl_segto = v_dsl_segto + string(v_dec_aux,'99999999999999'). /* Vlr Multa Tðtulo */
                        ...
                        assign v_dsl_segto = v_dsl_segto + tt_param_program_formul.ttv_des_contdo +  /* Data ArrecadaîÒo */
                        ...
                        assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo, "x(20)" /*l_x(20)*/ ) +  /* Seu Námero */
                               string(' ',"x(15)" /*l_x(15)*/ ) + /* Nosso N?mero */
                               string(' ',"x(10)" /*l_x(10)*/ ) + chr(10). /* Ocorr¬ncias */
                    ...
                    when '18' then do: /* DARF Simples */
                      ...
                      assign v_dsl_segto = '03'. /* IdentificaîÒo Tributo */
                      ...
                      if avail b_pessoa_jurid then do:
                             ...
                             assign v_dsl_segto = v_dsl_segto + 
                                                  string(v_tp_forn,'9') + /* Tipo InscriîÒo */
                                                  string(b_pessoa_jurid.cod_id_feder,'99999999999999'). /* InscriŒão Empresa */
                      end.
                      else do:
                          if fornecedor.num_pessoa modulo 2 = 0 then do:
                             assign v_tp_forn = 1. /* Pessoa Fisica */
                             ...
                             assign v_dsl_segto = v_dsl_segto + 
                                                  string(v_tp_forn,'9') + /* Tipo Inscri¯?o */
                                                  string(pessoa_jurid.cod_id_feder,'99999999999999'). /* InscriîÒo Empresa */
                          end.
                      end.
                      find first tt_param_program_formul where
                           tt_param_program_formul.tta_cdn_segment_edi = 289 and
                      ...
                      assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,"x(20)" /*l_x20*/ ) +  /* Seu N?mero */
                                           string(' ',"x(15)" /*l_x(15)*/ ) + /* Nosso N?mero */
                                           string(' ',"x(10)" + chr(10) /*l_x(10)*/ ). /* Ocorr¬ncias */
                    end.
                 end case.
...
PROCEDURE pi_segto_tipo_O:
            ...
            assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,"x(30)" /*l_x(30)*/ ). /* posicao 066 a 095 (Nome Concessionÿria) */
            ...
            assign v_dsl_segto = v_dsl_segto + string(tt_param_program_formul.ttv_des_contdo,"x(20)" /*l_x20*/ ) + /* posicao 175 a 194 (Seu N?mero) */
                                 fill(' ',21) + /* posicao 195 a 215 (Brancos) */  
                                 fill(' ',15) + /* posicao 216 a 230 (Nosso N?mero) */  
