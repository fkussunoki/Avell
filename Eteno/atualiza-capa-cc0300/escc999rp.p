...
{utp/ut-glob.i}
    ...
    assign c-programa  = "ESCC0999"
        c-versao       = "1.00.00.000"
        c-empresa      = "Eteno"
        c-revisao	   = "1"
        c-titulo-relat = "Atualiza pedidos".
/*                                                                      */
...
ASSIGN c-entrada = tt-param.diretorio-entrada.
...
REPEAT:
    ...
    if string(month(file-info:file-mod-date)) = string(month(today)) then do:
...
FOR EACH ttnfeproc:
    ...
    FIND FIRST pedido-compr WHERE pedido-compr.num-pedido = int(ttnfeproc.pedido) NO-ERROR.

    IF avail pedido-compr THEN DO:
        
        ASSIGN  pedido-compr.data-2 = DATE(INT(substring(ttnfeproc.dhemi, 6, 2)), INT(SUBSTRING(ttnfeproc.dhemi, 9, 2)), INT(SUBSTRING(ttnfeproc.dhemi, 1, 4))).

    END.
END.

FOR EACH ttnfeProc:
    disp stream str-rp ttnfeproc.pedido column-label "Pedido" format "9999999"
                       ttnfeproc.nnf column-label "NF" format "x(8)"
                       ttnfeproc.dhemi column-label "Data" format "x(12)"
