DEFINE TEMP-TABLE ttnfeProc
    ...
    FIELD chave          AS CHAR
    FIELD NOM-ANTIGO     AS CHAR
    FIELD ferrou         AS CHAR.
...
DEFINE TEMP-TABLE tt-estabelecimento
    FIELD cnpj           AS char
    FIELD cod            AS char.


CREATE tt-estabelecimento.
ASSIGN tt-estabelecimento.cnpj = '08516950000266'
       tt-estabelecimento.cod  = 'FBA'.

CREATE tt-estabelecimento.
ASSIGN tt-estabelecimento.cnpj = '08516950000185'
       tt-estabelecimento.cod  = 'MTZ'.
...
DEF NEW GLOBAL SHARED VAR V_ARQUIVO     AS CHAR NO-UNDO.
...
ASSIGN c-entrada = "C:\DESENV\xml\".
DEF VAR h-prog AS HANDLE NO-UNDO.


INPUT FROM os-dir(Trim(c-entrada)).

REPEAT:
    IMPORT c-arquivos.

    file-info:file-name = c-entrada + c-arquivos.
    IF LENGTH(c-arquivos) < 3 THEN NEXT. 
    IF c-arquivos MATCHES("*~~.xml") THEN  DO:
    
    if string(month(file-info:file-mod-date)) = string(month(today)) then 
        RUN pi-importa-xml( INPUT TRIM(c-entrada) + TRIM(c-arquivos)).
        ASSIGN V_ARQUIVO = TRIM(C-ENTRADA) + TRIM(C-ARQUIVOS).
...
RUN pi-inicializar IN h-prog ('Gerando').

FOR EACH ttnfeProc WHERE ttnfeProc.ferrou <> "verd":


    RUN pi-acompanhar IN h-prog(INPUT ttnfeProc.nom-antigo).
     ASSIGN c-data = substring(ttnfeproc.dhemi, 6, 2) + "_" + SUBSTRING(ttnfeproc.dhemi, 1, 4).

    FIND FIRST TT-estabelecimento NO-LOCK WHERE tt-estabelecimento.cnpj = TRIM(ttnfeProc.cnpjD) NO-ERROR.

    IF AVAIL tt-estabelecimento THEN DO:
    
        MESSAGE ttnfeProc.nom-antigo SKIP c-entrada VIEW-AS ALERT-BOX.
    OS-RENAME VALUE(ttnfeProc.nom-antigo) VALUE (c-entrada + c-data + "-" + tt-estabelecimento.cod + "-" +  ttnfeProc.xNomeE + "-" + ttnfeproc.Nnf + ".xml").
    END.
...
PROCEDURE StartElement:
   ...
   CASE qName:
       ...
       WHEN "ProcEventosNFE" THEN DO:
           CREATE ttnfeProc.
           ASSIGN ttnfeProc.nom-antigo = v_arquivo
                  ttnfeProc.ferrou      = "verd".
