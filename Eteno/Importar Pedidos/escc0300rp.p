...
define temp-table tt-cond-pagto
...
run pi-inicializar in h-prog (input "Condicao Pgto").
for each cond-pagto no-lock where cond-pagto.descricao <> "A Vista"
                            and   cond-pagto.per-pg-dup[1] = 100:
    run pi-acompanhar in h-prog(input string(cond-pagto.cod-cond-pag)).

    create tt-cond-pagto.
    assign tt-cond-pagto.cod-cond-pag = cond-pagto.cod-cond-pag
           tt-cond-pagto.prazo        = cond-pagto.prazos[1].
END.
run pi-finalizar in h-prog.

RUN utp/ut-acomp.p PERSISTENT SET h-prog.

RUN pi-inicializar IN h-prog (INPUT "Gerando TT-ITEM").

FOR EACH ITEM NO-LOCK WHERE ITEM.ge-codigo >= 30
                      AND   ITEM.ge-codigo <= 40:
run pi-acompanhar in h-prog(input item.it-codigo).

    CREATE tt-item.
    ASSIGN tt-item.it-codigo  = ITEM.it-codigo
           tt-item.tratado    = trim(replace(REPLACE(ITEM.it-codigo, "-", ""), " ", ""))
           tt-item.dep-padrao = ITEM.deposito-pad
           tt-item.un         = ITEM.un.
END.
run pi-finalizar in h-prog.
...
ASSIGN tt-param.coluna-inicial = 8
       ...
       tt-param.dir-arquivo    = tt-param.dir-arquivo
       tt-param.cod-estab      = tt-param.cod-estab.
...
DEFINE TEMP-TABLE tt-tratamento
    ...
    FIELD ttv-icms        AS DEC FORMAT '->>>,>.99'
    .
...
RUN pi-acompanhar IN h-prog (INPUT string(tt-dados.celula-linha) + " Dados " + tt-dados.formula).
    ...
    IF FIRST-OF(tt-dados.celula-linha) THEN DO:
        ...
        ASSIGN tt-tratamento.ttv-linha = tt-dados.celula-linha
               tt-tratamento.ttv-coluna = i-col-ini + 5.
    END.
        ...
        CASE tt-dados.celula-coluna:
            ...
            WHEN 4 THEN  DO:
                ...
                ASSIGN c-criterio = TRIM(tt-dados.formula-local). //se for 22 tem espacos separando caracteres do item, caso contrario, nao.
            END.

            WHEN 8 THEN  DO:
                find first tt-cond-pagto no-lock where tt-cond-pagto.prazo = int(entry(1,trim(tt-dados.valor),"A")) no-error.

                ASSIGN tt-tratamento.ttv-condicao          = tt-cond-pagto.cod-cond-pag.

            END.

            WHEN 9 THEN DO:

                IF c-criterio = "22" THEN DO:
                
                
                    FIND FIRST tt-ITEM WHERE tt-ITEM.tratado    = trim(replace(replace(tt-dados.valor, " PL14", ""), " ", "")) NO-ERROR.
    
                    IF AVAIL tt-ITEM THEN DO:
                        ASSIGN tt-tratamento.ttv-it-codigo  = tt-ITEM.it-codigo
                               tt-tratamento.ttv-deposito   = tt-item.dep-padrao
                               tt-tratamento.ttv-un         = tt-ITEM.un.
                ...
                ELSE DO:
                    ...
                    FIND FIRST tt-ITEM WHERE tt-ITEM.tratado    = trim(replace(tt-dados.valor, "PL14", "")) NO-ERROR.
            ...
            WHEN 20 THEN DO:
                ...
                ASSIGN tt-tratamento.ttv-icms              = DEC(tt-dados.formula-local).
...
find first item no-lock where item.it-codigo = tt-tratamento.ttv-it-codigo no-error.
    ...
    if avail item then do:
        ...
        FIND FIRST cond-pagto NO-LOCK WHERE cond-pagto.cod-cond-pag = tt-tratamento.ttv-condicao NO-ERROR.

        CREATE pedido-compr.
        ASSIGN pedido-compr.num-pedido       = fncProxNumPedido()
               pedido-compr.cod-cond-pag     =  cond-pagto.cod-cond-pag
        ...
        ASSIGN ordem-compra.numero-ordem         = fncProxNumOrdem()
               ...
               ordem-compra.qt-solic             = (tt-tratamento.ttv-quantidade * 1000)
               ordem-compra.preco-orig           = (tt-tratamento.ttv-preco / 1000) 
               ordem-compra.preco-unit           = (tt-tratamento.ttv-preco / 1000)
           ...
           ASSIGN prazo-compra.numero-ordem         = ordem-compra.numero-ordem
                  ...
                  prazo-compra.quantid-orig         = (tt-tratamento.ttv-quantidade * 1000)
                  prazo-compra.quantidade           = (tt-tratamento.ttv-quantidade * 1000)
                  prazo-compra.quant-saldo          = (tt-tratamento.ttv-quantidade * 1000)
                ...
                IF AVAIL tt-cotacao-item THEN DO:
                    ...
                    ASSIGN tt-cotacao-item.un           = prazo-compra.un
                           ...
                           tt-cotacao-item.aliquota-icm   = tt-tratamento.ttv-icm
