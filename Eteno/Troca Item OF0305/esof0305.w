    ...
    IF l-logico THEN DO:
        ...
        FOR EACH tt-notas WHERE tt-notas.ttv-selecionado = YES:
            ...
            FIND FIRST it-doc-fisc WHERE it-doc-fisc.cod-estabel = tt-notas.ttv-estab
                                           ...
                                           AND   it-doc-fisc.it-codigo    = tt-notas.ttv-it-codigo-or
                                           AND   it-doc-fisc.nr-doc-fis   = tt-notas.ttv-nota-fiscal
                                           AND   it-doc-fisc.nat-operacao = tt-notas.ttv-nat-oper
                                           NO-ERROR.


            ASSIGN it-doc-fisc.it-codigo = tt-notas.ttv-it-codigo.
...
ASSIGN c-natureza:SCREEN-VALUE IN FRAME {&FRAME-NAME} = '5102TT, 5102TO, 5102PI, 6123TT, 6123TO, 6123PI, 6102TT, 6102TO, 6102PI, 2102OO, 2102PI, 2102TO, 2102TT, 2152OO, 2152PI, 2152TO, 2512TT, 2202, 2202PI, 2202TT, 211MTZ'.
...
DO i = 1 TO NUM-ENTRIES(INPUT FRAME {&FRAME-NAME} c-natureza):
    ...
    FOR EACH it-nota-fisc NO-LOCK WHERE it-nota-fisc.cod-estabel >= INPUT FRAME {&FRAME-NAME} c-estab-ini
                              AND      it-nota-fisc.cod-estabel <= INPUT FRAME {&FRAME-NAME} c-estab-fim
                              AND      it-nota-fisc.dt-emis-nota >= INPUT FRAME {&FRAME-NAME} c-dt-ini
                              AND      it-nota-fisc.dt-emis-nota <= INPUT FRAME {&FRAME-NAME} c-dt-fim
                              AND      it-nota-fisc.nat-operacao = trim(ENTRY(i, INPUT FRAME {&FRAME-NAME} c-natureza))
                              and      it-nota-fisc.dt-cancela = ?:
       RUN pi-acompanhar IN h-prog (INPUT "Nat.Op " + it-nota-fisc.nat-operacao + " Dt.Emiss " + string(it-nota-fisc.dt-emis-nota)).


        FIND FIRST item NO-LOCK WHERE item.it-codigo = "1 " + it-nota-fisc.it-codigo 
                                or    item.it-codigo = "1"  + substring(it-nota-fisc.it-codigo, 1, 15) NO-ERROR.
    
        IF AVAILABLE item THEN DO:
            
            CREATE TT-NOTAS.
            ASSIGN TT-NOTAS.TTV-IT-CODIGO = item.it-codigo
                   TT-NOTAS.TTV-NOTA-FISCAL = it-nota-fisc.nr-nota-fis
                   TT-NOTAS.TTV-SERIE       = it-nota-fisc.serie
                   TT-NOTAS.TTV-NAT-OPER    = it-nota-fisc.nat-operacao
                   TT-NOTAS.TTV-EMITENTE    = it-nota-fisc.cd-emitente
                   TT-NOTAS.TTV-DT-EMISS    = it-nota-fisc.dt-emis-nota
                   tt-notas.ttv-estab       = it-nota-fisc.cod-estabel
                   tt-notas.ttv-it-codigo-or = it-nota-fisc.it-codigo.



            FIND FIRST it-doc-fisc WHERE it-doc-fisc.cod-estabel = it-nota-fisc.cod-estabel
                                           AND   it-doc-fisc.serie       = it-nota-fisc.serie
                                           AND   it-doc-fisc.cod-emitente = it-nota-fisc.cd-emitente
            ...
            IF NOT AVAIL it-doc-fisc THEN
                ...
                ASSIGN tt-notas.ttv-selecionado = NO.

            ELSE
                ASSIGN tt-notas.ttv-selecionado = YES.


           FIND FIRST B-ITEM WHERE B-ITEM.nat-docum = it-nota-fisc.nat-operacao
                             AND   B-ITEM.nr-docum   = it-nota-fisc.nr-nota-fis
                             AND   B-ITEM.serie-docum = it-nota-fisc.serie NO-ERROR.
           ...
           IF AVAIL B-ITEM THEN DO:
               ...
               CREATE TT-NOTAS.
               ASSIGN TT-NOTAS.TTV-IT-CODIGO =  item.it-codigo
                      TT-NOTAS.TTV-NOTA-FISCAL = B-ITEM.nr-nota-fis
                      TT-NOTAS.TTV-SERIE       = B-ITEM.serie
                      TT-NOTAS.TTV-NAT-OPER    = B-ITEM.nat-operacao
                      TT-NOTAS.TTV-EMITENTE    = B-ITEM.cd-emitente
                      TT-NOTAS.TTV-DT-EMISS    = B-ITEM.dt-emis-nota
                      tt-notas.ttv-estab       = B-ITEM.cod-estabel
                      tt-notas.ttv-it-codigo-or = B-ITEM.it-codigo.
 
               FIND FIRST it-doc-fisc         WHERE it-doc-fisc.cod-estabel = B-ITEM.cod-estabel
                                              AND   it-doc-fisc.serie       = B-ITEM.serie
                                              AND   it-doc-fisc.cod-emitente = B-ITEM.cd-emitente
                                              AND   it-doc-fisc.it-codigo    = B-ITEM.it-codigo
                                              AND   it-doc-fisc.nr-doc-fis  = B-ITEM.nr-nota-fis
                                              NO-ERROR.
...
DO i = 1 TO NUM-ENTRIES(INPUT FRAME {&FRAME-NAME} c-natureza):
    ...
    FOR EACH docum-est NO-LOCK WHERE   docum-est.cod-estabel >= INPUT FRAME {&FRAME-NAME} c-estab-ini
        ...
        EACH item-doc-est NO-LOCK WHERE item-doc-est.cod-emitente = docum-est.cod-emitente
                                  ...
                                  AND   item-doc-est.nat-of       = trim(ENTRY(i, INPUT FRAME {&FRAME-NAME} c-natureza))
                                  :

        RUN pi-acompanhar IN h-prog (INPUT "Nat.Op " + docum-est.nat-operacao + " Dt.Emiss " + string(docum-est.dt-trans)).


         FIND FIRST item NO-LOCK WHERE item.it-codigo = "1 " + item-doc-est.it-codigo 
                                or     item.it-codigo = "1"  + substring(item-doc-est.it-codigo, 1, 15) NO-ERROR.

         IF AVAILABLE item THEN DO:

             CREATE TT-NOTAS.
             ASSIGN TT-NOTAS.TTV-IT-CODIGO = item.it-codigo
                    TT-NOTAS.TTV-NOTA-FISCAL = item-doc-est.nro-docto
                    TT-NOTAS.TTV-SERIE       = item-doc-est.serie-docto
                    TT-NOTAS.TTV-NAT-OPER    = item-doc-est.nat-operacao
                    TT-NOTAS.TTV-EMITENTE    = item-doc-est.cod-emitente
                    TT-NOTAS.TTV-DT-EMISS    = docum-est.dt-trans
                    tt-notas.ttv-estab       = docum-est.cod-estabel
                    tt-notas.ttv-it-codigo-or = item-doc-est.it-codigo.
                    
             FIND FIRST it-doc-fisc         WHERE it-doc-fisc.cod-estabel = docum-est.cod-estabel
