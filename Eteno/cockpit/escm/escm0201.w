...
DEFINE TEMP-TABLE tt-ped-venda 
    ...
    field ttv-vlr-tit-vcdo  AS DEC FORMAT "->>>,>>>,>>>,>>>,>>>,>>>,>>9.99"
    FIELD ttv-vl-limite-grupo AS DEC FORMAT "->>>,>>>,>>>,>>>,>>>,>>>,>>9.99"
    FIELD ttv-usuar-checkin   AS char
...
DEFINE TEMP-TABLE tt-limite-grupo
    FIELD ttv-cdn-cliente AS INTEGER
    FIELD ttv-vl-limite   AS DEC FORMAT "->>>,>>>,>>>,>>>,>>9.99".
...
DEF NEW GLOBAL SHARED VAR glb_h_browser_1 AS HANDLE  NO-UNDO.
DEF NEW GLOBAL SHARED VAR glb_h_viewer_1  AS HANDLE  NO-UNDO.
DEF new global shared VAR l-matriz        AS LOGICAL NO-UNDO.
...
DEF var pde-saldo-vw AS DECIMAL NO-UNDO.
DEFINE VARIABLE  c-usuario AS char NO-UNDO.

def var de-saldo-cli   like nota-fiscal.vl-tot-nota no-undo.
DEF new global shared VAR de-lim-credito   AS DECIMAL NO-UNDO.
...
BUTTON-9 BUTTON-10 fi-nr-pedcli TOGGLE-1 f-situacao fi-email-repres ~
...
DEFINE BUTTON bt-rejeitar 
     LABEL "REJEITAR" 
     SIZE 15 BY 1.13.

DEFINE VARIABLE c-user AS CHARACTER FORMAT "X(256)":U 
     LABEL "Usuario - Checkin" 
     VIEW-AS FILL-IN 
     SIZE 17.72 BY .79 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-pedidos wWindow _FREEFORM
 ...
 tt-ped-venda.ttv-vlr-tit-aberto                                              LABEL "Vl. Dupl.Vencer"        FORMAT "->,>>>,>>>,>>9.99"
 tt-ped-venda.ttv-vlr-tit-vcdo                                                LABEL "Vl. Dupl.Vcdo"          FORMAT "->,>>>,>>>,>>9.99"
 ( tt-ped-venda.ttv-vlr-tit-aberto + tt-ped-venda.ttv-vlr-tit-vcdo )          LABEL "Total Titulos"          FORMAT "->,>>>,>>>,>>9.99"
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS MULTIPLE SIZE 137.72 BY 13.08
         FONT 1
...
DEFINE FRAME fpg0
     ...
     BUTTON-9 AT ROW 2.75 COL 87.72 WIDGET-ID 112
     BUTTON-10 AT ROW 2.75 COL 99.29 WIDGET-ID 114
...
DEFINE FRAME fpage1
     ...
     bt-aprova AT ROW 14.25 COL 1.72 WIDGET-ID 4
     bt-rejeitar AT ROW 14.29 COL 17.72 WIDGET-ID 6
     c-user AT ROW 14.42 COL 107.86 COLON-ALIGNED WIDGET-ID 8
...
DO:
    IF NOT AVAIL tt-ped-venda THEN DO:
       assign fi-email-repres:SCREEN-VALUE IN FRAME fPg0 = ""
              ...
              c-user:SCREEN-VALUE IN FRAME fpage1 = "".
    END.
  ELSE DO:
      ASSIGN fi-email-repres:SCREEN-VALUE IN FRAME fpg0    = tt-ped-venda.ttv-email
             ...
             c-user:SCREEN-VALUE IN FRAME fpage1 = tt-ped-venda.ttv-usuar-checkin.
     ...
     IF c-user:SCREEN-VALUE IN FRAME fpage1 <> v_cod_usuar_corren 
     THEN
         ASSIGN bt-aprova:SENSITIVE IN FRAME fpage1 = NO
                bt-rejeitar:SENSITIVE IN FRAME fpage1 = NO
                button-9:SENSITIVE IN FRAME fpg0 = NO
                BUTTON-10:SENSITIVE IN FRAME fpg0 = NO
                .

     IF c-user:SCREEN-VALUE IN FRAME fpage1 = v_cod_usuar_corren
     OR c-user:SCREEN-VALUE IN FRAME fpage1 = "" THEN
         ASSIGN bt-aprova:SENSITIVE IN FRAME fpage1 = YES
                bt-rejeitar:SENSITIVE IN FRAME fpage1 = YES
         button-9:SENSITIVE IN FRAME fpg0 = YES
         BUTTON-10:SENSITIVE IN FRAME fpg0 = YES.
...
ON CHOOSE OF bt-aprova IN FRAME fpage1 /* APROVAR */
DO:
...
&Scoped-define FRAME-NAME fpage1
&Scoped-define SELF-NAME bt-rejeitar
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-rejeitar wWindow
ON CHOOSE OF bt-rejeitar IN FRAME fpage1 /* REJEITAR */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-10 wWindow
ON CHOOSE OF BUTTON-10 IN FRAME fpg0 /* Check-out */
DO:
  ...
  ELSE DO:
      ...
      do i-cont = 1 to br-pedidos:NUM-SELECTED-ROWS IN FRAME fpage1:
          ...
          FIND FIRST ext-ped-venda2 WHERE ext-ped-venda2.nome-abrev = tt-ped-venda.ttv-nome-abrev
                                                 AND   ext-ped-venda2.nr-pedcli  = tt-ped-venda.ttv-nr-pedcli
                                                 NO-ERROR.
           IF avail ext-ped-venda2 THEN DO:
        
                    DELETE ext-ped-venda2.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON-9 wWindow
ON CHOOSE OF BUTTON-9 IN FRAME fpg0 /* Check-in */
DO:
  ...
  ELSE DO:
      ...
      do i-cont = 1 to br-pedidos:NUM-SELECTED-ROWS IN FRAME fpage1:
          ...
          FIND FIRST ext-ped-venda2 NO-LOCK WHERE ext-ped-venda2.nome-abrev = tt-ped-venda.ttv-nome-abrev
                                                 AND   ext-ped-venda2.nr-pedcli  = tt-ped-venda.ttv-nr-pedcli
                                                 NO-ERROR.
           IF NOT avail ext-ped-venda2 THEN DO:
        
                    CREATE ext-ped-venda2.
                    ASSIGN ext-ped-venda2.nome-abrev = tt-ped-venda.ttv-nome-abrev
                           ext-ped-venda2.nr-pedcli  = tt-ped-venda.ttv-nr-pedcli
                           ext-ped-venda2.date-1     = TODAY
                           ext-ped-venda2.cod-usuar  = v_cod_usuar_corren.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE rt-button BUTTON-4 bt-carrega BUTTON-8 BUTTON-9 BUTTON-10 fi-nr-pedcli 
         TOGGLE-1 f-situacao fi-email-repres fi-vl-tot-abe fi-vl-tot-liq 
         fi-vl-tot-ped 
      WITH FRAME fpg0 IN WINDOW wWindow.
  {&OPEN-BROWSERS-IN-QUERY-fpg0}
  DISPLAY c-user 
      WITH FRAME fpage1 IN WINDOW wWindow.
  ENABLE br-pedidos bt-aprova bt-rejeitar c-user 
      WITH FRAME fpage1 IN WINDOW wWindow.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCM0201" "1.00.00.001"}
...
------------------------------------------------------------------------------*/
    ...
    ELSE DO:                                                                                                               
        FOR EACH ped-venda no-lock                                                                                         
           ...
           WHERE emitente.cod-emitente = ped-venda.cod-emitente
            ...
            RUN pi-acompanhar IN h-acomp (INPUT "Nr Pedido: " + STRING(ped-venda.nr-pedido) + " - Data: " + STRING(ped-venda.dt-implant,"99/99/9999")).
            FIND FIRST ext-ped-venda2 NO-LOCK WHERE ext-ped-venda2.nome-abrev = ped-venda.nome-abrev
                                             AND   ext-ped-venda2.nr-pedcli  = ped-venda.nr-pedcli NO-ERROR.

            IF avail ext-ped-venda2 THEN
                ASSIGN c-usuario = ext-ped-venda2.cod-usuar.
            ELSE
                ASSIGN c-usuario = "".
    ...
    RUN pi-limite-grupos(INPUT TABLE tt-ped-venda).
...
assign tt-ped-venda.ttv-cod-estabel       = ped-venda.cod-estabel
       ...
       tt-ped-venda.ttv-usuar-checkin    =  c-usuario
...
for each ped-item of ped-venda no-lock:
    ...
    for each tit_acr no-lock where tit_acr.cod_empresa = v_cod_empres_usuar
        ...
        ELSE IF tit_acr.dat_vencto_tit_acr <= TODAY THEN DO:
         ...
         if tit_acr.ind_tip_espec_docto = "Antecipa‡Æo" or
            tit_acr.ind_tip_espec_docto = "Nota de cr‚dito" then
             ...
             assign tt-ped-venda.ttv-vlr-tit-vcdo     = tt-ped-venda.ttv-vlr-tit-vcdo - tit_acr.val_sdo_tit_acr.
         else
             assign tt-ped-venda.ttv-vlr-tit-vcdo     = tt-ped-venda.ttv-vlr-tit-vcdo + tit_acr.val_sdo_tit_acr.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-limite-grupos wWindow 
PROCEDURE pi-limite-grupos :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAM TABLE FOR tt-ped-venda.
DEF BUFFER b-emitente FOR emitente.
/* FOR EACH tt-ped-venda NO-LOCK BREAK BY tt-ped-venda.ttv-cdn-cliente:                             */
/*         FIND FIRST emitente WHERE emitente.cod-emitente = tt-ped-venda.ttv-cdn-cliente NO-ERROR. */
/*                                                                                                  */
/*         IF AVAIL emitente THEN DO:                                                               */
/*     run esp\ES0017B.P(input rowid(emitente), output pde-saldo-vw).                               */
/*                                                                                                  */
/*               MESSAGE de-lim-credito VIEW-AS ALERT-BOX.                                          */
/*     ASSIGN tt-ped-venda.ttv-vl-limite-grupo = de-lim-credito.                                    */
/*                                                                                                  */
/*   END.                                                                                           */
/* END.                                                                                             */
...
FOR EACH tt-ped-venda NO-LOCK BREAK BY tt-ped-venda.ttv-cdn-cliente:
    ...
    IF LAST-OF(tt-ped-venda.ttv-cdn-cliente) THEN DO:

        FIND FIRST emitente NO-LOCK WHERE emitente.cod-emitente = tt-ped-venda.ttv-cdn-cliente NO-ERROR.

        FOR EACH b-emitente NO-LOCK WHERE b-emitente.nome-matriz = emitente.nome-matriz
                                    AND   b-emitente.ind-abrange-aval = 2
                                    AND   b-emitente.dt-lim-cred >= TODAY BREAK BY b-emitente.nome-matriz :

            ACCUMULATE b-emitente.lim-credito (SUB-TOTAL BY b-emitente.nome-matriz).

            IF LAST-OF (b-emitente.nome-matriz) THEN DO:

                CREATE tt-limite-grupo.
                ASSIGN tt-limite-grupo.ttv-cdn-cliente = tt-ped-venda.ttv-cdn-cliente
                       tt-limite-grupo.ttv-vl-limite   = ACCUM SUB-TOTAL BY b-emitente.nome-matriz b-emitente.lim-credito.
...
FOR EACH tt-ped-venda:
    ...
    FIND FIRST tt-limite-grupo WHERE tt-limite-grupo.ttv-cdn-cliente = tt-ped-venda.ttv-cdn-cliente NO-ERROR.

    IF AVAIL tt-limite-grupo THEN DO:
    
    ASSIGN tt-ped-venda.ttv-vl-limite-grupo = tt-limite-grupo.ttv-vl-limite.
    END.

END.

EMPTY TEMP-TABLE tt-limite-grupo.
