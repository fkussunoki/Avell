...
**
** Programa..............: bas_demonst_ctbl_fin
** Descricao.............: Demonstrativo Cont†bil
** Versao................:  1.00.00.106
** Procedimento..........: con_demonst_ctbl_video
** Nome Externo..........: prgfin/mgl/MGLA204aa.p
** Data Geracao..........: 28/02/2014 - 14:18:35
** Criado por............: Dalpra
** Criado em.............: 28/11/2000 10:01:44
** Alterado por..........: si1768
** Alterado em...........: 29/11/2013 11:15:33
** Gerado por............: si1768
*****************************************************************************/

/*-- Filtro Multi-idioma Aplicado --*/

def var c-versao-prg as char initial " 1.00.00.106":U no-undo.
def var c-versao-rcode as char initial "[[[1.00.00.106[[[":U no-undo. /* Controle de Versao R-CODE - Nao retirar do Fonte */
...
DO:
    ...
    if  search("prgfin/mgl/MGLA204zb.r") = ? and search("prgfin/mgl/MGLA204zb.p") = ? then do:
        if  v_cod_dwb_user begins 'es_' then
            return "Programa execut†vel n∆o foi encontrado:" /*l_programa_nao_encontrado*/  + "prgfin/mgl/MGLA204zb.p".
        else do:
            message getStrTrans("Programa execut†vel n∆o foi encontrado:", "MGL") /*l_programa_nao_encontrado*/  "prgfin/mgl/MGLA204zb.p"
                   view-as alert-box error buttons ok.
            stop.
        end.
    end.
    else
        run prgfin/mgl/MGLA204zb.p /*prg_fnc_prefer_demonst_ctbl*/.
...
DO:
    ...
    assign v_nom_prog     = substring(current-window:title, 1, max(1, length(current-window:title) - 10))
           ...
           v_nom_prog_ext = "prgfin/mgl/MGLA204aa.p":U
           v_cod_release  = trim(" 1.00.00.106":U).
...
then do:
    run pi_version_extract ('bas_demonst_ctbl_fin':U, 'prgfin/mgl/MGLA204aa.p':U, '1.00.00.106':U, 'pro':U).
end /* if */.
    ...
    &if '{&emsfin_version}' >= '5.05' &then
        ...
        &else
            if  can-find(first emsbas.histor_exec_especial 
               where emsbas.histor_exec_especial.cod_prog_dtsul = 'spp_exec_orctaria')
            then do:
                assign v_log_exec_orctaria = yes.
            end /* if */.
        &endif
...
if  search("prgfin/mgl/MGLA204ab.r") = ? and search("prgfin/mgl/MGLA204ab.p") = ? then do:
    if  v_cod_dwb_user begins 'es_' then
        return "Programa execut†vel n∆o foi encontrado:" /*l_programa_nao_encontrado*/  + "prgfin/mgl/MGLA204ab.p".
    else do:
        message getStrTrans("Programa execut†vel n∆o foi encontrado:", "MGL") /*l_programa_nao_encontrado*/  "prgfin/mgl/MGLA204ab.p"
               view-as alert-box error buttons ok.
        return.
    end.
end.
else
    run prgfin/mgl/MGLA204ab.p /*prg_spp_demonst_ctbl_acerto*/.
...
PROCEDURE pi_tt_demonst_ctbl_fin_new_unid_organ:
    ...
    if  tt_demonst_ctbl_fin.tta_cod_unid_organ_filho <> "" then do:
        for each emsuni.estrut_unid_organ FIELDS(cod_unid_organ_pai cod_unid_organ_filho) no-lock
            where estrut_unid_organ.cod_unid_organ_pai    = tt_demonst_ctbl_fin.tta_cod_unid_organ_filho:

            find emsuni.unid_organ 
...
PROCEDURE pi_habilita_itens_expansao:
    ...
    if  not avail tt_compos_demonst_cadastro then do:
        ...
        and not can-find(first estrut_cta_ctbl
            ...
            assign  m_2:SENSITIVE  = NO
                    m_14:SENSITIVE = YES. /*alteracao para Belliz*/
        ...
        and not can-find(first emsuni.estrut_unid_organ
...
PROCEDURE pi_razao_contabil_demonst: 
    ...
    then do:
        ...
        if  search("prgfin/fgl/fgl208aa.r") = ? and search("prgfin/fgl/fgl208aa.p") = ? then do:
            if  v_cod_dwb_user begins 'es_' then
            ...
            run prgfin/fgl/fgl208aa.p /*prg_bas_cta_ctbl_razao*/.
    END.
    else do:
        ...
        FIND emsuni.ccusto NO-LOCK
            WHERE ccusto.cod_empresa    = plano_ccusto.cod_empresa
              ...
              AND ccusto.cod_ccusto = tt_demonst_ctbl_fin.tta_cod_ccusto_filho NO-ERROR.
...
PROCEDURE pi_bt_atz_demonst:
        ...
        &if integer(entry(1,proversion,'.')) >= 8 &then 
            &if '{&emsbas_version}' > '1.00' &then
                ...
                ***/
                run prgfin/mgl/MGLA204za.py(1,input table tt_exec_rpc,output v_des_lista_estab,output table tt_log_erros) no-error.
...
/*  END.                                                                                  */
 ...
 OTHERWISE DO: /* caso nao esteja como titulo, conta, centro de custo, estabelecimento, unidade de negocio faz*/
    ...
    END. /* begins ( */
     ...
     END. /* nao comeca com ( */
             ...
             IF AVAIL tt-receitas THEN DO:
                 ...
                 IF (v_coluna4) <= (100 - v_var_ccusto) 
                    ...
                    IF (absolute(v_coluna2) - ABSOLUTE(v_coluna3)) > v_vlr_minimo THEN DO: /* era R$ 300,00*/
                     ...
                     FIND FIRST ext_justificativa NO-LOCK WHERE ext_justificativa.Cod_demonst = conjto_prefer_demonst.cod_demonst_ctbl
                                                          ...
                                                          AND   ext_justificativa.Rubrica     = trim(tt_demonst_ctbl_fin.ttv_cod_campo_1)
                                                          AND   ext_justificativa.Entidade    = ext_justificativa.Entidade
        ...
        END.       /*fim da TT-Receitas */
         ...
         IF ((v_coluna4) - 100 >= v_var_ccusto 
         OR  (v_coluna4) - 100 <= v_var_ccusto_neg)
         THEN DO:

           
            IF (absolute(v_coluna3) - ABSOLUTE(v_coluna2)) > v_vlr_minimo THEN DO: /* era R$ 300,00*/
   ...
   END.   /* fim da variacao % */
        ...
        IF  v_coluna4 = 0 
        AND (absolute(v_coluna3) - ABSOLUTE(v_coluna2)) > v_vlr_minimo
         THEN DO:
...
PROCEDURE pi_consulta_empenho:
    ...
    IF conjto_prefer_demonst.cod_padr_col_demonst_ctbl = "Pacotes" THEN
    ...
    assign v_rec_empresa           = recid(empresa)
           ...
           v_cod_unid_orctaria_aux = "000000"
           v_cod_unid_orctaria_ate = "999999". 
...
PROCEDURE pi_cria_menu:
    ...
    then do:
        ...
        triggers:
            on choose persistent run prgfin/mgl/MGLA204zh.p (input p_num_menu,
                                                        input this-procedure).
        end triggers.
    end.
...
IF trim(tt_demonst_ctbl_fin.ttv_cod_campo_2) BEGINS "(" THEN 
 ...
 ELSE 
   ...
   ASSIGN v_cp = v_cp3 - v_cp2.
    ...
    RUN prgfin/mgl/mgla12010.w (INPUT conjto_prefer_demonst.cod_demonst_ctbl,
                                ...
                                INPUT conjto_prefer_demonst.cod_padr_col_demonst_ctbl,
                                INPUT IF TRIM(tt_demonst_ctbl_fin.ttv_cod_campo_4) BEGINS "(" THEN ENTRY(1, entry(2, TRIM(ttv_cod_campo_4), "(" ) , ")") ELSE TRIM(ttv_cod_campo_4) ,
                                INPUT v_cp).
