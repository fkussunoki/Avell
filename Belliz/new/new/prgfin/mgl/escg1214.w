...
DEFINE VARIABLE h_qrymgla1214 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vwrmgla1214 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Codigos de Situacao"
         HEIGHT             = 5.54
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Codigos de Situacao */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Codigos de Situacao */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/escgvwr/v01escg1214.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vwrmgla1214 ).
       RUN set-position IN h_vwrmgla1214 ( 2.92 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 3.42 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/escgqry/Q01escg1214.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qrymgla1214 ).
       RUN set-position IN h_qrymgla1214 ( 1.08 , 56.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_vwrmgla1214. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_vwrmgla1214 ).
       RUN add-link IN adm-broker-hdl ( h_qrymgla1214 , 'Record':U , h_vwrmgla1214 ).

       /* Links to SmartQuery h_qrymgla1214. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_qrymgla1214 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_qrymgla1214 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_qrymgla1214 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vwrmgla1214 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
