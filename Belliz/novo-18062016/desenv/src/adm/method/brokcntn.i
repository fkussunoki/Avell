...
------------------------------------------------------------------------------*/
  ...
  IF NOT VALID-HANDLE (p-proc-hdl) OR 
         p-proc-hdl:TYPE <> "PROCEDURE":U OR
         p-proc-hdl:FILE-NAME <> p-proc-name THEN 
     RETURN ERROR.  
  /* Setar o Private Data do objeto para receber o file-name, altera‡Æo 
     feita para preparar os EPCs para a altera‡Æo de nomes dos objetos. 
     Sergio Weber Jr. - 16/02/1999 */
  assign p-caller:private-data = p-caller:file-name.
  assign p-proc-hdl:private-data = p-proc-hdl:file-name. 
  ...
  IF obj-version NE "{&ADM-VERSION}":U THEN DO:
