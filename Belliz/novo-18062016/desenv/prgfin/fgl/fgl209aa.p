...
**
** Programa..............: bas_ccusto_razao
** Descricao.............: Consulta Raz∆o Centro Custo
** Versao................:  1.00.00.047
** Procedimento..........: con_razao_ccusto
** Nome Externo..........: prgfin/fgl/fgl209aa.p
** Data Geracao..........: 25/03/2015 - 16:12:54
** Criado por............: Henke
** Criado em.............: 20/12/1995 11:50:29
** Alterado por..........: si1768
** Alterado em...........: 04/10/2013 13:21:25
** Gerado por............: fut41061
*****************************************************************************/

/*-- Filtro Multi-idioma Aplicado --*/

def var c-versao-prg as char initial " 1.00.00.047":U no-undo.
def var c-versao-rcode as char initial "[[[1.00.00.047[[[":U no-undo. /* Controle de Versao R-CODE - Nao retirar do Fonte */
...
{include/i-license-manager.i bas_ccusto_razao FGL}
&ENDIF
...
run pi_messages (input "show",
                 input 5009,
                 input substitute ("&1~&2~&3~&4~&5~&6~&7~&8~&9", 
                                    "BAS_CCUSTO_RAZAO","~~EMSUNI", "~~{~&emsuni_version}", "~~1.00")) /*msg_5009*/.
&else
...
def new global shared var v_cod_aplicat_dtsul_corren
    ...
    format "x(3)":U
    no-undo.
&IF "{&emsuni_version}" >= "" AND "{&emsuni_version}" < "5.07" &THEN
def var v_cod_ccusto
    ...
    no-undo.
&ENDIF
&IF "{&emsuni_version}" >= "5.07" AND "{&emsuni_version}" < "9.99" &THEN
def var v_cod_ccusto
    as character
    format "x(20)":U
...
def var v_log_restric_estab
    ...
    no-undo.
def var v_log_sdo_cta_ctbl_ccusto
    as logical
    format "Sim/N∆o"
    initial yes
    view-as toggle-box
...
def new global shared var v_rec_plano_ccusto
    ...
    format ">>>>>>9":U
    no-undo.
def var v_rec_plano_ccusto_aux_1
    as recid
    format ">>>>>>9":U
    no-undo.
def new global shared var v_rec_plano_cta_ctbl
    ...
    format ">>>>>>9":U
    no-undo.
def new global shared var v_rec_sdo_cta_ctbl_ccusto
    as recid
    format ">>>>>>9":U
    initial ?
    no-undo.
def new global shared var v_rec_sdo_cta_ctbl_ccusto_aux
    as recid
    format ">>>>>>9":U
    no-undo.
def new global shared var v_rec_sdo_cta_ctbl_ccusto_ult
    as recid
    format ">>>>>>9":U
    no-undo.
def var v_rec_table
...
def var v_wgh_frame_epc
    ...
    format ">>>>>>9":U
    no-undo.
def var v_cod_altern_cta_ctbl_ant        as character       no-undo. /*local*/
def var v_cod_cenar_ctbl_ant             as character       no-undo. /*local*/
...
def var v_log_atualiz_finalid            as logical         no-undo. /*local*/
def var v_log_atualiz_period             as logical         no-undo. /*local*/
def var v_log_changed_unid_organ         as logical         no-undo. /*local*/
def var v_log_return                     as logical         no-undo. /*local*/
def var v_log_saida                      as logical         no-undo. /*local*/
...
def sub-menu  mi_table
    menu-item mi_faixa             label "Faixa"
    menu-item mi_filtro            label "Fi&ltro"
    RULE
    menu-item mi_plano_contas      label "Pla&no Contas"
    menu-item mi_plano_centros_de_custo label "Plano &Centros de Custo"
    RULE
    menu-item mi_detalhe           label "Detalhe"         accelerator "ALT-D"
    RULE
    menu-item mi_imprime           label "Imprime"
    RULE
    menu-item mi_atualiza          label "Atualiza"
    RULE
    menu-item mi_saida             label "Sa°da".
...
def menu      m_10_ccusto_razao     menubar
...
def query qr_bas_ccusto_razao
    for tt_item_lancto_ctbl_razao
    scrolling.
def query qr_bas_ccusto_razao_1
    for tt_item_lancto_ctbl_razao
    scrolling.
...
def browse br_bas_ccusto_razao query qr_bas_ccusto_razao display 
    ...
    width-chars 14.00
        column-label "DÇbitos"
    tt_item_lancto_ctbl_razao.ttv_val_sdo_ctbl_cr
    width-chars 14.00
        column-label "CrÇditos"
    ...
    with no-box separators single 
         ...
         font 1
         bgcolor 15.
def browse br_bas_ccusto_razao_1 query qr_bas_ccusto_razao_1 display 
    tt_item_lancto_ctbl_razao.tta_dat_lancto_ctbl
    width-chars 10.00
        column-label "Data"
    tt_item_lancto_ctbl_razao.tta_num_lote_ctbl
    ...
    width-chars 20.00
        column-label "v_cod_cta_ctbl_cpart"
    tt_item_lancto_ctbl_razao.ttv_cod_ccusto_cpart
    format "x(20)"
    width-chars 20.00
    tt_item_lancto_ctbl_razao.ttv_val_sdo_ctbl_fim
    width-chars 14.00
...
&endif
    size 1 by 1.
def button bt_plano_ccusto
    label "Altera Plano Centro de Custo"
    tooltip "Altera Plano Centro de Custo"
&if "{&window-system}" <> "TTY" &then
    image-up file "image/im-pcust.bmp"
    image-insensitive file "image/ii-pcust.bmp"
&endif
    size 1 by 1.
...
def frame f_bas_10_ccusto_razao
    rt_001
         at row 14.50 col 02.00
    " Saldos " view-as text
         at row 14.20 col 04.00
    rt_rgf
         at row 01.00 col 01.00 bgcolor 7 
    rt_mold
         at row 02.50 col 02.00
    bt_exi
         at row 01.08 col 80.72 font ?
         help "Sa°da"
    bt_hel1
         at row 01.08 col 84.72 font ?
         help "Ajuda"
    bt_ran2
         at row 01.13 col 02.00 font ?
         help "Faixa"
    bt_fil2
         at row 01.13 col 06.00 font ?
         help "Filtro"
    bt_plano_cta_ctbl
         at row 01.13 col 11.00 font ?
         help "Plano de Contas"
    bt_plano_ccusto
         at row 01.13 col 15.00 font ?
         help "Altera Plano Centro de Custo"
    bt_det1
         at row 01.13 col 20.00 font ?
         help "Detalhe"
    bt_pri
         at row 01.13 col 25.00 font ?
         help "Imprime"
    bt_alter
         at row 01.13 col 30.00 font ?
         help "Alternativa/Cta Ctbl"
    bt_atz
         at row 01.13 col 35.00 font ?
         help "Atualiza"
    cenar_ctbl.cod_cenar_ctbl
         at row 02.71 col 18.29 colon-aligned label "Cen†rio"
         view-as fill-in
         size-chars 9.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_98638
         at row 02.71 col 29.43
    cenar_ctbl.des_cenar_ctbl
         at row 02.71 col 34.00 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    cta_ctbl.cod_altern_cta_ctbl
         at row 03.71 col 18.29 colon-aligned label "Alternativa Conta"
         view-as fill-in
         size-chars 13.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_226185
         at row 03.71 col 33.43
    cta_ctbl.cod_cta_ctbl
         at row 03.71 col 18.29 colon-aligned label "Conta"
         view-as fill-in
         size-chars 21.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_226186
         at row 03.71 col 41.43
    cta_ctbl.des_tit_ctbl
         at row 03.71 col 46.14 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    emscad.ccusto.cod_ccusto
         at row 04.71 col 18.29 colon-aligned label "CCusto"
         view-as fill-in
         size-chars 12.14 by .88
         fgcolor ? bgcolor 15 font 2
    bt_zoo_97588
         at row 04.71 col 32.43
    emscad.ccusto.des_tit_ctbl
         at row 04.71 col 46.14 no-label
         view-as fill-in
         size-chars 41.14 by .88
         fgcolor ? bgcolor 15 font 2
    br_bas_ccusto_razao_1
         at row 05.92 col 02.14
         help "Itens de Lanáamento Cont†bil no Per°odo"
    br_bas_ccusto_razao
         at row 05.92 col 02.14
         help "Itens de Lanáamento Cont†bil no Per°odo"
    v_val_sdo_ctbl_db
         at row 14.75 col 36.00 colon-aligned label "DB"
    ...
    v_val_sdo_ctbl_fim
         ...
         fgcolor ? bgcolor 15 font 2
    v_val_sdo_ctbl_cr
         at row 15.75 col 36.00 colon-aligned label "CR"
         help "Saldo Cont†bil Ö CrÇdito"
         view-as fill-in
         size-chars 19.14 by .88
         fgcolor ? bgcolor 15 font 2
    with 1 down side-labels no-validate keep-tab-order three-d
         size-char 88.14 by 16.88
         at row 01.00 col 01.00
         font 1 fgcolor ? bgcolor 8
         title "Consulta Raz∆o Centro Custo".
    /* adjust size of objects in this frame */
    assign bt_alter:width-chars           in frame f_bas_10_ccusto_razao = 04.00
           bt_alter:height-chars          in frame f_bas_10_ccusto_razao = 01.13
           bt_atz:width-chars             in frame f_bas_10_ccusto_razao = 04.00
           bt_atz:height-chars            in frame f_bas_10_ccusto_razao = 01.13
           bt_det1:width-chars            in frame f_bas_10_ccusto_razao = 04.00
           bt_det1:height-chars           in frame f_bas_10_ccusto_razao = 01.13
           bt_exi:width-chars             in frame f_bas_10_ccusto_razao = 04.00
           bt_exi:height-chars            in frame f_bas_10_ccusto_razao = 01.13
           bt_fil2:width-chars            in frame f_bas_10_ccusto_razao = 04.00
           bt_fil2:height-chars           in frame f_bas_10_ccusto_razao = 01.13
           bt_hel1:width-chars            in frame f_bas_10_ccusto_razao = 04.00
           bt_hel1:height-chars           in frame f_bas_10_ccusto_razao = 01.13
           bt_plano_ccusto:width-chars    in frame f_bas_10_ccusto_razao = 04.00
           bt_plano_ccusto:height-chars   in frame f_bas_10_ccusto_razao = 01.13
           bt_plano_cta_ctbl:width-chars  in frame f_bas_10_ccusto_razao = 04.00
           bt_plano_cta_ctbl:height-chars in frame f_bas_10_ccusto_razao = 01.13
           bt_pri:width-chars             in frame f_bas_10_ccusto_razao = 04.00
           bt_pri:height-chars            in frame f_bas_10_ccusto_razao = 01.13
           bt_ran2:width-chars            in frame f_bas_10_ccusto_razao = 04.00
           bt_ran2:height-chars           in frame f_bas_10_ccusto_razao = 01.13
           rt_001:width-chars             in frame f_bas_10_ccusto_razao = 86.00
           rt_001:height-chars            in frame f_bas_10_ccusto_razao = 02.38
           rt_mold:width-chars            in frame f_bas_10_ccusto_razao = 85.86
           rt_mold:height-chars           in frame f_bas_10_ccusto_razao = 01.21
           rt_rgf:width-chars             in frame f_bas_10_ccusto_razao = 87.86
           rt_rgf:height-chars            in frame f_bas_10_ccusto_razao = 01.29.
&if '{&emsbas_version}' >= '5.06' &then
if OPSYS = 'WIN32':U then do:
assign br_bas_ccusto_razao:ALLOW-COLUMN-SEARCHING in frame f_bas_10_ccusto_razao = no
       br_bas_ccusto_razao:COLUMN-MOVABLE in frame f_bas_10_ccusto_razao = no.
end.
&endif
&if '{&emsbas_version}' >= '5.06' &then
if OPSYS = 'WIN32':U then do:
assign br_bas_ccusto_razao_1:ALLOW-COLUMN-SEARCHING in frame f_bas_10_ccusto_razao = no
       br_bas_ccusto_razao_1:COLUMN-MOVABLE in frame f_bas_10_ccusto_razao = no.
end.
&endif
    /* set private-data for the help system */
    assign bt_exi:private-data                       in frame f_bas_10_ccusto_razao = "HLP=000004665":U
           bt_hel1:private-data                      in frame f_bas_10_ccusto_razao = "HLP=000004666":U
           bt_ran2:private-data                      in frame f_bas_10_ccusto_razao = "HLP=000008773":U
           bt_fil2:private-data                      in frame f_bas_10_ccusto_razao = "HLP=000008766":U
           bt_plano_cta_ctbl:private-data            in frame f_bas_10_ccusto_razao = "HLP=000008762":U
           bt_plano_ccusto:private-data              in frame f_bas_10_ccusto_razao = "HLP=000008761":U
           bt_det1:private-data                      in frame f_bas_10_ccusto_razao = "HLP=000010830":U
           bt_pri:private-data                       in frame f_bas_10_ccusto_razao = "HLP=000010833":U
           bt_alter:private-data                     in frame f_bas_10_ccusto_razao = "HLP=000024223":U
           bt_atz:private-data                       in frame f_bas_10_ccusto_razao = "HLP=000021479":U
           bt_zoo_98638:private-data                 in frame f_bas_10_ccusto_razao = "HLP=000009431":U
           cenar_ctbl.cod_cenar_ctbl:private-data    in frame f_bas_10_ccusto_razao = "HLP=000005310":U
           cenar_ctbl.des_cenar_ctbl:private-data    in frame f_bas_10_ccusto_razao = "HLP=000011446":U
           bt_zoo_226185:private-data                in frame f_bas_10_ccusto_razao = "HLP=000009431":U
           cta_ctbl.cod_altern_cta_ctbl:private-data in frame f_bas_10_ccusto_razao = "HLP=000005338":U
           bt_zoo_226186:private-data                in frame f_bas_10_ccusto_razao = "HLP=000009431":U
           cta_ctbl.cod_cta_ctbl:private-data        in frame f_bas_10_ccusto_razao = "HLP=000011308":U
           cta_ctbl.des_tit_ctbl:private-data        in frame f_bas_10_ccusto_razao = "HLP=000025188":U
           bt_zoo_97588:private-data                 in frame f_bas_10_ccusto_razao = "HLP=000009431":U
           emscad.ccusto.cod_ccusto:private-data            in frame f_bas_10_ccusto_razao = "HLP=000006624":U
           emscad.ccusto.des_tit_ctbl:private-data          in frame f_bas_10_ccusto_razao = "HLP=000025191":U
           br_bas_ccusto_razao_1:private-data        in frame f_bas_10_ccusto_razao = "HLP=000008885":U
           br_bas_ccusto_razao:private-data          in frame f_bas_10_ccusto_razao = "HLP=000008885":U
           v_val_sdo_ctbl_db:private-data            in frame f_bas_10_ccusto_razao = "HLP=000024212":U
           v_val_sdo_ctbl_inic_ant:private-data      in frame f_bas_10_ccusto_razao = "HLP=000024214":U
           v_val_sdo_ctbl_fim:private-data           in frame f_bas_10_ccusto_razao = "HLP=000024215":U
           v_val_sdo_ctbl_cr:private-data            in frame f_bas_10_ccusto_razao = "HLP=000024213":U
           frame f_bas_10_ccusto_razao:private-data                                 = "HLP=000008885".
    /* enable function buttons */
    assign bt_zoo_98638:sensitive  in frame f_bas_10_ccusto_razao = yes
           bt_zoo_226185:sensitive in frame f_bas_10_ccusto_razao = yes
           bt_zoo_226186:sensitive in frame f_bas_10_ccusto_razao = yes
           bt_zoo_97588:sensitive  in frame f_bas_10_ccusto_razao = yes.
    /* move buttons to top */
    bt_zoo_98638:move-to-top().
    bt_zoo_226185:move-to-top().
    bt_zoo_226186:move-to-top().
    bt_zoo_97588:move-to-top().



{include/i_fclfrm.i f_bas_10_ccusto_razao }
/*************************** Frame Definition End ***************************/
&if '{&emsbas_version}' >= '5.06' &then
ON WINDOW-MAXIMIZED OF wh_w_program
DO:
def var v_whd_widget as widget-handle no-undo.
assign frame f_bas_10_ccusto_razao:width-chars  = wh_w_program:width-chars
       frame f_bas_10_ccusto_razao:height-chars = wh_w_program:height-chars no-error.
...
ON DEFAULT-ACTION OF br_bas_ccusto_razao IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    enable br_bas_ccusto_razao
           with frame f_bas_10_ccusto_razao.
END. /* ON DEFAULT-ACTION OF br_bas_ccusto_razao IN FRAME f_bas_10_ccusto_razao */

ON DEFAULT-ACTION OF br_bas_ccusto_razao_1 IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    enable br_bas_ccusto_razao_1
           with frame f_bas_10_ccusto_razao.
END. /* ON DEFAULT-ACTION OF br_bas_ccusto_razao_1 IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_alter IN FRAME f_bas_10_ccusto_razao
DO:

    if  v_log_alter = no
    then do: 
        run pi_v_log_alter_no_ccusto /*pi_v_log_alter_no_ccusto*/.
    end /* if */.
    else do: 
        run pi_v_log_alter_yes_ccusto /*pi_v_log_alter_yes_ccusto*/.
    end /* else */.
END. /* ON CHOOSE OF bt_alter IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_atz IN FRAME f_bas_10_ccusto_razao
DO:

    /* --- Cen†rio e Per°odo Ctbl ---*/
    find first cenar_ctbl no-lock
       where cenar_ctbl.cod_cenar_ctbl = input frame f_bas_10_ccusto_razao cenar_ctbl.cod_cenar_ctbl no-error.
    ...
    then do:
        ...
        apply "entry" to cenar_ctbl.cod_cenar_ctbl in frame f_bas_10_ccusto_razao.
        return no-apply.
    end /* else */.
    ...
    if v_log_alter = no then 
       find cta_ctbl no-lock
            where cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
              and cta_ctbl.cod_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl no-error.
    else 
       find cta_ctbl no-lock 
            where cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl 
              and cta_ctbl.cod_altern_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_altern_cta_ctbl no-error.
    if  avail cta_ctbl and
        cta_ctbl.ind_espec_cta_ctbl = "Anal°tica" /*l_analitica*/ 
    then do:
        assign v_rec_cta_ctbl = recid(cta_ctbl)
               v_cod_cta_ctbl = cta_ctbl.cod_cta_ctbl.
    end /* if */.
    else do:
        if  avail cta_ctbl
        then do:
        ...
        if v_log_alter = no then 
           apply "entry" to cta_ctbl.cod_cta_ctbl in frame f_bas_10_ccusto_razao.
        else
           apply "entry" to cta_ctbl.cod_altern_cta_ctbl in frame f_bas_10_ccusto_razao.
        return no-apply.
    end /* else */.
    ...
    find first emscad.ccusto no-lock
         where ccusto.cod_empresa      = v_cod_unid_organ
           ...
           and ccusto.cod_ccusto       = input frame f_bas_10_ccusto_razao ccusto.cod_ccusto no-error.
    if  not avail ccusto
    then do:
        run pi_busca_ccusto_mgl204.
        if not avail ccusto then do:
            message substitute(getStrTrans("&1 inexistente.", "FGL") /*l_not_found*/ ,getStrTrans("Centro Custo", "FGL"))
                   view-as alert-box warning buttons ok.
            find ccusto where recid(ccusto) = v_rec_ccusto no-lock no-error.
            apply "entry" to ccusto.cod_ccusto in frame f_bas_10_ccusto_razao.
            return no-apply.
        end.
    end /* else */.

    run pi_avail_cenar_ctbl_ccusto /*pi_avail_cenar_ctbl_ccusto*/.
    if  return-value = "NOK" /*l_nok*/  then
        return no-apply.
    run pi_avail_ccusto_bt_ent /*pi_avail_ccusto_bt_ent*/.
    if  return-value = "NOK" /*l_nok*/  then
        return no-apply.
    ...
    if  v_log_atualiz_finalid = yes then do:
        ...
        assign v_log_atualiz_finalid = no.
    end.

    if  v_log_atualiz_period = yes then do:
        find first period_ctbl no-lock
             where period_ctbl.cod_cenar_ctbl = cenar_ctbl.cod_cenar_ctbl
               ...
               and period_ctbl.dat_fim_period_ctbl >= v_dat_inic_period_ctbl_pri /*cl_dat_inic_period_pri of period_ctbl*/ no-error.
        ...
        assign v_log_atualiz_period = no.
    end.

    assign v_rec_sdo_cta_ctbl_ccusto = ?.

    for each tt_estab_unid_negoc_select no-lock:
        find first sdo_cta_ctbl_ccusto 
             where sdo_cta_ctbl_ccusto.cod_empresa        = v_cod_empres_usuar
             and   sdo_cta_ctbl_ccusto.cod_finalid_econ   = v_cod_finalid_econ_ini
             and   sdo_cta_ctbl_ccusto.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
             and   sdo_cta_ctbl_ccusto.cod_cta_ctbl       = cta_ctbl.cod_cta_ctbl
             and   sdo_cta_ctbl_ccusto.cod_cenar_ctbl     = period_ctbl.cod_cenar_ctbl
             ...
             and   sdo_cta_ctbl_ccusto.cod_plano_ccusto   = plano_ccusto.cod_plano_ccusto
             and   sdo_cta_ctbl_ccusto.cod_ccusto         = ccusto.cod_ccusto
             and   sdo_cta_ctbl_ccusto.dat_sdo_ctbl      >= period_ctbl.dat_inic_period_ctbl
             no-lock no-error.
        if  avail sdo_cta_ctbl_ccusto then do:
            assign v_rec_sdo_cta_ctbl_ccusto = recid(sdo_cta_ctbl_ccusto).
            leave.
        end.
    end.

    find sdo_cta_ctbl_ccusto 
       where recid(sdo_cta_ctbl_ccusto) = v_rec_sdo_cta_ctbl_ccusto 
       no-lock no-error.
    if  not avail sdo_cta_ctbl_ccusto then
        assign v_rec_sdo_cta_ctbl_ccusto = ?
               v_rec_cta_ctbl            = ?
               v_rec_ccusto              = ?.
    ...
    if v_log_alter = no then 
       display cenar_ctbl.cod_cenar_ctbl when avail cenar_ctbl
               ...
               cta_ctbl.cod_cta_ctbl when avail cta_ctbl
               ...
               ccusto.cod_ccusto when avail ccusto
               "" when not avail ccusto @ ccusto.cod_ccusto
       ...
       display cenar_ctbl.cod_cenar_ctbl when avail cenar_ctbl
               ...
               cenar_ctbl.des_cenar_ctbl when avail cenar_ctbl
               "" when not avail cenar_ctbl @ cenar_ctbl.des_cenar_ctbl
               cta_ctbl.cod_altern_cta_ctbl when avail cta_ctbl
    ...
    run pi_criar_item_lancto_ctbl_ccusto /*pi_criar_item_lancto_ctbl_ccusto*/.
    run pi_open_bas_ccusto_razao /*pi_open_bas_ccusto_razao*/.

    &if '{&window-system}' <> 'TTY' &then
        if  bt_atz:load-image('image/im-tick.bmp') then .
    &endif

    if  session:set-wait-state('') then .

END. /* ON CHOOSE OF bt_atz IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_det1 IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    then do:
        if  search("prgint/utb/utb066ia.r") = ? and search("prgint/utb/utb066ia.p") = ? then do:
            if  v_cod_dwb_user begins 'es_' then
                return "Programa execut†vel n∆o foi encontrado:" /*l_programa_nao_encontrado*/  + "prgint/utb/utb066ia.p".
            else do:
                message getStrTrans("Programa execut†vel n∆o foi encontrado:", "FGL") /*l_programa_nao_encontrado*/  "prgint/utb/utb066ia.p"
                       view-as alert-box error buttons ok.
                return.
            end.
        end.
...
END. /* ON CHOOSE OF bt_det1 IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_exi IN FRAME f_bas_10_ccusto_razao
DO:

    run pi_close_program /*pi_close_program*/.
END. /* ON CHOOSE OF bt_exi IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_fil2 IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    then do:
        assign v_log_atualiz_finalid = yes.

        &if '{&window-system}' <> 'TTY' &then
            if  bt_atz:load-image('image/ii-tick2.bmp') then .
        &endif
    end /* if */.
END. /* ON CHOOSE OF bt_fil2 IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_hel1 IN FRAME f_bas_10_ccusto_razao
DO:
...
END. /* ON CHOOSE OF bt_hel1 IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_plano_ccusto IN FRAME f_bas_10_ccusto_razao
DO:

    /************************* Variable Definition Begin ************************/

    def var v_rec_plano_ccusto_aux           as recid           no-undo. /*local*/


    /************************** Variable Definition End *************************/

    assign v_rec_plano_ccusto_aux = v_rec_plano_ccusto.
    if  search("prgint/utb/utb083ka.r") = ? and search("prgint/utb/utb083ka.p") = ? then do:
        if  v_cod_dwb_user begins 'es_' then
            ...
            message getStrTrans("Programa execut†vel n∆o foi encontrado:", "FGL") /*l_programa_nao_encontrado*/  "prgint/utb/utb083ka.p"
                   view-as alert-box error buttons ok.
            return.
        end.
    end.
    ...
    if  (v_rec_plano_ccusto <> ?) and
        (v_rec_plano_ccusto <> v_rec_plano_ccusto_aux)
    then do:
        ...
        then do:
            assign v_rec_plano_ccusto = v_rec_plano_ccusto_aux.
            find plano_ccusto where recid(plano_ccusto) = v_rec_plano_ccusto no-lock no-error.
            return no-apply.
        end /* if */.
        ...
        then do:
            ...
            assign emscad.ccusto.cod_ccusto:screen-value in frame f_bas_10_ccusto_razao = ?
                   ccusto.cod_ccusto:format in frame f_bas_10_ccusto_razao = plano_ccusto.cod_format_ccusto
                   ccusto.cod_ccusto:screen-value in frame f_bas_10_ccusto_razao = v_cod_ccusto_000.
        end /* if */.
        assign v_rec_sdo_cta_ctbl_ccusto = ?
...
END. /* ON CHOOSE OF bt_plano_ccusto IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_plano_cta_ctbl IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    then do:
        ...
        then do:
            ...
            run pi_retornar_inic_zero (Input cta_ctbl.cod_cta_ctbl:handle in frame f_bas_10_ccusto_razao,
...
END. /* ON CHOOSE OF bt_plano_cta_ctbl IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_pri IN FRAME f_bas_10_ccusto_razao
DO:

    if  search("prgtec/btb/btb901zc.r") = ? and search("prgtec/btb/btb901zc.p") = ? then do:
        if  v_cod_dwb_user begins 'es_' then
            return "Programa execut†vel n∆o foi encontrado:" /*l_programa_nao_encontrado*/  + "prgtec/btb/btb901zc.p".
        else do:
            message getStrTrans("Programa execut†vel n∆o foi encontrado:", "FGL") /*l_programa_nao_encontrado*/  "prgtec/btb/btb901zc.p"
                   view-as alert-box error buttons ok.
            return.
        end.
    end.
    else
        run prgtec/btb/btb901zc.p (Input 'bas_ccusto_razao') /*prg_fnc_reports*/.
END. /* ON CHOOSE OF bt_pri IN FRAME f_bas_10_ccusto_razao */

ON CHOOSE OF bt_ran2 IN FRAME f_bas_10_ccusto_razao
DO:
    ...
    then do:
        assign v_log_atualiz_period = yes.
...
END. /* ON CHOOSE OF bt_ran2 IN FRAME f_bas_10_ccusto_razao */

ON LEAVE OF ccusto.cod_ccusto IN FRAME f_bas_10_ccusto_razao
DO:

    assign v_cod_format_1 = input frame f_bas_10_ccusto_razao emscad.ccusto.cod_ccusto no-error.
    ...
    then do:
       ...
       run pi_messages (input "show",
                        input 4488,
                        input substitute ("&1~~&2~~&3~~&4~~&5~~&6~~&7~~&8~~&9",
                                          v_cod_format_ccusto,"" /*l_null*/)) /*msg_4488*/.
       return no-apply.
    end /* if */.
    find ccusto no-lock
         where ccusto.cod_empresa = v_cod_empres_usuar
           and ccusto.cod_plano_ccusto = plano_ccusto.cod_plano_ccusto
           and ccusto.cod_ccusto = input frame f_bas_10_ccusto_razao ccusto.cod_ccusto /*cl_bas_ccusto_razao of ccusto*/ no-error.
    display ccusto.des_tit_ctbl when avail ccusto
            "" when not avail ccusto @ ccusto.des_tit_ctbl
            with frame f_bas_10_ccusto_razao.

    if  v_cod_ccusto_ant <> input frame f_bas_10_ccusto_razao ccusto.cod_ccusto then do:
        &if '{&window-system}' <> 'TTY' &then
            if  bt_atz:load-image('image/ii-tick2.bmp') then .
        &endif
        assign v_cod_ccusto_ant = input frame f_bas_10_ccusto_razao ccusto.cod_ccusto.
    end.


END. /* ON LEAVE OF ccusto.cod_ccusto IN FRAME f_bas_10_ccusto_razao */

ON LEAVE OF cenar_ctbl.cod_cenar_ctbl IN FRAME f_bas_10_ccusto_razao
DO:

    find first cenar_ctbl no-lock
         where cenar_ctbl.cod_cenar_ctbl = input frame f_bas_10_ccusto_razao cenar_ctbl.cod_cenar_ctbl /*cl_bas_ccusto_razao of cenar_ctbl*/ no-error.
    display cenar_ctbl.des_cenar_ctbl when avail cenar_ctbl
            "" when not avail cenar_ctbl @ cenar_ctbl.des_cenar_ctbl
            with frame f_bas_10_ccusto_razao.

    if  v_cod_cenar_ctbl_ant <> input frame f_bas_10_ccusto_razao cenar_ctbl.cod_cenar_ctbl then do:
        &if '{&window-system}' <> 'TTY' &then
            if  bt_atz:load-image('image/ii-tick2.bmp') then .
        &endif
        assign v_cod_cenar_ctbl_ant = input frame f_bas_10_ccusto_razao cenar_ctbl.cod_cenar_ctbl.
    end.

END. /* ON LEAVE OF cenar_ctbl.cod_cenar_ctbl IN FRAME f_bas_10_ccusto_razao */

ON LEAVE OF cta_ctbl.cod_altern_cta_ctbl IN FRAME f_bas_10_ccusto_razao
DO:

    find first cta_ctbl no-lock
         where cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
           and cta_ctbl.cod_altern_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_altern_cta_ctbl
         use-index ctactbl_altern no-error.
    display cta_ctbl.des_tit_ctbl when avail cta_ctbl
            "" when not avail cta_ctbl @ cta_ctbl.des_tit_ctbl
            with frame f_bas_10_ccusto_razao.

    if  v_cod_altern_cta_ctbl_ant <> input frame f_bas_10_ccusto_razao cta_ctbl.cod_altern_cta_ctbl then do:
        &if '{&window-system}' <> 'TTY' &then
            if  bt_atz:load-image('image/ii-tick2.bmp') then .
        &endif
        assign v_cod_altern_cta_ctbl_ant = input frame f_bas_10_ccusto_razao cta_ctbl.cod_altern_cta_ctbl.
    end.
END. /* ON LEAVE OF cta_ctbl.cod_altern_cta_ctbl IN FRAME f_bas_10_ccusto_razao */

ON LEAVE OF cta_ctbl.cod_cta_ctbl IN FRAME f_bas_10_ccusto_razao
DO:

    if  v_cod_format_Cta_Ctbl = "" 
    and avail plano_cta_ctbl then
        assign v_cod_format_cta_ctbl = plano_cta_ctbl.cod_format_cta_ctbl.

    assign v_cod_format          = input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl
           v_cod_format_Cta_Ctbl = plano_cta_ctbl.cod_format_cta_ctbl no-error.
    if  error-status:error 
    or (v_cod_format_cta_ctbl <> ""
    and index(string(v_cod_format, v_cod_format_Cta_Ctbl), chr(32)) <> 0)
    then do:
       /* Formato &2 Inv†lido ! */
       run pi_messages (input "show",
                        input 4488,
                        input substitute ("&1~~&2~~&3~~&4~~&5~~&6~~&7~~&8~~&9",
                                          v_cod_format_cta_ctbl,"" /*l_null*/)) /*msg_4488*/.
       return no-apply.
    end.
    find first cta_ctbl no-lock
         where cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
    ...
    if  v_cod_cta_ctbl_ant <> input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl then do:
        &if '{&window-system}' <> 'TTY' &then
            if  bt_atz:load-image('image/ii-tick2.bmp') then .
        &endif
        assign v_cod_cta_ctbl_ant = input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl.
    end.


END. /* ON LEAVE OF cta_ctbl.cod_cta_ctbl IN FRAME f_bas_10_ccusto_razao */
...
ON  CHOOSE OF bt_zoo_97588 IN FRAME f_bas_10_ccusto_razao
OR F5 OF ccusto.cod_ccusto IN FRAME f_bas_10_ccusto_razao DO:
    ...
    if  search("prgint/utb/utb066ka.r") = ? and search("prgint/utb/utb066ka.p") = ? then do:
        if  v_cod_dwb_user begins 'es_' then
            ...
            message getStrTrans("Programa execut†vel n∆o foi encontrado:", "FGL") /*l_programa_nao_encontrado*/  "prgint/utb/utb066ka.p"
                   view-as alert-box error buttons ok.
            return.
        end.
    end.
    ...
    then do:
        ...
        assign ccusto.cod_ccusto:screen-value in frame f_bas_10_ccusto_razao =
    ...
    apply "entry" to ccusto.cod_ccusto in frame f_bas_10_ccusto_razao.
end. /* ON  CHOOSE OF bt_zoo_97588 IN FRAME f_bas_10_ccusto_razao */

ON  CHOOSE OF bt_zoo_98638 IN FRAME f_bas_10_ccusto_razao
OR F5 OF cenar_ctbl.cod_cenar_ctbl IN FRAME f_bas_10_ccusto_razao DO:
    ...
    then do:
        find cenar_ctbl where recid(cenar_ctbl) = v_rec_cenar_ctbl no-lock no-error.
        assign cenar_ctbl.cod_cenar_ctbl:screen-value in frame f_bas_10_ccusto_razao =
                   string(cenar_ctbl.cod_cenar_ctbl)
               cenar_ctbl.des_cenar_ctbl:screen-value in frame f_bas_10_ccusto_razao =
                   cenar_ctbl.des_cenar_ctbl.
        apply "entry" to cenar_ctbl.cod_cenar_ctbl in frame f_bas_10_ccusto_razao.
    end /* if */.

end. /* ON  CHOOSE OF bt_zoo_98638 IN FRAME f_bas_10_ccusto_razao */

ON  CHOOSE OF bt_zoo_226185 IN FRAME f_bas_10_ccusto_razao
OR F5 OF cta_ctbl.cod_altern_cta_ctbl IN FRAME f_bas_10_ccusto_razao DO:
    ...
    then do:
        find cta_ctbl where recid(cta_ctbl) = v_rec_cta_ctbl no-lock no-error.
        assign cta_ctbl.cod_altern_cta_ctbl:screen-value in frame f_bas_10_ccusto_razao =
                   string(cta_ctbl.cod_altern_cta_ctbl)
               cta_ctbl.des_tit_ctbl:screen-value in frame f_bas_10_ccusto_razao =
                   cta_ctbl.des_tit_ctbl.
        apply "entry" to cta_ctbl.cod_altern_cta_ctbl in frame f_bas_10_ccusto_razao.
    end /* if */.

end. /* ON  CHOOSE OF bt_zoo_226185 IN FRAME f_bas_10_ccusto_razao */

ON  CHOOSE OF bt_zoo_226186 IN FRAME f_bas_10_ccusto_razao
OR F5 OF cta_ctbl.cod_cta_ctbl IN FRAME f_bas_10_ccusto_razao DO:
    ...
    if  search("prgint/utb/utb080nc.r") = ? and search("prgint/utb/utb080nc.p") = ? then do:
        if  v_cod_dwb_user begins 'es_' then
            ...
            message getStrTrans("Programa execut†vel n∆o foi encontrado:", "FGL") /*l_programa_nao_encontrado*/  "prgint/utb/utb080nc.p"
                   view-as alert-box error buttons ok.
            return.
        end.
    end.
    ...
    then do:
        find cta_ctbl where recid(cta_ctbl) = v_rec_cta_ctbl no-lock no-error.
        assign cta_ctbl.cod_cta_ctbl:screen-value in frame f_bas_10_ccusto_razao =
...
end. /* ON  CHOOSE OF bt_zoo_226186 IN FRAME f_bas_10_ccusto_razao */
...
ON ENDKEY OF FRAME f_bas_10_ccusto_razao
DO:
    ...
    do:
        assign v_rec_table_epc = recid(emscad.ccusto).    
    ...
    do:
        assign v_rec_table_epc = recid(ccusto).    
...
END. /* ON ENDKEY OF FRAME f_bas_10_ccusto_razao */

ON END-ERROR OF FRAME f_bas_10_ccusto_razao
DO:

    run pi_close_program /*pi_close_program*/.
END. /* ON END-ERROR OF FRAME f_bas_10_ccusto_razao */

ON HELP OF FRAME f_bas_10_ccusto_razao ANYWHERE
DO:
...
END. /* ON HELP OF FRAME f_bas_10_ccusto_razao */

ON RIGHT-MOUSE-DOWN OF FRAME f_bas_10_ccusto_razao ANYWHERE
DO:
...
END. /* ON RIGHT-MOUSE-DOWN OF FRAME f_bas_10_ccusto_razao */

ON RIGHT-MOUSE-UP OF FRAME f_bas_10_ccusto_razao ANYWHERE
DO:
...
END. /* ON RIGHT-MOUSE-UP OF FRAME f_bas_10_ccusto_razao */
...
&if '{&emsbas_version}' >= '5.06' &then
    ...
    if not avail tt_maximizacao then do:
        assign v_whd_field_group = frame f_bas_10_ccusto_razao:first-child.
        repeat while valid-handle(v_whd_field_group):
            ...
            repeat while valid-handle(v_whd_widget):
                ...
                assign tt_maximizacao.frame-width-original   = frame f_bas_10_ccusto_razao:width.
                assign tt_maximizacao.frame-height-original  = frame f_bas_10_ccusto_razao:height.
...
DO:
    ...
    apply "choose" to bt_exi in frame f_bas_10_ccusto_razao.
...
ON CHOOSE OF MENU-ITEM mi_faixa IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_ran2 in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_faixa IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_filtro IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_fil2 in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_filtro IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_plano_centros_de_custo IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_plano_ccusto in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_plano_centros_de_custo IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_detalhe IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_det1 in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_detalhe IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_imprime IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_pri in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_imprime IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_atualiza IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_atz in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_atualiza IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_saida IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_exi in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_saida IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_contents IN MENU m_10_ccusto_razao
DO:

    apply "choose" to bt_hel1 in frame f_bas_10_ccusto_razao.
END. /* ON CHOOSE OF MENU-ITEM mi_contents IN MENU m_10_ccusto_razao */

ON CHOOSE OF MENU-ITEM mi_about IN MENU m_10_ccusto_razao
DO:
    ...
    assign v_nom_prog     = substring(current-window:title, 1, max(1, length(current-window:title) - 10))
                          + chr(10)
                          + "bas_ccusto_razao":U
           v_nom_prog_ext = "prgfin/fgl/fgl209aa.p":U
           v_cod_release  = trim(" 1.00.00.047":U).
...
END. /* ON CHOOSE OF MENU-ITEM mi_about IN MENU m_10_ccusto_razao */
...
{include/i-ctrlrp5.i bas_ccusto_razao}
...
then do:
    run pi_version_extract ('bas_ccusto_razao':U, 'prgfin/fgl/fgl209aa.p':U, '1.00.00.047':U, 'pro':U).
end /* if */.
...
if  search("prgtec/men/men901za.r") = ? and search("prgtec/men/men901za.py") = ? then do:
    ...
    run prgtec/men/men901za.py (Input 'bas_ccusto_razao') /*prg_fnc_verify_security*/.
...
if can-find(prog_dtsul
       where prog_dtsul.cod_prog_dtsul = 'bas_ccusto_razao' 
         and prog_dtsul.log_gera_log_exec = yes) then do transaction:
    create log_exec_prog_dtsul.
    assign log_exec_prog_dtsul.cod_prog_dtsul           = 'bas_ccusto_razao'
...
find prog_dtsul
    where prog_dtsul.cod_prog_dtsul = "bas_ccusto_razao":U
    no-lock no-error.
...
assign v_wgh_frame_epc = frame f_bas_10_ccusto_razao:handle.



assign v_nom_table_epc = 'ccusto':U
       v_rec_table_epc = recid(ccusto).

&endif

/* End_Include: i_verify_program_epc */
OUTPUT TO c:\desenv\tt-unid-neg.txt.
FOR EACH tt_unid_negoc:
  EXPORT tt_unid_negoc.

END.
OUTPUT CLOSE.

OUTPUT TO c:\desenv\tt-unid-org.txt.
FOR EACH tt_unid_organ:
  EXPORT tt_unid_organ.

END.
OUTPUT CLOSE.

/* Begin_Include: ix_p00_bas_ccusto_razao */
...
hide cta_ctbl.cod_altern_cta_ctbl in frame f_bas_10_ccusto_razao
     bt_zoo_226185 in frame f_bas_10_ccusto_razao.
...
then do:
    ...
    MESSAGE "entrei" VIEW-AS ALERT-BOX.
...
if Verifica_Program_Name('mgl204aa':U,30) then do:
    find ccusto where recid(ccusto) = v_rec_ccusto no-lock no-error.
    find plano_ccusto where recid(plano_ccusto) = v_rec_plano_ccusto no-lock no-error.
    assign v_rec_plano_ccusto_aux_1 = recid(plano_ccusto)
           v_rec_ccusto_aux       = recid(ccusto).
end.
...
if Verifica_Program_Name('mgl204aa':U,30) then do:
    if v_rec_plano_ccusto_aux_1 <> v_rec_plano_ccusto then
        assign v_rec_plano_ccusto = v_rec_plano_ccusto_aux_1.
end.

/* Plano Centros de Custo */
find plano_ccusto where recid(plano_ccusto) = v_rec_plano_ccusto no-lock no-error.
if  avail plano_ccusto
then do:
...
if v_rec_ccusto <> ? then
    find first ccusto no-lock
        where recid(ccusto) = v_rec_ccusto no-error.

if not avail ccusto then do:
    find first tt_ccusto no-lock
         where tt_ccusto.cod_empresa = plano_ccusto.cod_empresa
           and tt_ccusto.cod_plano_ccusto = plano_ccusto.cod_plano_ccusto
          no-error.
    if  not avail tt_ccusto
    then do:
        find first tt_ccusto no-lock no-error.
        find first plano_ccusto no-lock
             where plano_ccusto.cod_empresa = tt_ccusto.cod_empresa
               and plano_ccusto.cod_plano_ccusto = tt_ccusto.cod_plano_ccusto
              no-error.
        assign v_rec_plano_ccusto = recid(plano_ccusto).
    end /* if */.
end.
/* Mostrar CCusto no formato do Plano */
if  avail plano_ccusto
then do:
    run pi_retornar_inic_zero (Input ccusto.cod_ccusto:handle in frame f_bas_10_ccusto_razao,
                               Input plano_ccusto.cod_format_ccusto) /*pi_retornar_inic_zero*/.
end /* if */.

assign v_log_atualiz_finalid = yes
       v_log_atualiz_period  = no.

/* Conta Cont†bil e Centro de Custo */
if  v_rec_cta_ctbl <> ? 
and v_rec_ccusto   <> ? then do:
    find cta_ctbl where recid(cta_ctbl) = v_rec_cta_ctbl no-lock no-error.
    if  avail cta_ctbl 
    and cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl 
    and cta_ctbl.ind_espec_cta_ctbl = "Anal°tica" /*l_analitica*/ 
    then do:
        if not avail ccusto then do:
            find first tt_ccusto no-lock
                 where tt_ccusto.ttv_rec_ccusto = v_rec_ccusto /*cl_rec_ccusto of tt_ccusto*/ no-error.
            if  not avail tt_ccusto then
                assign v_rec_sdo_cta_ctbl_ccusto = ?.
            else do:
                assign ccusto.cod_ccusto:screen-value in frame f_bas_10_ccusto_razao = tt_ccusto.cod_ccusto
                       v_rec_ccusto = tt_ccusto.ttv_rec_ccusto.
                find ccusto where recid(ccusto) = v_rec_ccusto no-lock no-error.
            end.
        end.
        else
            assign ccusto.cod_ccusto:screen-value in frame f_bas_10_ccusto_razao = ccusto.cod_ccusto
...
then do:
    assign tt_item_lancto_ctbl_razao.ttv_cod_cta_ctbl_cpart:label in browse br_bas_ccusto_razao_1 = getStrTrans("Contra Partida", "FGL") /*l_contra_partida*/
           tt_item_lancto_ctbl_razao.ttv_cod_ccusto_cpart:label in browse br_bas_ccusto_razao_1 = getStrTrans("CCusto CPar", "FGL") /*l_ccusto_cpar*/ .
    assign br_bas_ccusto_razao:visible in frame f_bas_10_ccusto_razao = no.
end.
else do:
    assign br_bas_ccusto_razao_1:visible in frame f_bas_10_ccusto_razao = no.
end.

/* End_Include: i_declara_GetDefinedFunction */

/* redefiniá‰es do menu */
assign sub-menu  mi_table:label in menu m_10_ccusto_razao = "Arquivo" /*l_file*/ .

/* redefiniá‰es da window e frame */

/* Begin_Include: i_std_window */
/* tratamento do t°tulo, menu, vers∆o e dimens‰es */
assign wh_w_program:title         = frame f_bas_10_ccusto_razao:title
                                  + chr(32)
                                  + chr(40)
                                  + trim(" 1.00.00.047":U)
                                  + chr(41)
       frame f_bas_10_ccusto_razao:title       = ?
       wh_w_program:width-chars   = frame f_bas_10_ccusto_razao:width-chars
       wh_w_program:height-chars  = frame f_bas_10_ccusto_razao:height-chars - 0.85
       frame f_bas_10_ccusto_razao:row         = 1
       frame f_bas_10_ccusto_razao:col         = 1
       wh_w_program:menubar       = menu m_10_ccusto_razao:handle
...
run pi_frame_settings (Input frame f_bas_10_ccusto_razao:handle) /*pi_frame_settings*/.

/* ix_p02_bas_ccusto_razao */

pause 0 before-hide.

view frame f_bas_10_ccusto_razao.
...
/* Begin_Include: ix_p05_bas_ccusto_razao */
assign ccusto.cod_ccusto:width in frame f_bas_10_ccusto_razao = 21.14
       bt_zoo_97588:column     in frame f_bas_10_ccusto_razao = bt_zoo_97588:column in frame f_bas_10_ccusto_razao + 9.00.

if  v_log_funcao_lista_cta_cpart then
    assign br_bas_ccusto_razao_1:num-locked-columns in frame f_bas_10_ccusto_razao = 1.
else
    assign br_bas_ccusto_razao:num-locked-columns in frame f_bas_10_ccusto_razao = 1.
...
if  avail cta_ctbl then 
    disp  cta_ctbl.cod_cta_ctbl with frame f_bas_10_ccusto_razao.

/* End_Include: ix_p05_bas_ccusto_razao */
...
do on endkey undo main_block, leave main_block on error undo main_block, leave main_block:
    ...
    /* Begin_Include: ix_p10_bas_ccusto_razao */
    if  v_log_plano_cta_ctbl_uni = yes
    then do:
        disable bt_plano_cta_ctbl
                with frame f_bas_10_ccusto_razao.
        assign menu-item mi_plano_contas:sensitive in menu m_10_ccusto_razao = no.
    end /* if */.

    if  v_log_plano_ccusto_uni = yes
    then do:
        disable bt_plano_ccusto
                with frame f_bas_10_ccusto_razao.
        assign menu-item mi_plano_centros_de_custo:sensitive in menu m_10_ccusto_razao = no.
    end /* if */.
    ...
    plano_cta_ctbl:
    for
        each tt_plano_cta_ctbl no-lock
        where tt_plano_cta_ctbl.ind_tip_plano_cta_ctbl = "Prim†rio" /*cl_primario of tt_plano_cta_ctbl*/:
        assign v_des_plano_cta_ctbl = v_des_plano_cta_ctbl + "," + tt_plano_cta_ctbl.cod_plano_cta_ctbl.
    end /* for plano_cta_ctbl */.

    if  num-entries(v_des_plano_cta_ctbl) = 2
    then do:
        disable bt_plano_cta_ctbl
                with frame f_bas_10_ccusto_razao.
    end /* if */.
    else do:
        enable bt_plano_cta_ctbl
               with frame f_bas_10_ccusto_razao.
    end /* else */.

    assign v_cod_cta_ctbl_ant        = cta_ctbl.cod_cta_ctbl:screen-value
           v_cod_cenar_ctbl_ant      = cenar_ctbl.cod_cenar_ctbl:screen-value
           v_cod_ccusto_ant          = ccusto.cod_ccusto:screen-value
           v_cod_altern_cta_ctbl_ant = cta_ctbl.cod_altern_cta_ctbl:screen-value.

    if  cta_ctbl.cod_cta_ctbl:sensitive = yes 
    and cta_ctbl.cod_cta_ctbl:visible   = yes then do:
        find first cta_ctbl no-lock
             where cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
               and cta_ctbl.cod_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl no-error.
        display cta_ctbl.des_tit_ctbl when avail cta_ctbl
    ...
    if  cenar_ctbl.cod_cenar_ctbl:sensitive = yes then
        apply "leave" to cenar_ctbl.cod_cenar_ctbl.

    if  ccusto.cod_ccusto:sensitive = yes then do:
        find ccusto no-lock
             where ccusto.cod_empresa = v_cod_empres_usuar
               and ccusto.cod_plano_ccusto = plano_ccusto.cod_plano_ccusto
               and ccusto.cod_ccusto = input frame f_bas_10_ccusto_razao ccusto.cod_ccusto /*cl_bas_ccusto_razao of ccusto*/ no-error.
    ...
    if  cta_ctbl.cod_altern_cta_ctbl:sensitive = yes 
    and cta_ctbl.cod_altern_cta_ctbl:visible   = yes then
        apply "leave" to cta_ctbl.cod_altern_cta_ctbl.
    ...
    OR  PROGRAM-NAME(2) BEGINS "pi_razao_contabil_demonst":U THEN
        apply "choose" to bt_atz in frame f_bas_10_ccusto_razao.
    &endif
    /* End_Include: ix_p10_bas_ccusto_razao */

    if  this-procedure:persistent = no
    then do:
        wait-for choose of bt_exi in frame f_bas_10_ccusto_razao.
    end /* if */.
end /* do main_block */.
...
/*****************************************************************************
** Procedure Interna.....: pi_open_bas_ccusto_razao
** Descricao.............: pi_open_bas_ccusto_razao
** Criado por............: Henke
** Criado em.............: 29/12/1995 15:49:25
** Alterado por..........: fut31947
** Alterado em...........: 28/06/2007 09:51:21
*****************************************************************************/
PROCEDURE pi_open_bas_ccusto_razao:
       ...
       if avail period_ctbl then do:
           ...
           if not v_log_busca_sdo and day(v_dat_inic_period_ctbl_pri) <> 01 then do:
              find first b_item_lancto_ctbl
                   ...
                   and   b_item_lancto_ctbl.cod_ccusto         = ccusto.cod_ccusto
                   and   b_item_lancto_ctbl.cod_plano_ccusto   = ccusto.cod_plano_ccusto
        ...
        if v_log_busca_sdo then do:
           run prgfin/fgl/fgl905za.py (Input 1,
                                       ...
                                       Input "",
                                       Input "",
                                       Input ccusto.cod_plano_ccusto,
                                       Input ccusto.cod_ccusto,
    ...
    if  v_log_funcao_lista_cta_cpart
    then do:
        close query qr_bas_ccusto_razao_1.
        open query qr_bas_ccusto_razao_1 for
        ...
        close query qr_bas_ccusto_razao.
        open query qr_bas_ccusto_razao for
            each tt_item_lancto_ctbl_razao no-lock.

        enable br_bas_ccusto_razao
               with frame f_bas_10_ccusto_razao.
    end.
    if  not avail tt_item_lancto_ctbl_razao
    then do:
        assign v_rec_sdo_cta_ctbl_ccusto = ?.
    end /* if */.
    if  v_log_funcao_lista_cta_cpart then
        apply "value-changed" to br_bas_ccusto_razao_1 in frame f_bas_10_ccusto_razao.
    else
        apply "value-changed" to br_bas_ccusto_razao in frame f_bas_10_ccusto_razao.
    display v_val_sdo_ctbl_cr
            v_val_sdo_ctbl_db
            v_val_sdo_ctbl_fim
            v_val_sdo_ctbl_inic_ant
            with frame f_bas_10_ccusto_razao.
END PROCEDURE. /* pi_open_bas_ccusto_razao */
/*****************************************************************************
    ...
    then do:
        ...
        then do:
            ...
            then do:
                ...
                if Verifica_Program_Name('mgl204aa':U,30) then do:
                    find plano_ccusto where recid(plano_ccusto) = v_rec_plano_ccusto_aux_1 no-lock no-error.
                    run pi_criar_ccusto_usuar (Input v_cod_usuar_corren) /* pi_criar_ccusto_usuar*/.
                end.        
            ...
            then do:
                assign v_rec_plano_ccusto = recid(plano_ccusto).
                if not Verifica_Program_Name('fgl208aa':U,1) or (Verifica_Program_Name('fgl208aa':U,1)
...
PROCEDURE pi_criar_ccusto_usuar:
    ...
    if Verifica_Program_Name('mgl204aa':U,30) then do:
        find first ccusto no-lock
            where recid(ccusto) = v_rec_ccusto_aux no-error.
        if avail ccusto then do:
            run pi_verifica_segur_ccusto (buffer ccusto,
                                          output v_log_return) /* pi_verifica_segur_ccusto*/.
            if  v_log_return then do:
                find first tt_ccusto no-lock 
                     where tt_ccusto.cod_empresa      = ccusto.cod_empresa     
                       and tt_ccusto.cod_plano_ccusto = ccusto.cod_plano_ccusto
                       and tt_ccusto.cod_ccusto       = ccusto.cod_ccusto no-error.
...
/*****************************************************************************
** Procedure Interna.....: pi_criar_item_lancto_ctbl_ccusto
** Descricao.............: pi_criar_item_lancto_ctbl_ccusto
** Criado por............: Henke
** Criado em.............: 04/02/1997 13:58:59
** Alterado por..........: fut43120
** Alterado em...........: 28/09/2009 14:06:44
*****************************************************************************/
PROCEDURE pi_criar_item_lancto_ctbl_ccusto:
    ...
    for
        ...
        item_lancto_ctbl:
        for
            each item_lancto_ctbl 
            fields (cod_empresa cod_estab cod_unid_negoc cod_plano_cta_ctbl
                    cod_cta_ctbl cod_plano_ccusto cod_ccusto cod_cenar_ctbl
                    cod_indic_econ dat_lancto_ctbl des_histor_lancto_ctbl
                    ind_sit_lancto_ctbl ind_natur_lancto_ctbl num_lote_ctbl
                    num_lancto_ctbl num_seq_lancto_ctbl num_seq_lancto_ctbl_cpart
                    val_lancto_ctbl log_lancto_apurac_restdo)
            no-lock
            ...
            and   item_lancto_ctbl.cod_plano_ccusto = emscad.ccusto.cod_plano_ccusto
            and   item_lancto_ctbl.cod_ccusto          = ccusto.cod_ccusto
            ...
            /* Verifica Word-index */
            if  v_des_histor <> "" 
            and index(item_lancto_ctbl.des_histor_lancto_ctbl, v_des_histor) = 0 then
                next item_lancto_ctbl.
            ...
            then do:
                ...
                then do:
                    if  item_lancto_ctbl.ind_natur_lancto_ctbl = "DB" /*l_db*/  then
                        ...
                        assign tt_item_lancto_ctbl_razao.ttv_val_sdo_ctbl_cr = fnAjustDec(aprop_lancto_ctbl.val_lancto_ctbl
                                                                             / v_val_cotac_indic_econ,v_cod_moed_finalid).
                end /* if */.
            end /* if */.
            else do:
                if  item_lancto_ctbl.ind_natur_lancto_ctbl = "DB" /*l_db*/  then
                    ...
                    assign tt_item_lancto_ctbl_razao.ttv_val_sdo_ctbl_cr = fnAjustDec(item_lancto_ctbl.val_lancto_ctbl
                                                                         / v_val_cotac_indic_econ,v_cod_moed_finalid).
            end /* else */.
            ...
            then do:
                ...
                if  avail b_item_lancto_ctbl then
                    assign tt_item_lancto_ctbl_razao.ttv_cod_cta_ctbl_cpart = string(b_item_lancto_ctbl.cod_cta_ctbl,plano_cta_ctbl.cod_format_cta_ctbl)
                           tt_item_lancto_ctbl_razao.ttv_cod_ccusto_cpart   = string(b_item_lancto_ctbl.cod_ccusto,plano_ccusto.cod_format_ccusto).
            end.
        end /* for item_lancto_ctbl */.
    end /* for estab_un_block */.
END PROCEDURE. /* pi_criar_item_lancto_ctbl_ccusto */
/*****************************************************************************
...
/*****************************************************************************
** Procedure Interna.....: pi_v_log_alter_no_ccusto
** Descricao.............: pi_v_log_alter_no_ccusto
** Criado por............: Julio
** Criado em.............: 19/03/1999 14:46:55
** Alterado por..........: Julio
** Alterado em...........: 19/03/1999 14:53:12
*****************************************************************************/
PROCEDURE pi_v_log_alter_no_ccusto:

    find cta_ctbl where 
         cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl and 
         cta_ctbl.cod_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_cta_ctbl no-lock no-error.
    hide cta_ctbl.cod_cta_ctbl in frame f_bas_10_ccusto_razao
         bt_zoo_226186 in frame f_bas_10_ccusto_razao.
    view cta_ctbl.cod_altern_cta_ctbl in frame f_bas_10_ccusto_razao
         bt_zoo_226185 in frame f_bas_10_ccusto_razao.
    enable cta_ctbl.cod_altern_cta_ctbl
           with frame f_bas_10_ccusto_razao.
    assign cta_ctbl.des_tit_ctbl:column in frame f_bas_10_ccusto_razao = 38.14
           v_log_alter = yes.
    if avail cta_ctbl then 
       assign cta_ctbl.cod_altern_cta_ctbl:screen-value in frame 
           f_bas_10_ccusto_razao = string(cta_ctbl.cod_altern_cta_ctbl).

END PROCEDURE. /* pi_v_log_alter_no_ccusto */
/*****************************************************************************
** Procedure Interna.....: pi_v_log_alter_yes_ccusto
** Descricao.............: pi_v_log_alter_yes_ccusto
** Criado por............: Julio
** Criado em.............: 19/03/1999 14:47:17
** Alterado por..........: Julio
** Alterado em...........: 19/03/1999 14:53:41
*****************************************************************************/
PROCEDURE pi_v_log_alter_yes_ccusto:

    find cta_ctbl where cta_ctbl.cod_plano_cta_ctbl  = plano_cta_ctbl.cod_plano_cta_ctbl and
         cta_ctbl.cod_altern_cta_ctbl = input frame f_bas_10_ccusto_razao cta_ctbl.cod_altern_cta_ctbl no-lock no-error.
    hide cta_ctbl.cod_altern_cta_ctbl in frame f_bas_10_ccusto_razao
         bt_zoo_226185 in frame f_bas_10_ccusto_razao.
    view cta_ctbl.cod_cta_ctbl in frame f_bas_10_ccusto_razao
         bt_zoo_226186 in frame f_bas_10_ccusto_razao.
    assign cta_ctbl.des_tit_ctbl:column in frame f_bas_10_ccusto_razao = 46.14
           v_log_alter = no.
    if avail cta_ctbl then 
       assign cta_ctbl.cod_cta_ctbl:screen-value in frame 
           f_bas_10_ccusto_razao = string(cta_ctbl.cod_cta_ctbl).        

END PROCEDURE. /* pi_v_log_alter_yes_ccusto */
/*****************************************************************************
** Procedure Interna.....: pi_avail_cenar_ctbl_ccusto
** Descricao.............: pi_avail_cenar_ctbl_ccusto
** Criado por............: Julio
** Criado em.............: 19/03/1999 14:55:12
** Alterado por..........: src370
** Alterado em...........: 07/11/2000 17:18:52
*****************************************************************************/
PROCEDURE pi_avail_cenar_ctbl_ccusto:
...
END PROCEDURE. /* pi_avail_cenar_ctbl_ccusto */
/*****************************************************************************
** Procedure Interna.....: pi_avail_ccusto_bt_ent
** Descricao.............: pi_avail_ccusto_bt_ent
** Criado por............: Julio
** Criado em.............: 19/03/1999 14:59:56
** Alterado por..........: Menna
** Alterado em...........: 03/12/1999 17:40:08
*****************************************************************************/
PROCEDURE pi_avail_ccusto_bt_ent:

    find first tt_ccusto no-lock
         where tt_ccusto.cod_empresa = emscad.ccusto.cod_empresa
           and tt_ccusto.cod_plano_ccusto = ccusto.cod_plano_ccusto
           and tt_ccusto.cod_ccusto  = ccusto.cod_ccusto no-error.
    if  avail tt_ccusto
    then do:
        assign v_rec_ccusto = tt_ccusto.ttv_rec_ccusto
               v_cod_ccusto = tt_ccusto.cod_ccusto.
    end /* if */.
    else do:
        /* Usu†rio sem permiss∆o para acessar o centro de custo ! */
        run pi_messages (input "show",
                         input 1926,
                         input substitute ("&1~~&2~~&3~~&4~~&5~~&6~~&7~~&8~~&9",
                                           ccusto.cod_ccusto)) /*msg_1926*/.
        apply "entry" to ccusto.cod_ccusto in frame f_bas_10_ccusto_razao.
        return "NOK" /*l_nok*/ .
    end /* else */.
    return "OK" /*l_ok*/ .

END PROCEDURE. /* pi_avail_ccusto_bt_ent */
/*****************************************************************************
...
/*****************************************************************************
** Procedure Interna.....: pi_busca_ccusto_mgl204
** Descricao.............: pi_busca_ccusto_mgl204
** Criado por............: fut35183
** Criado em.............: 18/02/2009 14:33:13
** Alterado por..........: fut35183
** Alterado em...........: 18/02/2009 14:54:03
*****************************************************************************/
PROCEDURE pi_busca_ccusto_mgl204:

    if Verifica_Program_Name('mgl204aa':U,30) then
        find emscad.ccusto no-lock where recid(ccusto) = v_rec_ccusto_aux no-error.

END PROCEDURE. /* pi_busca_ccusto_mgl204 */
