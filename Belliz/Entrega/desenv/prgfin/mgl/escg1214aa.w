...
*******************************************************************************/
{include/i-prgvrs.i ESCG1214aa 5.12.16.100}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod_situacao bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod_situacao 
...
DEFINE VARIABLE i_cod_situacao AS INTEGER FORMAT "->,>>>,>>9":U INITIAL 0 
     LABEL "Situacao" 
     VIEW-AS FILL-IN 
     SIZE 10.29 BY .92 NO-UNDO.
...
DEFINE FRAME d-vapara
     i_cod_situacao AT ROW 1.75 COL 29.86 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Situacao */
DO:
FIND ext_situacao_orcto NO-LOCK
    WHERE ext_situacao_orcto.cod_situacao EQ INPUT FRAME {&FRAME-NAME} i_cod_situacao NO-ERROR.

IF NOT AVAILABLE ext_situacao_orcto THEN DO:
    {utp/ut-table.i mgesp ext_situacao_orcto 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(ext_situacao_orcto).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Situacao */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod_situacao 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod_situacao bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCG1214" "5.12.16.100"}
