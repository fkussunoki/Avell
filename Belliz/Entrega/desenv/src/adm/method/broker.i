...
------------------------------------------------------------------------------*/
 ...
 /**Atualiza‡Æo 06/10/06 por tech30713 - FO: 1391535  
     Erro: Attribute MOVE-AFTER-TAB-ITEM for the FRAME Panel-Frame was passed an invalid widget. (4062) **/
  if  (can-query(p-anchor,"no-focus") and p-anchor:no-focus = no) or
      (not can-query(p-anchor,"no-focus")) then
      IF p-method = "BEFORE":U then
         adm-object-handle:MOVE-BEFORE(p-anchor).
      ELSE
         adm-object-handle:MOVE-AFTER(p-anchor).
  /**Fim atualiza‡Æo 06/10/06 por tech30713 - FO: 1391535  */
