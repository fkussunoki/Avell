...
------------------------------------------------------------------------------*/
    ...
    /* If this method is one that needs an enclosing transaction block
       ...
       in the transaction. */
/* altera‡Æo feita para setar o private-data garantindo a facil altera‡Æo
   das customiza‡äes ap¢s a duplica‡Æo dos objetos */
    define var c-private-data as character no-undo.
    if p-caller:PRIVATE-DATA <> "ok" then
      assign c-private-data2 = p-caller:PRIVATE-DATA.
    ...
    DO:
      DO TRANSACTION ON ERROR  UNDO, RETURN "ADM-ERROR":U
         ...
         /**********************************************************************
         * Alterado: Andr‚ C. de Gruiter
         * Data    : 23 de Fevereiro de 1999
         * Motivo  : Progress 9 usa 6456 ao inv‚s de 2129, logo, ‚ necess rio
         *           preparar a include para o uso na 9 e na 8.
         ***********************************************************************/
        &IF substr(proversion, 1, 1) = "8" &THEN
          IF ERROR-STATUS:ERROR AND ERROR-STATUS:GET-NUMBER(1) <> 2129
              THEN UNDO, RETURN "ADM-ERROR":U.
        &ENDIF
        
        /*&IF substr(proversion, 1, 1) = "9" &THEN*/
        &IF INTEGER(ENTRY(1,PROVERSION,".")) >= 9 &THEN /* ALTERA€ÇO FEITA PARA ATENDER A VERSAO 10 - MARCILENE - 26-11-2003 */
          IF ERROR-STATUS:ERROR AND ERROR-STATUS:GET-NUMBER(1) <> 6456
              THEN UNDO, RETURN "ADM-ERROR":U.
        &ENDIF
    ...
    ELSE do:
      if p-caller:PRIVATE-DATA <> "ok" then
        ...
        &IF "{&mguni_version}" >= "2.06B" OR "{&aplica_facelift}" = "YES" &THEN
        /*Alterado 11/03/2006 - tech1007/tech14187 - Alteracao para contemplar funcionalidade de faceLift em Viewers*/
	    /**** Alteracao efetuada por tech14187/tech1007/tech38629 para o projeto Facelift ****/
        IF p-method-name = "local-enable-fields" THEN DO:
            if valid-handle(p-caller) then do:
                ASSIGN hFrame = p-caller:CURRENT-WINDOW.
        ...
        IF p-method-name = "local-copy-record" OR p-method-name = "adm-copy-record" THEN DO:
            if valid-handle(p-caller) then do:
                ASSIGN hFrame = p-caller:CURRENT-WINDOW.
                if valid-handle(hFrame) then do:
                    ASSIGN hFrame = hFrame:FIRST-CHILD.
        ...
        END.
        IF p-method-name = "local-add-record" OR p-method-name = "adm-add-record" THEN DO:
            if valid-handle(p-caller) then do:
                ASSIGN hFrame = p-caller:CURRENT-WINDOW.
                if valid-handle(hFrame) then do:
                    ASSIGN hFrame = hFrame:FIRST-CHILD.
    ...
    if p-caller:PRIVATE-DATA = ? OR
       p-caller:PRIVATE-DATA = "ok" then
    assign p-caller:PRIVATE-DATA = c-private-data2.
    ...
    DO:
      ...
      /**********************************************************************
      * Alterado: John C. Jaraceski
      * Data    : 23 de Mar‡o de 1999
      * Motivo  : Progress 9 usa 6456 ao inv‚s de 2129, logo, ‚ necess rio
      *           preparar a include para o uso na 9 e na 8.
      ***********************************************************************/
      &IF substr(proversion, 1, 1) = "8" &THEN
      IF adm-show-dispatch-errors OR
        ERROR-STATUS:GET-NUMBER(1) <> 2129 THEN  /* procedure not found */ 
        DO:
      ...
      /*&IF substr(proversion, 1, 1) = "9" &THEN*/
      &IF INTEGER(ENTRY(1,PROVERSION,".")) >= 9 &THEN /* ALTERA€ÇO FEITA PARA O WEBENABLER - MARCILENE - 26-11-2003 */
      IF adm-show-dispatch-errors OR
...
/* -----------------------------------------------------------
    ...
    DEFINE BUFFER   adm-link-buffer FOR adm-link-table.
    if p-source-hdl:private-data <> "OK" then
       assign c-private-data2 = p-source-hdl:private-data.
    ...
    DO:                           /* Send to the link in the default list */
        ...
        END.
    END.
    assign p-source-hdl:private-data = c-private-data2.
    RETURN.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  assign p-caller:private-data = p-caller:file-name.  
  IF VALID-HANDLE(adm-watchdog-hdl) THEN
  DO:
    RUN receive-message IN adm-watchdog-hdl 
       (INPUT p-caller, INPUT "":U,
...
/************ Customizacao TOTVS S.A. - John Cleber Jaraceski - 14/09/1999
              Implementado teste do valor da vari vel attr-name, pois quando
              est  possuir o conte£do "create-on-add", nÆo deve ser utilizada.
              **********************************/
      
      IF TRIM(attr-name) <> TRIM("create-on-add":U) THEN DO:
