...
/*--------------------------------------------------------------------------
    File        : c-broker.i
    Purpose     : Customiza»„es do Broker
...
------------------------------------------------------------------------------*/
 ...
 IF  c_Container = "?" THEN RETURN "ADM-ERROR":U.
...
------------------------------------------------------------------------------*/
  ...
  RUN get-link-handle IN adm-broker-hdl (INPUT h_Container, 
...
DEFINE VAR   c_Type                   AS CHAR INITIAL "Fill-in,Radio-set,Combo-box,Editor,Selection-List,Toggle-box,Slider":U.
DO :
...
FOR FIRST adm-link-table WHERE adm-link-table.link-target = h_Issuer-hdl AND 
                               adm-link-table.link-type <> "PAGE":U AND
                               adm-link-table.link-type BEGINS "PAGE":U NO-LOCK :
...
RUN get-link-handle  (h_Issuer-hdl,"CONTAINER-SOURCE":U, OUTPUT c_Container).
