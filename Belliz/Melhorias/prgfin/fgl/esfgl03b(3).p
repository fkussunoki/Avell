...
**   Descricao.............: Relat�rio Apropria��es Cont�beis Espec�fico
**   Criado por............: Henke
**   Criado em.............: 25/02/2006
**   Vers�o em.............: 27/02/2006 - Henke - 1.01.00.000 - Acerto RPW e elimina��o de temp-tables;
**                           02/03/2006 - Henke - 1.01.01.000 - Apresenta��o de Lote, Lancto e Seq;
...
**                           22/03/2006 -olado os gatilhos para compilar somente
**                           no Windows;
**                           10/08/2006 - Henke - 1.03.00.000 - L� somente o 1o. item de lancto ctbl do
...
&if "{&window-system}" <> "TTY" &then 
    ...
    " Impress�o/Execu��o " view-as text 
...
&endif 
    ...
    with 1 down side-labels no-validate keep-tab-order three-d 
         ...
         view-as dialog-box 
         font 1 fgcolor ? bgcolor 8 
         title "Relat�rio Apropria��o Cont�bil - ESFGL03A - 1.03.00.000". 
...
def frame f_log 
    ...
    v_log_print_par
         at row 3.48 col 10.00 label "Imprime Par�metros"
         help "Imprime Par�metros" 
    rt_001 
...
procedure pi_vld_param:
   ...
   if not avail finalid_econ then do:
       ...
       message "Finalidade Econ�mica inexistente!" view-as ALERT-BOX INFORMATION.
       return error.
   end.
   find plano_cta_ctbl no-lock 
...
procedure pi_salva_param:
    ...
    /* Recuperar par�metros da �ltima execu��o */
    do transaction:
...
procedure pi_esp_aprop_ctbl_impr:
   ...
   if avail emsuni.empresa then do:
      assign v_cod_cta_ctbl     = ""
             ...
             v_nom_report_title = "Relat�rio Apropria��es Cont�beis Espec�fico"
...
procedure pi_pesq_item_lancto_ctbl:
   ...
   if v_log_lote_ctbl = no then do:
       for each cta_ctbl no-lock
          ...
          if cta_ctbl.ind_espec_cta_ctbl = "Sint�tica" then 
             next.
...
procedure pi_impr_tt_item_lancto_ctbl:
   ...
   for each tt_item_lancto_ctbl
       ...
       if v_log_cpart = yes then
           run pi_impr_cpart (input no /* "Sint�tico" */,
...
procedure pi_aprop_lancto_ctbl_cep:
    ...
    if avail item_lancto_ctbl then do:
       ...
       **/   
       ...
       /* Verifica se � Conta Ctbl de Saldo ou de Impostos */
...
procedure pi_impr_cabec:
   ...
   if v_log_analit = yes then do:
       if v_log_cpart = no then
           put stream s_plha unformatted 
               "Data;Lote|Lancto|Seq;Cta Ctbl;Descri��o da Conta;Ccusto;Descri��o do C.C;Dep;Esp;Ser;Docto;Parc;Emit;Nome Emit;Item;Un;Qtd;"
               "Movto D�bito;Movto Cr�dito;Saldo;Origem;Historico;Obs. Pedido"
               skip.
       else
           put stream s_plha unformatted 
               "Data;Lote|Lancto|Seq;Cta Ctbl;Descri��o da Conta;Ccusto;Descri��o do C.C;"
               "Seq Cpart;Cta Ctbl Cpart;T�tulo;Ccusto Cpart;T�tulo;Estab Cpart;Nome Abrev;Un Neg Cpart;Descri��o;"
               "Dep;Esp;Ser;Docto;Parc;Emit;Nome Emit;Item;Un;Qtd;Movto D�bito;Movto Cr�dito;Saldo ;Origem;Historico;Obs. Pedido"
               skip.
   end.
