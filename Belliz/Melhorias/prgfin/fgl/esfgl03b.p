...
if v_num_ped_exec_corren <> 0 then do:
   ...
   run pi_esp_aprop_ctbl_impr.
    ...
    repeat on endkey undo main_block, leave main_block
       ...
       IF v_rec_empresa_1 = ? THEN DO:
       
           FIND FIRST emscad.empresa NO-LOCK WHERE emscad.empresa.cod_empresa = v_cod_empres_usuar NO-ERROR.

       ASSIGN v_rec_empresa_1 = RECID(emscad.empresa). 
       END.
