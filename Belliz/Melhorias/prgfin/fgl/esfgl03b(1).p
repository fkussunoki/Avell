...
on leave of v_cod_plano_ccusto in frame f_param do:
   ...
   if avail plano_ccusto then do:
      ...
      if v_cod_ccusto_ini = "" then 
         assign v_cod_ccusto_ini    = fill ("0", length (v_cod_format_ccusto))
                v_cod_ccusto_fim    = fill ("9", length (v_cod_format_ccusto)).
...
PROCEDURE pi_justificativa:
    ...
    FIND FIRST cta_ctbl NO-LOCK WHERE RECID(cta_ctbl) = v_rec_cta_ctbl NO-ERROR.

    ASSIGN v_cod_cta_ctbl_ini  = cta_ctbl.cod_cta_ctbl  
           v_cod_cta_ctbl_fim  = cta_ctbl.cod_cta_ctbl
           v_cod_cta_ctbl_ini:SENSITIVE IN FRAME f_select= NO
           v_cod_cta_ctbl_fim:SENSITIVE IN FRAME f_select= NO. 

    FIND FIRST demonst_ctbl NO-LOCK WHERE RECID(demonst_ctbl) = v_rec_demonst_ctbl NO-ERROR.

    ASSIGN v_cod_ccusto_ini    = SUBSTRING(demonst_ctbl.cod_demonst_ctbl,1, 4)
           v_cod_ccusto_fim    = SUBSTRING(demonst_ctbl.cod_demonst_ctbl,1, 4)
           v_cod_ccusto_ini:SENSITIVE IN FRAME f_select = NO
