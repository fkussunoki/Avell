...
define var v-ccusto-fim as char no-undo.
define var v-ccusto-ini as char no-undo.
...
DEFINE VARIABLE c-ccusto-fim AS CHARACTER FORMAT "X(5)":U INITIAL "zzzzzzzz" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE c-ccusto-ini AS CHARACTER FORMAT "X(5)":U 
     LABEL "Demonstrativo" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.
...
/* SETTINGS FOR FILL-IN c-cenario-orc IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN c-empresa IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
DO:
    ...
    THEN DO:
        MESSAGE "ê necess†rio preencher todos os campos" VIEW-AS ALERT-BOX.
    RETURN.
    END.
...
&Scoped-define FRAME-NAME f-pg-sel
&Scoped-define SELF-NAME c-ccusto-fim
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-ccusto-fim w-relat
ON LEAVE OF c-ccusto-fim IN FRAME f-pg-sel
DO:


  
    IF c-empresa:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "MAE" THEN DO:

     assign v-ccusto-fim = c-ccusto-fim:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
            
            if c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "" then
            assign v-ccusto-fim = "zzzzzz".

                   else 
                   assign v-ccusto-fim = v-ccusto-fim + "m".

        ASSIGN c-ccusto-fim:SCREEN-VALUE IN FRAME {&FRAME-NAME} = v-ccusto-fim.
    END.
END.
...
&Scoped-define SELF-NAME c-ccusto-ini
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL c-ccusto-ini w-relat
ON LEAVE OF c-ccusto-ini IN FRAME f-pg-sel /* Demonstrativo */
DO:

  IF c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} BEGINS "6" THEN DO:

      ASSIGN c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "6000"
             c-ccusto-fim:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "6999"
             c-ccusto-fim:SENSITIVE = NO.

  END.

  ELSE DO:
      ASSIGN c-ccusto-fim:SCREEN-VALUE IN FRAME {&FRAME-NAME} = INPUT FRAME {&FRAME-NAME} c-ccusto-ini
             c-ccusto-fim:SENSITIVE = YES.
  END.


  IF c-empresa:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "MAE" THEN DO:

     assign v-ccusto-ini = c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME}.
            
            if c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "" then
            assign v-ccusto-ini = "".

                   else 
                   assign v-ccusto-ini = v-ccusto-ini + "m".

        ASSIGN c-ccusto-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} = v-ccusto-ini.
    END.
...
ON F5 OF seq-orcto IN FRAME f-pg-sel /* Sequencia Oráamento */
DO:
        ...
        assign c-cenario-orc:screen-value  in frame f-pg-sel = string(vers_orcto_ctbl_bgc.cod_cenar_orctario)
               ...
               c-unid-orctaria:SCREEN-VALUE IN FRAME f-pg-sel = string(vers_orcto_ctbl_bgc.cod_unid_orctaria).
                ...
                IF vers_orcto_ctbl_bgc.cod_unid_orctaria = "Belliz" THEN
                ASSIGN c-empresa:SCREEN-VALUE IN FRAME f-pg-sel = "BLZ".
                ELSE
                ASSIGN c-empresa:SCREEN-VALUE IN FRAME f-pg-sel = "MAE".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL seq-orcto w-relat
ON LEFT-MOUSE-DBLCLICK OF seq-orcto IN FRAME f-pg-sel /* Sequencia Oráamento */
DO:
  APPLY 'f5' TO SELF.
END.
...
ON F5 OF seq-orcto-3 IN FRAME f-pg-sel /* Sequencia Oráamento */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL seq-orcto-3 w-relat
ON LEFT-MOUSE-DBLCLICK OF seq-orcto-3 IN FRAME f-pg-sel /* Sequencia Oráamento */
DO:
  APPLY 'f5' TO SELF.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE c-cenario-ctbl seq-orcto-3 seq-orcto c-cta-ini c-cta-fim c-ccusto-ini 
         c-ccusto-fim c-dt-ini c-dt-fim c-arquivo-entrada bt-arquivo-entrada 
         IMAGE-5 IMAGE-6 IMAGE-7 IMAGE-8 IMAGE-9 IMAGE-10 RECT-8 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
