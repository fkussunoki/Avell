...
DEF TEMP-TABLE tt-permissao
    FIELD ttv-cod_demonst_ctbl AS char
    FIELD ttv-cod-ccusto       AS CHARACTER
    FIELD ttv-grp-usuar        AS char.
...
def new global shared var v_cod_grp_usuar_lst as CHARACTER no-undo.
/* ---------------------------------------------------------------------------------------- */
/* ----------------------------- Configura‡äes Excel -------------------------------------- */
/* ---------------------------------------------------------------------------------------- */
...
DEF VAR c-custo-ini AS char.
DEF VAR c-custo-fim AS char.
DEF VAR v-controle  AS INTEGER.
...
ASSIGN c-custo-fim = substring(tt-param.ttv-cc-fim, 1, 4)
       c-custo-ini = substring(tt-param.ttv-cc-ini, 1, 4).
...
RUN pi-inicializar IN h-prog("Montando relatorio.....").
    ...
    FIND FIRST tt-param NO-ERROR.
    IF substring(tt-param.ttv-cc-ini, 1, 4) = "1216" THEN
        RUN pi-cria-temp-tables-rh.
    ELSE
...
PROCEDURE pi-verifica-permissao:
    ...
    do v_num_cont = 1 to num-entries(v_cod_grp_usuar_lst):
        ...
        FOR EACH demonst_ctbl NO-LOCK WHERE demonst_ctbl.cod_demonst_ctbl >= c-custo-ini    
                                      AND   demonst_ctbl.cod_demonst_ctbl <= c-custo-fim:

            FIND FIRST segur_demonst_ctbl NO-LOCK WHERE segur_demonst_ctbl.cod_demonst_ctbl = demonst_ctbl.cod_demonst_ctbl
                                                  AND   (segur_demonst_ctbl.cod_grp_usuar    = entry(v_num_cont, v_cod_grp_usuar_lst)
                                                  OR    segur_demonst_ctbl.cod_grp_usuar    = '*') NO-ERROR.

            IF AVAIL segur_demonst_ctbl THEN DO:
                CREATE tt-permissao.
                ASSIGN tt-permissao.ttv-cod_demonst_ctbl = segur_demonst_ctbl.cod_demonst_ctbl
                       tt-permissao.ttv-cod-ccusto       = SUBSTRING(segur_demonst_ctbl.cod_demonst_ctbl, 1, 4)
                       tt-permissao.ttv-grp-usuar        = entry(v_num_cont, v_cod_grp_usuar_lst).
            END.
    ...
    FOR EACH blz-sdo_ctbl:
        ...
        FIND FIRST tt-permissao WHERE substring(tt-permissao.ttv-cod_demonst_ctbl, 1 , 4) = blz-sdo_ctbl.cod_ccusto NO-ERROR.
...
PROCEDURE pi-cria-temp-tables-rh:
...
INPUT CLOSE.
    ...
    FOR EACH sdo_ctbl NO-LOCK WHERE  sdo_ctbl.cod_empresa                                              = tt-param.ttv-empresa
                               ...
                               AND   sdo_ctbl.cod_ccusto                                               >= "1000"
                               AND   sdo_ctbl.cod_ccusto                                               <  "6000"
...
//               v-vlr-orcado      = sdo_orcto_ctbl_bgc.val_orcado
     ...
     FOR EACH blz-sdo_ctbl:
         ...
         CASE substring(blz-sdo_ctbl.cod_ccusto,1, 1):
             ...
             WHEN "6" THEN
             ASSIGN blz-sdo_ctbl.cod_ccusto         = IF blz-sdo_ctbl.cod_empresa = "MAE" THEN "6000" ELSE "6100".
          OTHERWISE
...
INPUT CLOSE.
    ...
    FOR EACH sdo_ctbl NO-LOCK WHERE  sdo_ctbl.cod_empresa                                              = tt-param.ttv-empresa
                               AND   sdo_ctbl.cod_finalid_econ                                         = "corrente"
                               AND   sdo_ctbl.cod_ccusto                                               >= SUBSTRING(tt-param.ttv-cc-ini, 1, 4)

                               AND   sdo_ctbl.cod_ccusto                                               <= SUBSTRING(tt-param.ttv-cc-fim, 1, 4)
                               AND   sdo_ctbl.cod_cta_ctbl                                             >= tt-param.ttv-cta-ini
                               AND   sdo_ctbl.cod_cta_ctbl                                             <= tt-param.ttv-cta-fim
                               AND   sdo_ctbl.dat_sdo_ctb                                              >= tt-param.ttv-periodo-ini
                               AND   sdo_ctbl.dat_sdo_ctb                                              <= tt-param.ttv-periodo-fim
...
PROCEDURE pi-logica-fixa:
   ...
   FOR EACH blz-sdo_ctbl NO-LOCK WHERE blz-sdo_ctbl.cod_empresa                                          = tt-param.ttv-empresa
                             ...
                             AND   blz-sdo_ctbl.cod_ccusto                                               >= SUBSTRING(tt-param.ttv-cc-ini, 1, 4) 
                             AND   blz-sdo_ctbl.cod_ccusto                                               <= SUBSTRING(tt-param.ttv-cc-fim, 1, 4)
...
/*                                 IF LAST-OF(blz-sdo_ctbl.cod_empresa + blz-sdo_ctbl.cod_cta_ctbl + blz-sdo_ctbl.cod_ccusto + string(blz-sdo_ctbl.dat_sdo_ctbl)) THEN DO:  */
   ...
   IF TT-PARAM.TTV-EMPRESA = "BLZ" THEN DO:
       ...
       ASSIGN col-ini = col-ini + 1.
          ...
          IF ext_justificativa.situacao = "<selecione>" THEN
