...
def var c-versao-prg as char initial " 5.12.16.109":U no-undo.
def var c-versao-rcode as char initial "[[[5.12.16.109[[[":U no-undo. /* Controle de Versao R-CODE - Nao retirar do Fonte */
...
def new global shared var v_rec_empresa
    ...
    format ">>>>>>9":U
    no-undo.
def new global shared var v_rec_empresa_1
    as recid
    format ">>>>>>9":U
    no-undo.
...
DEF NEW GLOBAL SHARED VAR v_periodo_ini AS DATE NO-UNDO.
DEF NEW GLOBAL SHARED VAR v_periodo_fim AS DATE NO-UNDO.
...
DO:
    ...
    assign v_nom_prog     = substring(current-window:title, 1, max(1, length(current-window:title) - 10))
           ...
           v_cod_release  = trim(" 5.12.16.109":U).
...
run pi_cria_menu (input "Razao (ESFGL03B)" /*l_razao_cta_ctbl*/ ,
                  input "Normal" /*l_normal*/ ,
                  input 9,
                  input no,
                  input menu m_menu:handle,
...
run pi_cria_menu (input "Razao (ESFGL03B)" /*l_razao_ccusto*/ ,
                  input "Normal" /*l_normal*/ ,
                  input 10,
                  input no,
                  input menu m_menu:handle,
...
/*                                                                                                                                              */
    ...
    then do:
        ...
        if  search("prgfin/fgl/esfgl03b.p") = ? and search("prgfin/fgl/esfgl03b.p") = ? then do:
            if  v_cod_dwb_user begins 'es_' then
                return "Programa execut†vel n∆o foi encontrado:" /*l_programa_nao_encontrado*/  + "prgfin/fgl/esfgl03b.p".
            else do:
                message getStrTrans("Programa execut†vel n∆o foi encontrado:", "MGL") /*l_programa_nao_encontrado*/  "prgfin/fgl/esfgl03b.p"
                       view-as alert-box error buttons ok.
                return.
            end.
        end.
        else
            run prgfin/fgl/esfgl03b.p /*prg_bas_cta_ctbl_razao*/.
    END.
    else do:
        ...
        if  search("prgfin/fgl/esfgl03b.p") = ? and search("prgfin/fgl/esfgl03b.p") = ? then do:
            ...
            run prgfin/fgl/esfgl03b.p /*prg_bas_ccusto_razao*/.
    END.
    ...
    END. /* begins ( */
     ...
     END. /* nao comeca com ( */
        ...
        END.
             ...
             FIND FIRST mgesp.ext_exc_justificativas WHERE ext_exc_justificativas.cod_padr_coluna = conjto_prefer_demonst.cod_padr_col_demonst_ctbl
             ...
             IF AVAIL tt-receitas THEN DO:
                 ...
                 IF (v_coluna4) <= (100 - v_var_ccusto) 
                    ...
                    IF (absolute(v_coluna2) - ABSOLUTE(v_coluna3)) > v_vlr_minimo THEN DO: /* era R$ 300,00*/
                     ...
                     FIND FIRST mgesp.ext_justificativa NO-LOCK WHERE ext_justificativa.Cod_demonst = demonst_ctbl.des_tit_ctbl
...
/*****************************************************************************
** Procedure Interna.....: pi_prepara_var_global
** Descricao.............: pi_prepara_var_global
** Criado por............: fut1180
** Criado em.............: 26/07/2004 11:41:52
** Alterado por..........: fut1180
** Alterado em...........: 14/09/2005 17:33:02
*****************************************************************************/
PROCEDURE pi_prepara_var_global:
    ...
    if  v_cod_demonst_ctbl <> '' then do:
        ...
        assign v_num_ano_aux = integer(string(substr(v_cod_exec_period_1,1,4),'9999'))
               v_num_mes_aux = int(STRING(substr(v_cod_exec_period_1,5,2),'99')).
        ASSIGN 
    v_periodo_ini = dat_inic_period_ctbl
    v_periodo_fim = dat_fim_period_ctbl.
    end.
    ...
    FIND cta_ctbl NO-LOCK
        WHERE cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
          AND cta_ctbl.cod_cta_ctbl = TRIM(ENTRY(1, tt_demonst_ctbl_fin.ttv_cod_campo_1, "|")) NO-ERROR.
    ...
    IF tt_demonst_ctbl_fin.tta_cod_ccusto_filho = "" THEN DO:
        ...
        &else
            ASSIGN v_cod_ccusto_inic_aux = substring(conjto_prefer_demonst.cod_demonst_ctbl, 1, 4)
                   v_cod_ccusto_fim_aux  = substring(conjto_prefer_demonst.cod_demonst_ctbl, 1, 4).
        &endif
    END.


    IF LENGTH(v_cod_demonst_ctbl) = 4 THEN DO:
    
    find emscad.empresa no-lock
        where empresa.cod_empresa = "BLZ" no-error.

        assign v_rec_empresa_1           = recid(empresa).

    END.

        ELSE DO:
            find emscad.empresa no-lock
            where empresa.cod_empresa = "MAE" no-error.
     ...
     ASSIGN v_rec_cenar_ctbl        = recid(cenar_ctbl) 
            ...
            v_cod_ccusto            = v_cod_ccusto_inic_aux
            v_cod_ccusto_final      = v_cod_ccusto_fim_aux
    ...
    ELSE DO:
        FIND cta_ctbl NO-LOCK
            WHERE cta_ctbl.cod_plano_cta_ctbl = plano_cta_ctbl.cod_plano_cta_ctbl
              AND cta_ctbl.cod_cta_ctbl = TRIM(replace(ENTRY(1, tt_demonst_ctbl_fin.ttv_cod_campo_1, "- "), ".", "")) NO-ERROR.
