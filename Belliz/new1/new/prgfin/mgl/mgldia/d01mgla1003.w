...
*******************************************************************************/
{include/i-prgvrs.i d01mgla1003 1.00.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod_usuario bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod_usuario 
...
DEFINE VARIABLE c_cod_usuario AS CHARACTER FORMAT "x(12)":U 
     LABEL "Usu rio" 
     VIEW-AS FILL-IN 
     SIZE 28.14 BY .92 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_cod_usuario AT ROW 1.88 COL 12.57 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Usuario */
DO:
FIND usuar_mestre NO-LOCK
    WHERE usuar_mestre.cod_usuario EQ INPUT FRAME {&FRAME-NAME} c_cod_usuario NO-ERROR.

IF NOT AVAILABLE usuar_mestre THEN DO:
    {utp/ut-table.i emsfnd usuar_mestre 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(usuar_mestre).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Usuario */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod_usuario 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod_usuario bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "D01MGLA1003" "1.00.00.001"}
