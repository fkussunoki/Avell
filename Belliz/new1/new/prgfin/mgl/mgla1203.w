...
*******************************************************************************/
{include/i-prgvrs.i MGLA1001 1.00.00.001}
...
DEFINE VARIABLE h_b01mgla1203 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qrymgla1201 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vwrmgla1201-01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     BUTTON-1 AT ROW 16.42 COL 9.57 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-filho ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Usuario x Nivel"
         HEIGHT             = 16.75
         WIDTH              = 90.14
         MAX-HEIGHT         = 21.21
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadpaifilho-filho /* Manuten‡Æo Usuario x Nivel */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Manuten‡Æo Usuario x Nivel */
DO:
...
ON CHOOSE OF BUTTON-1 IN FRAME f-cad /* Incluir */
DO:

    run pi-posicao-query  in h_qrymgla1201(output v-row-1).
  RUN prgfin/mgl/mgla1203aa.w (INPUT v-row-1).

  RUN pi-refresh IN h_b01mgla1203.
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglvwr/vwrmgla1201-01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vwrmgla1201-01 ).
       RUN set-position IN h_vwrmgla1201-01 ( 3.08 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 82.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglbrw/b01mgla1203.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01mgla1203 ).
       RUN set-position IN h_b01mgla1203 ( 9.33 , 9.57 ) NO-ERROR.
       /* Size in UIB:  ( 6.75 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglqry/qrymgla1201.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qrymgla1201 ).
       RUN set-position IN h_qrymgla1201 ( 1.50 , 26.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_vwrmgla1201-01. */
       RUN add-link IN adm-broker-hdl ( h_qrymgla1201 , 'Record':U , h_vwrmgla1201-01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartBrowser h_b01mgla1203. */
       RUN add-link IN adm-broker-hdl ( h_qrymgla1201 , 'Record':U , h_b01mgla1203 ).

       /* Links to SmartQuery h_qrymgla1201. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_qrymgla1201 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_qrymgla1201 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_qrymgla1201 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vwrmgla1201-01 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_vwrmgla1201-01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01mgla1203 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "MGLA1203- Relaciona Usuario x Nivel" "1.00.00.000"}
