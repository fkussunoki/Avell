...
*******************************************************************************/
{include/i-prgvrs.i MGLA1211 1.00.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button BUTTON-3 
...
DEFINE VARIABLE h_brw01mgla1211 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qrymgla1211 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vwrmgla1211 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     BUTTON-3 AT ROW 16.38 COL 2.43 WIDGET-ID 2
...
ON END-ERROR OF w-concom /* Relaciona Excecao */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Relaciona Excecao */
DO:
...
ON CHOOSE OF BUTTON-3 IN FRAME f-cad /* Incluir */
DO:
      run pi-posicao-query  in h_qrymgla1211(output v-row-1).
     RUN prgfin/mgl/mgla1211aa.w (INPUT v-row-1).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglvwr/vwrmgla1211.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vwrmgla1211 ).
       RUN set-position IN h_vwrmgla1211 ( 3.25 , 2.14 ) NO-ERROR.
       /* Size in UIB:  ( 3.33 , 79.72 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglbrw/brw01mgla1211.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_brw01mgla1211 ).
       RUN set-position IN h_brw01mgla1211 ( 9.58 , 17.43 ) NO-ERROR.
       /* Size in UIB:  ( 6.75 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglqry/qrymgla1211.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qrymgla1211 ).
       RUN set-position IN h_qrymgla1211 ( 1.67 , 38.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_vwrmgla1211. */
       RUN add-link IN adm-broker-hdl ( h_qrymgla1211 , 'Record':U , h_vwrmgla1211 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartBrowser h_brw01mgla1211. */
       RUN add-link IN adm-broker-hdl ( h_vwrmgla1211 , 'Record':U , h_brw01mgla1211 ).

       /* Links to SmartQuery h_qrymgla1211. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_qrymgla1211 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             BUTTON-3:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vwrmgla1211 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_vwrmgla1211 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_brw01mgla1211 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "mgla1211" "1.00.00.001"}
