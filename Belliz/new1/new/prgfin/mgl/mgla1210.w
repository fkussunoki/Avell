...
DEFINE VARIABLE h_qrymgla1210 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vwrmgla1210 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Excecao Pacotes / Rubricas"
         HEIGHT             = 4.04
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Excecao Pacotes / Rubricas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Excecao Pacotes / Rubricas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglvwr/vwrmgla1210.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vwrmgla1210 ).
       RUN set-position IN h_vwrmgla1210 ( 3.17 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 77.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/mgl/mglqry/qrymgla1210.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_qrymgla1210 ).
       RUN set-position IN h_qrymgla1210 ( 1.42 , 57.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_vwrmgla1210. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_vwrmgla1210 ).
       RUN add-link IN adm-broker-hdl ( h_qrymgla1210 , 'Record':U , h_vwrmgla1210 ).

       /* Links to SmartQuery h_qrymgla1210. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_qrymgla1210 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vwrmgla1210 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "MGLA1210" "1.00.00.000"}
