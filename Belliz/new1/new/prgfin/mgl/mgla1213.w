...
*******************************************************************************/
{include/i-prgvrs.i MGLA1213 1.00.00.001}
...
DEFINE VARIABLE h_qrymgla1213 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vwrmgla1213 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 90.29 BY 5.88 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Nivel Usuario"
         HEIGHT             = 5.88
         WIDTH              = 90.29
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90.29
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Nivel Usuario */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Nivel Usuario */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'c:/orcamento/orcamento/prgfin/mgl/mglvwr/vwrmgla1213.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vwrmgla1213 ).
       RUN set-position IN h_vwrmgla1213 ( 2.96 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 3.33 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'c:/orcamento/orcamento/prgfin/mgl/mglqry/qrymgla1213.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = prgfin\mgl\mgla1213ab.p,
                     ProgVaPara = prgfin\mgl\mgla1213ac.p,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_qrymgla1213 ).
       RUN set-position IN h_qrymgla1213 ( 1.25 , 55.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.83 , 6.86 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_vwrmgla1213. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_vwrmgla1213 ).
       RUN add-link IN adm-broker-hdl ( h_qrymgla1213 , 'Record':U , h_vwrmgla1213 ).

       /* Links to SmartQuery h_qrymgla1213. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_qrymgla1213 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_qrymgla1213 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_qrymgla1213 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vwrmgla1213 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
