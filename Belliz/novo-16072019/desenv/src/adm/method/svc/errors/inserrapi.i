...
/*--------------------------------------------------------------------------
    Library    : method/svc/errors/inserrapi.i
    Purpose    : Include que insere erros na temp-table RowErrors a partir de uma API

    Parameters : 
        &Handledbo        : Handle do DBO onde serÆo criados os erros. Existe um
                            teste na include para verificar se o handle ‚ v lid ou nÆo.
    ...
    Author     : Valdir Bittencourt Junior.
...
IF VALID-HANDLE({&Handledbo}) THEN DO:
    ...
    &ELSEIF "{&ErrorType}":U = "INTERNAL":U &THEN
        ...
        /*--- Executa m‚todo que inclui erros (_insertError) ---*/
        RUN _insertError IN {&Handledbo} (INPUT {&ErrorNumber},
                                            INPUT "INTERNAL":U,
                                            INPUT "{&ErrorSubType}":U,
                                            INPUT "":U).
    &ELSE
        &IF "{&ErrorDescription}":U <> "":U &THEN
            ...
            RUN _insertErrorManual IN {&Handledbo} (INPUT {&ErrorNumber},
                                                      INPUT "{&ErrorType}":U,
                                                      INPUT "{&ErrorSubType}":U,
