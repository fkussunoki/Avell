...
&IF DEFINED(EPC-ENABLED) <> 0 &THEN
    ...
    or  c-nom-prog-upc-mg97 <> "" then do:
        {include/i-epc200.i3 &POSITION=BEFORE} /* cria os parƒmetros padräes na tt-epc */

        /* DPC */
        if  c-nom-prog-dpc-mg97  <> "" then do:
            run value( c-nom-prog-dpc-mg97  ) 
                     ( string("BEFORE-":U + caps(cMethodName)), 
...
&ENDIF
    ...
    if lExistsCustomBO then do:
        run value(cCustomBOName) ("start", cMethodName, input-output table RowObject).
    end.
end procedure.
