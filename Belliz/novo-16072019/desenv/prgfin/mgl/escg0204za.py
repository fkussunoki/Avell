...
then do:
    run pi_version_extract ('api_demonst_ctbl_video':U, 'prgfin/mgl/escg0204za.py':U, '5.12.16.100':U, 'pro':U).
end /* if */.
...
PROCEDURE pi_item_demonst_ctbl_video:
    ...
    if  can-find(first tt_input_sdo) then do:
        ...
        if v_log_sped then do:
            if  search("prgfin/mgl/escg0204zl.r") = ? and search("prgfin/mgl/escg0204zl.py") = ? then do:
                if  v_cod_dwb_user begins 'es_' then
                    return "Programa execut vel nÆo foi encontrado:" /*l_programa_nao_encontrado*/  + "prgfin/mgl/escg0204zl.py".
                else do:
                    message getStrTrans("Programa execut vel nÆo foi encontrado:", "MGL") /*l_programa_nao_encontrado*/  "prgfin/mgl/escg0204zl.py"
                           view-as alert-box error buttons ok.
                    return.
                end.
            end.
            else
                run prgfin/mgl/escg0204zl.py /*prg_api_demonst_ctbl_sped_recalc_sdo*/.
        end.
            ...
            if v_log_tit_demonst then do: 
                ...
                for each tt_col_demonst_ctbl no-lock
                    where tt_col_demonst_ctbl.cod_padr_col_demonst_ctbl = v_cod_padr_col_demonst_ctbl
                      and tt_col_demonst_ctbl.ind_orig_val_col_demonst  = "T¡tulo" /*l_titulo*/  :
                    assign v_num_cont_1 = v_num_cont_1 + 1
                           v_num_cont_2 = v_num_cont_1. /* A pi_busca_descricoes_video tbm ² tratado no escg0204aa, precisarÿ desta var*/
