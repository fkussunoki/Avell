...
*******************************************************************************/
{include/i-prgvrs.i CE0341 2.00.00.020 } /*** 010020 ***/
...
DEFINE VARIABLE l-prazo-contr AS LOGICAL INITIAL no 
     LABEL "Considera Prazo Entrega do Contrato" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-sel
     c-estabelec-ini AT ROW 3.46 COL 21.14 COLON-ALIGNED HELP
          ""
          LABEL "Estab":R7
     c-estabelec-fim AT ROW 3.46 COL 51.72 COLON-ALIGNED HELP
          "" NO-LABEL
...
DEFINE FRAME f-pg-dig
     ...
     bt-salvar AT ROW 10.46 COL 47 WIDGET-ID 4
     bt-recuperar AT ROW 10.46 COL 58 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.96
         SIZE 77.43 BY 10.75.
...
DEFINE FRAME f-pg-par
     l-pto-enc AT ROW 2.75 COL 12
     l-periodico AT ROW 4 COL 12
     l-narrativa AT ROW 7.5 COL 12
     l-multip AT ROW 8.75 COL 12
     l-split AT ROW 8.75 COL 12
     l-prazo-contr AT ROW 10 COL 12 WIDGET-ID 2
     cb-icms AT ROW 7.5 COL 45
     fi-requisitante AT ROW 8.75 COL 49 COLON-ALIGNED
...
DO:
    ...
    if l-ok then do:
        ...
        delete tt-digita.
        FOR EACH tt-digita:
            FIND FIRST necessidade-oc NO-LOCK
                 WHERE necessidade-oc.it-codigo = tt-digita.it-codigo
                   AND necessidade-oc.cod-estabel = tt-digita.cod-estabel NO-ERROR.
            IF AVAIL necessidade-oc THEN
            DO:
            ...
            DO:
                CREATE tt-orfaos.
                BUFFER-COPY tt-digita TO tt-orfaos.
            END.
        END.
        IF CAN-FIND(FIRST tt-orfaos) THEN
        DO:
            run utp/ut-msgs.p (input "show", input 54502, input "").
            if return-value = "yes" then do:
                DEFINE VARIABLE cArquivo AS CHARACTER   NO-UNDO.
                cArquivo = SESSION:TEMP-DIRECTORY + "ce0341-ignorados.d".

                OUTPUT TO VALUE(cArquivo).
                FOR EACH tt-orfaos:
                    FOR EACH tt-digita WHERE tt-digita.rw-nec = tt-orfaos.rw-nec:
                        DELETE tt-digita.
                    END.
                    EXPORT tt-orfaos EXCEPT rw-nec.
                END.
                OUTPUT CLOSE.

                run utp/ut-msgs.p (input "show", input 54503, input cArquivo).
            END.
            ELSE
            DO:
                RETURN NO-APPLY.
...
DO:
    ...
    if l-ok then do:
        ...
        for each tt-digita:
            export DELIMITER ';' tt-digita EXCEPT rw-nec.
        end.
        output close. 
...
DO:
    IF l-split:CHECKED IN FRAME {&FRAME-NAME} = YES THEN DO:
        ASSIGN l-prazo-contr:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
    END.
    ELSE DO:
        ASSIGN l-prazo-contr:CHECKED   IN FRAME {&FRAME-NAME} = NO
               l-prazo-contr:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
    END.
END.
...
{utp/ut9000.i "CE0341" "2.00.00.013"}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE br-digita bt-marca bt-todos bt-nenhum bt-salvar bt-recuperar 
  ...
  DISPLAY l-pto-enc l-periodico l-narrativa l-multip l-split l-prazo-contr 
          cb-icms fi-requisitante l-nec c-ordem 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE l-pto-enc l-periodico l-narrativa l-multip l-split l-prazo-contr 
         cb-icms fi-requisitante RECT-16 RECT-17 RECT-19 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    for each tt-digita no-lock:
        ...
        if tt-digita.marca = "*" AND tt-digita.data-entrega < today then do:
            run utp/ut-msgs.p (input "show",
                               input 8313,
                               input " ").
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.l-prazo-contr   = input frame f-pg-par l-prazo-contr
...
&IF '{&bf_mat_versao_ems}' >= '2.08' &THEN
    ASSIGN l-multip     :HIDDEN IN FRAME f-pg-par = YES
           l-split      :HIDDEN IN FRAME f-pg-par = NO
           l-prazo-contr:HIDDEN IN FRAME f-pg-par = NO.
    APPLY "VALUE-CHANGED":U TO l-split IN FRAME f-pg-par.
&ELSE
    ASSIGN l-multip     :HIDDEN IN FRAME f-pg-par = NO
           l-split      :HIDDEN IN FRAME f-pg-par = YES
           l-prazo-contr:HIDDEN IN FRAME f-pg-par = YES.
&ENDIF
